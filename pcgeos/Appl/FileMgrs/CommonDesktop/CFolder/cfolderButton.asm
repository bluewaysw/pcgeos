...
endif ; GPC_MAIN_SCREEN_LINK
		jmp	done
		
notLogout:
		cmp	es:[di].FR_desktopInfo.DI_objectType, WOT_SIGN_UP
		jne	notSignUp
if GPC_SIGN_UP_ICON
		call	NDSignUp
endif
...
fromSignUpKey		char	"fromSignUpIcon",0
fromRegKey		char	"fromRegisterIcon",0

NDSignUp	proc	near
		uses	ax, bx, cx, dx, ds, si, di, bp
		.enter
	;
	; set state for Welcome
	;
		call	UtilGetSignUpState	; ax = reg, bx = sign up
		mov	dx, offset fromRegKey	; assume need to register
		cmp	ax, TRUE
		jne	doReg
		cmp	bx, TRUE
		je	done			; nothing to do!
		mov	dx, offset fromSignUpKey	; else must be sign up
doReg:
		segmov	ds, cs, cx
	...
	;
		...
		mov	ax, MSG_DESKTOP_LAUNCH_FROM_TOKEN
		mov	bx, handle 0
...
if GPC_MAIN_SCREEN_LINK
welcomePermName	char "welcome "

GotoCUI	proc	near
		uses	ax, bx, cx, dx, si, di, bp, es
		.enter
	;
	; Send a message to switch to the CUI field. 
	; First, find running Welcome
	;
		segmov	es, cs, di
		mov	di, offset welcomePermName
		mov	ax, GEODE_NAME_SIZE
		mov	cx, mask GA_PROCESS
		clr	dx
		call	GeodeFind		; bx = geode
		jnc	exit
	;
	; Next, send a message to Welcome to switch fields
	;
		push	bx			; save geode handle
		clr	bx, si
		mov	ax, MSG_STARTUP_PROCESS_GOTO_FIELD
		mov	cx, SF_FIELD1		; CUI field
		mov	di, mask MF_RECORD
		call	ObjMessage		; message => DI
		mov	cx, di
		mov	ax, MSG_GEN_SEND_TO_PROCESS
		mov	di, mask MF_FIXUP_DS
		pop	bx			; bx = geode
		call	GeodeGetAppObject	; ^bx:si = app obj
		call	ObjMessage		; send to process via app obj
	;
	; OK, we're done.
	;
exit:
...
if GPC_PRESERVE_DIRECTORIES
	...
	;
	push	bx, si, cx, dx, bp
	mov	ax, MSG_GEN_SET_ENABLED	; enable as parent could have disabled
