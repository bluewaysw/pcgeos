...
SYNOPSIS:	return correct icon for this workspace object

		If XIP the icon will be copied to the stack.  The caller must
		reset the stack using SysRemoveFromStack

CALLED BY:	INTERNAL
			DrawFolderObjectIcon

PASS:		es:bp - pointer to workspace buffer entry for this object

RETURN:		ds:si - pointer to icon for this object
...
if GPC_TEXT_MODE_ICONS ne TRUE

GetFolderObjectIcon	proc	near
FXIP<	uses	cx							>
	.enter

	segmov	ds, cs				; all bitmaps are here
	mov	si, offset folderIconBitmap	; assume subdir object
FXIP<	mov	cx, offset folderIconBitmapEnd				>
FXIP<	sub	cx, offset folderIconBitmap				>
	test	es:[bp].FR_fileAttrs, mask FA_SUBDIR; check if a subdir
	jnz	done			; if so, done
	mov	si, offset fileIconBitmap	; else, file
FXIP<	mov	cx, offset fileIconBitmapEnd				>
FXIP<	sub	cx, offset fileIconBitmap				>
done:
FXIP<	call	SysCopyToStackDSSI					>

	.leave
	ret
GetFolderObjectIcon	endp

.assert (segment GetFolderObjectIcon) eq (segment fileIconBitmap)
.assert (segment GetFolderObjectIcon) eq (segment folderIconBitmap)
.assert (segment DrawFolderObjectIcon) eq (segment fileIconBitmap)
.assert (segment DrawFolderObjectIcon) eq (segment folderIconBitmap)

endif  ; GPC_TEXT_MODE_ICONS ne TRUE
