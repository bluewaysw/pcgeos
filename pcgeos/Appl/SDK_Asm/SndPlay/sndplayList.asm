...
RCS STAMP:
	...
	$Id: sndplayList.asm,v 1.1 97/04/04 16:32:43 newdeal Exp $
...
udata	segment
	...
	currentPosition		word	; the current position in the
					; sound buffer
...
;	Temporary code.  Delete when you start making changes.
		...
		push	ax, si, di, ds, es		; save registers
	...
	;
		...
		pop	ax, si, di, ds, es		; restore registers
...
SoundPlayGetNoteValue	proc	near
	...
	;
	;  Get a valid pointer into list for the note
	;  we wish to alter
	;
		call	SoundPlayFindNote	; si <- offset of note
...
SoundPlaySetNoteValue	proc	near
	...
	;
		...
		pop	cx, dx				; restore tone & durat.
		jc	done

		mov	es, ds:[noteListSegment]	; es:si <- note
		mov	es:[si].LN_tone, cx		; set values
...
SoundPlayInsertNote	proc	near
	...
	;
	;  Stick the note into the array at current position in list
	;
		push	cx, dx				; save tone & duration
		...
		mov	es, ds:[noteListSegment]	; es <- list segment
...
;	Temporary code.  Delete when you start making changes.
		...
		cmp	cx, ds:[currentNumOfNotes]	; did we go off the end?
		jae	error				; is carry clear?
	...
	;
		...
		stc					; everything went fine
error:
		cmc					; return set on error,
							; carry clear on ok
;#############################################################################
;
		.leave
		destroy		ax, cx, dx	; if not, adjust uses clause and
		ret				; routine header!
