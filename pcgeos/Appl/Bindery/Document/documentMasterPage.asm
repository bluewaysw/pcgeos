...
ROUTINES:
	Name			Description
	----			-----------
    INT DuplicateMasterPage	Duplicate a master page

    INT GetSectionToOperateOn	Determine which section to operate on

    INT DEMP_ObjMessageNoFlags	Determine which section to operate on

    INT DEMP_ObjMessageFixupDS	Determine which section to operate on

    INT FindMasterToOpenCallback 
				Callback routine to find a master page

    INT SetMPDisplayName	Set the name of a master page display

    INT DestroyUIForMasterPage	Delete the UI for a master page

    INT DetachUIForMasterPage	Detach the UI for a master page

    INT AttachUIForMasterPage	Attach the UI for a master page

    INT SendMPSizeToView	Send the master page size to the view

    INT DeleteMasterPage	Delete a master page block

    INT FindOpenMasterPage	Find an open master page (if it exists)

METHODS:
	Name			Description
	----			-----------
    StudioDocumentEditMasterPage Edit the master page for the document

				MSG_STUDIO_DOCUMENT_EDIT_MASTER_PAGE,
				MSG_STUDIO_DOCUMENT_EDIT_HEADER,
				MSG_STUDIO_DOCUMENT_EDIT_FOOTER
				StudioDocumentClass

    StudioDocumentOpenMasterPage Open a master page

				MSG_STUDIO_DOCUMENT_OPEN_MASTER_PAGE
				StudioDocumentClass

    StudioDocumentCloseMasterPage  
				Close a master page

				MSG_STUDIO_DOCUMENT_CLOSE_MASTER_PAGE
				StudioDocumentClass
...
DESCRIPTION:
	This file contains the master page related code for StudioDocumentClass

	$Id: documentMasterPage.asm,v 1.1 97/04/04 14:38:52 newdeal Exp $
...
EC <	call	AssertIsStudioDocument					>
	...
	pop	ds:[di].SMPGOBI_mainGrobjBody
	call	MemUnlock
	pop	ds
	.warn @private
...
MESSAGE:	StudioDocumentEditMasterPage --
		MSG_STUDIO_DOCUMENT_EDIT_MASTER_PAGE for StudioDocumentClass
...
------------------------------------------------------------------------------@
StudioDocumentEditMasterPage	method dynamic	StudioDocumentClass,
					MSG_STUDIO_DOCUMENT_EDIT_MASTER_PAGE,
					MSG_STUDIO_DOCUMENT_EDIT_HEADER,
					MSG_STUDIO_DOCUMENT_EDIT_FOOTER
...
fileHandle		local	hptr
	...
	cmp	ax, MSG_STUDIO_DOCUMENT_EDIT_HEADER
...
gotMasterPage:
	...
	cmp	msg, MSG_STUDIO_DOCUMENT_EDIT_HEADER
	jz	gotObjectToEdit
	mov	di, offset MPBH_footer
	cmp	msg, MSG_STUDIO_DOCUMENT_EDIT_FOOTER
	jnz	abort
gotObjectToEdit:
	mov	ax, bodyVMBlock
	mov	bx, fileHandle
...
StudioDocumentEditMasterPage	endm

COMMENT @----------------------------------------------------------------------

MESSAGE:	StudioDocumentOpenMasterPage --
		MSG_STUDIO_DOCUMENT_OPEN_MASTER_PAGE for StudioDocumentClass
...
------------------------------------------------------------------------------@
StudioDocumentOpenMasterPage	method dynamic	StudioDocumentClass,
					MSG_STUDIO_DOCUMENT_OPEN_MASTER_PAGE
...
EC <	ERROR_NC MASTER_PAGE_NOT_FOUND					>
	...
	mov	ax, MSG_STUDIO_MASTER_PAGE_CONTENT_SET_DOCUMENT_AND_MP
	...
	mov	displayBlock, bx
	mov	si, offset MasterPageDisplay
	mov	ax, MSG_STUDIO_MASTER_PAGE_DISPLAY_SET_DOCUMENT_AND_MP
	mov	cx, ds:[LMBH_handle]
	mov	dx, document			;cxdx = document
	push	bp
	mov	bp, bodyVMBlock
	...
	GetResourceHandleNS	StudioDisplayControl, bx
	mov	si, offset StudioDisplayControl
...
StudioDocumentOpenMasterPage	endm
...
MESSAGE:	StudioDocumentCloseMasterPage --
		MSG_STUDIO_DOCUMENT_CLOSE_MASTER_PAGE for StudioDocumentClass
...
------------------------------------------------------------------------------@
StudioDocumentCloseMasterPage	method dynamic	StudioDocumentClass,
					MSG_STUDIO_DOCUMENT_CLOSE_MASTER_PAGE
...
StudioDocumentCloseMasterPage	endm
...
EC <	call	AssertIsStudioDocument					>
	...
	call	StudioVMBlockToMemBlock			;ax = body block
