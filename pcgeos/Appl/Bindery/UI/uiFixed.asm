...
DESCRIPTION:
	...
	$Id: uiFixed.asm,v 1.1 97/04/04 14:39:55 newdeal Exp $
...
FixedFindMaxCharsLinePage	proc	near
	...
	; Registers ;
	;  bp = RegionParams ptr, ds:di = instance ptr ;
	; ----------------------------------------
	;
	...
	rndwwf	dxcx, bx		; bx = most chars per line!
...
afterCharsMax:
	...
	;
	mov	dx, bx			; max chars/line
	pop	cx			; max lines/page
	clc				; OK!

afterGettingGoodValues:
	pop	ax
	mov	sp, ax			; reset stack w/out affecting flags
...
FixedSetCharsLinePageMaxs	proc	near
	...
	push	cx			; save Lines/Page

	mov	si, offset CharsPerLineValue
	clr	cx			; dx.0 = maximum Chas/Line
	...
	pop	dx
	clr	cx			; dx.0 = maximum Lines/page
...
FixedComputeSpacings	proc	near
	...
	tst	dh
	jg	fcsCharOverflow  	; jump if wildly positive..
	ja	afterCharRounding_DL_CH  ; truncate negative values..

	rndwwbf	dxcx			; round to 8-bit fraction
afterCharRounding_DL_CH:
	mov	ah, dl			;  as a signed BBFixed value
	mov	al, ch
afterCharRounding_AX:
	...
	mov	ch, dl			; .. in cx
	tst	cx
	jz	fcsCharOverflow	; (actually underflow; same result)
...
;fcsDone:
	...
	;
fcsLineOverflow:
	mov	ax, MAX_TRACK_KERNING-1	; stash maximum values.
	jmp	afterCharRounding_AX
...
ComputeRegionParams	proc	near
	...
	tst	ds:[di].FixedAttrs.FCLPI_widthRegion.WSF_int
	jna	computeIndeterminate

	;	Load up the top/bottom, abort if size is 0.
	tst	ds:[di].FixedAttrs.FCLPI_heightRegion.WSF_int
	ja	beforeGstate
