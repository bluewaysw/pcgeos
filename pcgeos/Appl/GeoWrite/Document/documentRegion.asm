...
DESCRIPTION:
	This file contains the ... related code for WriteDocumentClass

	$Id: documentRegion.asm,v 1.1 97/04/04 15:56:27 newdeal Exp $
...
MESSAGE:	WriteDocumentRecalcLayout -- MSG_WRITE_DOCUMENT_RECALC_LAYOUT
...
------------------------------------------------------------------------------@
WriteDocumentRecalcLayout	method dynamic	WriteDocumentClass,
						MSG_WRITE_DOCUMENT_RECALC_LAYOUT
...
WriteDocumentRecalcLayout	endm

COMMENT @----------------------------------------------------------------------

MESSAGE:	WriteDocumentRecalcInval -- MSG_WRITE_DOCUMENT_RECALC_INVAL
							for WriteDocumentClass
...
------------------------------------------------------------------------------@
WriteDocumentRecalcInval	method dynamic	WriteDocumentClass,
						MSG_WRITE_DOCUMENT_RECALC_INVAL
	call	IgnoreUndoNoFlush
	call	LockMapBlockES
	call	SuspendDocument
	call	RecalculateInvalidArticleRegions
...
flowThroughExists:
	...
	mov	ax, flowThroughChunk		;optimize memory usage by
	call	LMemFree			;deleting unneeded stuff
	mov	ax, tempChunk
	clr	cx
	call	LMemReAlloc
...
gotResultRegion:
	...
	call	DBUnlock
	pop	ax
	call	LMemFree			;discard old result chunk
	mov	si, tempChunk
...
noRegion:
	...
	clr	di				;ds is freed block and/or
						;	no longer needed
	call	ObjMessage
	popf
