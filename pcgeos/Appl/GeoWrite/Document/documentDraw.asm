...
DESCRIPTION:
	...
	$Id: documentDraw.asm,v 1.1 97/04/04 15:56:17 newdeal Exp $
...
MESSAGE:	WriteDocumentDraw -- MSG_VIS_DRAW for WriteDocumentClass
...
------------------------------------------------------------------------------@
WriteDocumentDraw	method dynamic	WriteDocumentClass, MSG_VIS_DRAW
					uses cx, es
	mov	di, bp				;di = gstate
flags		local	word	\
		push	cx
...
afterTop:
	...
	push	ds:[bx].WDI_size.PD_x.low
	mov	ax, ds:[bx].WDI_size.PD_y.low
	mov	bx, ds:[bx].WDI_size.PD_y.high	;bxax = height
...
DrawMasterPages	proc	near	uses si, di, ds, es
	.enter inherit WriteDocumentDraw
...
drawLoop:
	...
	test	es:miscSettings, mask WMS_DISPLAY_SECTION_AND_PAGE
...
DBCS <	shr	cx, 1					;# bytes -> # chars>
	...
	segmov	ds, cs
	mov	si, offset sepString		; ds:si = separator string
	clr	cx
