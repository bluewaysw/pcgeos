...
DESCRIPTION:
	...
	$Id: documentSection.asm,v 1.1 97/04/04 15:56:48 newdeal Exp $
...
MESSAGE:	WriteDocumentInsertPage -- MSG_WRITE_DOCUMENT_INSERT_PAGE
...
------------------------------------------------------------------------------@
WriteDocumentInsertPage	method dynamic	WriteDocumentClass,
					MSG_WRITE_DOCUMENT_INSERT_PAGE,
					MSG_WRITE_DOCUMENT_APPEND_PAGE

	call	IgnoreUndoAndFlush

	mov_tr	dx, ax

	mov	ax, offset InsertPageString
	cmp	dx, MSG_WRITE_DOCUMENT_INSERT_PAGE
	jz	gotString
	mov	ax, offset AppendPageString
gotString:
	call	ConfirmIfNeeded			;ax = InteractionCommand
	cmp	ax, IC_YES
	jnz	done

	call	LockMapBlockES
	mov	ax, ds:[di].WDI_currentPage	;reference page
...
WriteDocumentInsertPage	endm

COMMENT @----------------------------------------------------------------------

MESSAGE:	WriteDocumentDeletePage -- MSG_WRITE_DOCUMENT_DELETE_PAGE
						for WriteDocumentClass
...
------------------------------------------------------------------------------@
WriteDocumentDeletePage	method dynamic	WriteDocumentClass,
					MSG_WRITE_DOCUMENT_DELETE_PAGE

	call	IgnoreUndoAndFlush

	mov	ax, offset DeletePageString
	call	ConfirmIfNeeded			;ax = InteractionCommand
	cmp	ax, IC_YES
	jnz	done
...
appendFlag	local	word	\
		push	di
SBCS <sectionName	local	NAME_ARRAY_MAX_NAME_SIZE dup (char)	>
DBCS <sectionName	local	NAME_ARRAY_MAX_NAME_SIZE dup (wchar)	>
nameSize	local	word
	.enter
...
DBCS <	shr	cx, 1				;pass name length	>
	call	NameArrayFind
...
if _SIMPLIFIED_PAGE_NUMBERING or _ALLOW_STARTING_PAGE
	; We must have added the title page, set to start at page 0
	mov	ds:[di].SAE_startingPageNum, 0
	andnf	ds:[di].SAE_flags, not mask SF_PAGE_NUMBER_FOLLOWS_LAST_SECTION
endif
...
if _SIMPLIFIED_PAGE_NUMBERING
	...
	inc	ax
	mov	si, offset SectionArray
	call	ChunkArrayElementToPtr		;point at first section
	ornf	ds:[di].SAE_flags, mask SF_PAGE_NUMBER_FOLLOWS_LAST_SECTION
	pop	ax
endif
...
doneUnlock:
	pushf
	call	VMDirtyES
	call	VMUnlockES			; ec-segment destroys es
	call	loadDocumentObj
	popf
if _REGION_LIMIT
	jc	abort
...
doneNoUnlock:
	...
	jc	exit
	mov	ax, mask NF_SECTION or mask NF_PAGE
	call	LockMapBlockES			; so get the map block again
...
if _REGION_LIMIT
	;
	; Couldn't add new section because of region limit.
	;
abort:
	mov	ax, MSG_GEN_DOCUMENT_REVERT_TO_AUTO_SAVE
	call	ObjCallInstanceNoLock
	clc					;clear carry so caller doesn't
						; display a warning
	jmp	exit
endif
...
FindPageAndSection	proc	far	uses bp
	...
	push	cx			; (pushed separately so can be
					;  popped separately w/o loss of
					;  code clarity or assuming order
					;  of multi-reg push...)
	...
	jz	coordConvertDone	; => no region; skip getting region
					;  position and (eventually) bail with
					;  carry set

	; get the real position of the region

	mov	si, offset ArticleRegionArray
	mov	ax, cx			; (need CX for check @ coordConvertDone)
...
coordConvertDone:
	...
	add	sp, size PointDWFixed	; clear off point
	pop	ax, bx, si, di, ds
	
	cmp	cx, CA_NULL_ELEMENT	; did we find a region?
	pop	cx
	stc				; assume not
	je	done			; we didn't. sigh.
