...
DC_ESCRIPTION:
	This file contains all the Printer Control Codes for the eplx 9-pin
	driver.
		
	$Id: eplx9ControlCodes.asm,v 1.1 97/04/18 11:54:38 newdeal Exp $

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

;*****************************************************************************
;
;CONTROL CODES FOR THE EPSON LX 9-PIN PRINTERS.....
;
...
pr_codes_InitPrinter	label	byte
	byte	14,0		;send a null to wait till after any reset garb
	byte	C_ESC,"O"	;cancel any perforation skipping.
	byte	C_ESC,"t",1	;set the foreign charracter table.
	byte	C_ESC,"6"	;print in 80h to 9fh range.
	byte	C_ESC,"U",1	;set uni-directional
	byte	C_ESC,"r",0	;set black color.
