...
FILE:		textPrintStyleRunAddX.asm
...
DESCRIPTION:
	This file is the same as the PrintStyleRun routine EXCEPT it adds to
	the cursor x position for every run sent.

	$Id: textPrintStyleRunAddX.asm,v 1.1 97/04/18 11:50:04 newdeal Exp $
...
tiled:
	mov	cx,ax			;get the X pos from ax
	clr	ax
	push	si
...
getStringMetrics:
	...
	add	es:[PS_cursorPos].P_x,dx	;add the width for next time.
