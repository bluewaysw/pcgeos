...
FILE:		graphicsPrintSwathPCLCMY.asm
...
DESCRIPTION:
	...
	$Id: graphicsPrintSwathPCLCMY.asm,v 1.1 97/04/18 11:51:23 newdeal Exp $
...
scanlineLoop:
	...
	mov	bandStart,si
		...
		;we need to send 3 scanlines for every CMY composite scanline,
		;so go through here 3 times. We are guaranteed to have all 3 
		;planes dereferenced so all we need to do is adjust offsets to
		;get to all the color planes. We need to re-order from YCM to
		;CMY.
colorLoop:
