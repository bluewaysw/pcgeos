...
DESCRIPTION:
	...
	$Id: cursorPrFormFeed60.asm,v 1.1 97/04/18 11:49:47 newdeal Exp $
...
PrFormFeed	proc	near
	...
	mov	dx,es:[PS_currentMargins].[PM_bottom] ;get the bottom margin
				;value in points, now convert it to 1/60".
	clr	cx		;get initial values to mult.
	mov	bx,cx
	mov	ax,54613	;fraction 60/72
	call	GrMulWWFixed	;get page length in 1/60"
	rcl	cx		;get carry loaded for round.
	adc	dx,0		;round.
	mov	cx,dx		;get into right reg for write to stream....
EC <	test	cx,0ff80h	;see if we are too big for a byte value >
