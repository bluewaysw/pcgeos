...
DESCRIPTION:
	...
	$Id: dos7InitExit.asm,v 1.1 97/04/10 11:55:36 newdeal Exp $
...
MS7<		call	MSInitOpenFilePathCache				>
MS7<		call	MSInitRenameBoolean				>
...
SYNOPSIS:	Initializes the dos7RenameWhileOpen boolean variable in
		dgroup.
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	jimw	3/ 5/97    	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
renameStr	char	'MS7 rename while open', 0
MSInitRenameBoolean	proc	near
		uses	ax,cx,dx,si
		.enter
	;
	; Set up and read the thing.
	;
		push	ds			; save dgroup
		segmov	ds, cs
		mov	si, offset systemCatStr	; ds:si <- category
		mov	cx, ds
		mov	dx, offset renameStr
		mov	ax, FALSE
		call	InitFileReadBoolean
		pop	ds
		mov	ds:[dos7RenameWhileOpen], ax

		
		.leave
		ret
MSInitRenameBoolean	endp




COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		MSInitOpenFilePathCache
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Allocate an lmem heap to keep track of the paths of open
		files.  There will be an entry for SFN entry here.

CALLED BY:	MSInit
PASS:		ds	= dgroup
RETURN:		dos7PathCache* contains chunk array info for the path cache.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
MSInitOpenFilePathCache	proc	near
		.enter
	;
	; Allocate the heap 
	;
		mov	ax, LMEM_TYPE_GENERAL
		mov	cx, size DOS7PathCacheHeader
		call	MemAllocLMem
		mov	ds:[dos7PathCache], bx
	;
	; Make the thing sharable.
	;
		clr	ah			; clear no flags
		mov	al, mask HF_SHARABLE	; set this one
		call	MemModifyFlags
	;
	; Zero the header out.
	;
		call	MemLock
		mov	es, ax
		mov	di, offset D7PCH_table
		mov	cx, (size DOS7PathCacheHeader) \
					-  (size LMemBlockHeader)
		clr	ax
		rep	stosb
