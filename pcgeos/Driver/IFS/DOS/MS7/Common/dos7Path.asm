...
DESCRIPTION:
	...
	$Id: dos7Path.asm,v 1.1 97/04/10 11:55:31 newdeal Exp $
...
DOSAllocOpFinishOpen proc near
	...
	; 
		...
		je	oldGeos
if _MS7
		test	ds:[dos7FindData].W32FD_fileAttrs.low.low, FA_GEOS_FILE
else
		test	ds:[dosNativeFFD].FFD_attributes, FA_GEOS_FILE
endif
		jnz	isGeos
...
if DBCS_PCGEOS
	...
	;
if _MS7
	; final component already mapped for MS7.
		ornf	ds:[dos7FindData].W32FD_fileAttrs.low.low, FA_GEOS_FILE
else
		ornf	ds:[dosNativeFFD].FFD_attributes, FA_GEOS_FILE
		mov	ds:[dosFinalComponent].segment, ds
		mov	ds:[dosFinalComponent].offset,
...
if _MS7
		andnf	ds:[dos7FindData].W32FD_fileAttrs.low.low, \
							not FA_GEOS_FILE
else
		andnf	ds:[dosNativeFFD].FFD_attributes, not FA_GEOS_FILE
endif
...
if _MS7
		mov	dl, ds:[dos7FindData].W32FD_fileAttrs.low.low
else
...
cachePath:
	;
	; We cache paths to data and vm files because they can have their
	; ext attrs changed.  Other types cannot.  You don't want to know.
	;
		pop	bx		; sfn
if _SFN_CACHE
		pushf
		cmp	ds:[dosFileType], GFT_DATA
		je	cacheIt
		cmp	ds:[dosFileType], GFT_VM
		jne	afterCache
cacheIt:
...
RETURN:		carry clear if OK:
		...
		if MS7, then ds:dx = dos7FindData
...
PSEUDO CODE/STRATEGY:
	...
	In ms7 we don't worry about creating the at-dirname files at all, as
	we can handle long directory names.
...
if not _MS7
	;
	; The MS7 driver supports GeosLongname length directory names, so
	; we don't need to do this part.
	;
		call	PathOps_LoadVarSegDS
...
PASS:		DOS current dir set to directory in which dir will be
		...
		MS7 :
		ds:dx	= name of the directory to be created
...
REVISION HISTORY:
	...
	----	----		-----------
	ardeb	2/14/92		Initial version
	jim	2/14/97		MS7 additions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
DOSPathOpDeleteDir proc	far
disk		local	word		push si
if _MS7
fd		local 	Win32FindData
searchHandle	local	word
else
...
MS7<		clr	ss:[searchHandle]			>
	...
	; 
if not _MS7
	; MS7 can handle the long name itself which is in ds:dx already.
		mov	dx, offset dosNativeFFD.FFD_name
else
		segmov	es, ds
		mov	di, dx
...
afterInt21:		
	...
	; Check for . and ..
	;
		cmp	ss:[fd].W32FD_fileName.MSD7GN_longName, '.'
		je	scanLoop

		tst	ss:[fd].W32FD_fileName.MSD7GN_longName
		jz	scanLoop
	;
	; Check for @ too; we may have an @dirname here.
	;
		cmp	ss:[fd].W32FD_fileName.MSD7GN_longName, '@'
		jne	scanLoop
...
if _MS7
	;
	; Just use the dosFinalComponent in the longname shme.
	;
		push	ds
		lds	dx, ds:[dosFinalComponent]
		mov	ax, MSDOS7F_DELETE_DIR
else
		mov	dx, offset dosNativeFFD.FFD_name
		mov	ah, MSDOS_DELETE_DIR
endif
...
undo:
	...
	; 
		push	ax
		mov	dx, offset dosNativeFFD.FFD_name
		segmov	es, ss		lea	di, ss:[srcName]
		mov	ah, MSDOS_RENAME_FILE
		call	DOSUtilInt21
		pop	ax
		stc
...
if _MS7
	;
	; Handle the deletion right here for MS7.
	;
		push	si, cx
		clr	si, cx
		call	PathOps_LoadVarSegDS
...
if _MS7
setUpForNotify:
		cmp	dx, MSDOS7F_GET_SET_ATTRIBUTES
else
...
createLink:
if _MS7
		ERROR	MSDOS7_ERROR_NO_LINK_SUPPORT
else		
...
if _MS7
		...
		clr	bl			; Zero to get
		mov	dx, offset dos7FindData.W32FD_fileName.MSD7GN_longName
		cmp	ds:[dos7FindData].W32FD_fileName.MSD7GN_longName, '.'
else
...
hack:
		clr	ch
if _MS7
		mov	cl, ds:[dos7FindData].W32FD_fileAttrs.low.low
else
