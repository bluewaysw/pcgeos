...
idata           segment
	...
	writeDataPort           word    dspDataWrite    ; + basePortAddress
        ...
        isrunning		byte	00h	; 1 = Ausgabe aktiv
        					; (Dient zur Vermeidung
						;  unn”tiger Initialisierung
						;  da SBDDACSetSample durch
						;  Soundlibrary immer wieder
						;  aufgerufen wird !)
                                                ; . . . . . . . .
                                                ;
                                                ; 1 = Output active
...
;---------------------------------
	...
	mov	ds:[sampleRate],dx	; store Samplerate
	mov	ds:[bsSampleRate],dx
...
;	runningflag setzen
;---------------------------------
        mov	ds:[isrunning],1	; set running flag
...
;	streamState
;-----------------------
	tst	ds:[streamState]	; Transfer already started ?
	jne     done			; yes, go to end

;-----------------------
; Wait for end of old output
;-----------------------
        mov	cx,10			; 10 loops
...
waitfordac:
	tst	ds:[dacStatus]		; dacStatus = 0 ?
        jz	streamset		; yes
        mov	ax,6			; wait 1/10 sec
        call	TimerSleep
        dec     cx			; Waitcounter at End ?
        jnz	waitfordac		; no, continue

        jmp	error			; Timeout = ABORT
...
streamset:
	...
	mov	ds:[streamSize],dx	; Streamlen
	clr	ds:[endeFlag]		; Endflag reset
...
;----------------------------------
	...
	call    StreamStrategy		; set Notificationroutine
...
;	prepare output
;---------------------------------
        mov	ds:[streamState],1	; set Streamstatus
...
alloc:
	 ...
	 shr	cx			; Streamlen / 2
        mov	cx,4000h
	call	BSSecondAlloc		; Allocate Secondarybuffer
	jc	error
...
restwav:
        ...
        jz	unlock			; no
        call	BSFillRest		; erase Rest of data
        call	BSSingletransfer	; Transfer start
	mov	ds:[bufferFree],dx	; mark End
	jmp	waitdsp

;------------------------
; Release Secondbuffer
;------------------------
unlock:
        call	BSSecondUnlock
...
;--------------------------
	mov	al,1
	tst	ds:[dacStatus]		; DAC in use ?
        jnz	setflag			; yes

        clr	al			; clear endFlag
setflag:
	mov	ds:[endeFlag],al
...
slowFormatList  DACFormatRange  \
	...
	<MANUFACTURER_ID_BSW,		DACSF_8_BIT_MONO    , 4000, 23000>,
...
SBDDACGetFormatSetting  endp
	...
	;
	;  Tabelle mit DSP-Kommandos fr das jeweilige
        ;  Sampleformat. Fr ManufacturerID = BSW wird
        ;  nur ein Pseudocode zurckgeliefert der von
        ;  SBDDACDMANextBlock interpretiert wird.
        ; . . . . . . . . . . . . . . . . . . . . . . .
        ;
        ;  Table with DSP-commands
        ;
        ;  Code 80..FF is not a real DSP-command
        ;  This values will be analysed by SBDDACDMANextBlock
        ;
