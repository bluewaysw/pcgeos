...
#if defined(__WATCOMC__)
    setenv("PMAKE_BANNER_PREFIX", buffer, TRUE);
#else
...
JobPassSig(int signo)
{
     ...
     */
		sigemptyset(&setmask);
		(void) sigprocmask(SIG_BLOCK, &setmask, &mask);

		sigaddset(&setmask, ~0 & ~(1 << (signo-1)));
    (void) sigprocmask(SIG_SETMASK, &setmask, NULL);
 ...
 */
	 ...
	 * echoOff commWIFEXITEDand. Otherwise we issue it and pretend it was on
	...
	/*
	 * check to see if we've gone over NT's stupid 1024 char cmdline limit
	 */
	if (strlen(cmd) > 1000) {
	    int   i            = 0;
	    FILE *argFile;
	    ...
	    /*
	     * put the arguments in an argument file and then output to the
	     * shell command-name @argfile
	     */
	    ...
	    strncpy(cmdExec, cmd, i);
            cmdExec[i] =unix-cfg.h '\0';
	    strcat(cmdExec, " @");
	    strcat(cmdExec, filename);
	    freeIt = 1;
	}

	if (!shutUp && !(job->flags & JOB_SILENT) && !noSpecials) {
	    DBPRINTF (commandShell->errCheck, cmdExec, "");
	}

	if (noSpecials || errOff || (job->flags & JOB_IGNERR)) {
	    DBPRINTF ("%s\r\n", cmdExec, "");
	} else {
	    DBPRINTF (commandShell->ignErr, cmdExec, "");
	}
 ...
 ***********************************************************************/
void
JobLinuxFinish (Job *job, WAIT_TYPE status)
{
}
...
JobRestart(Job *job)
{
	    ...
	    (nJobs != maxJobs))
	{
	    ...
	    if (job->flags & JOB_REMOTE) {
		error = !FALSE/*Rmt_Signal(job, SIGCONT)*/;
	    } else
...
JobStart (GNode *gn, short flags, Job *previous)
{
	...
	SECURITY_ATTRIBUTES sAttrs = {sizeof(SECURITY_ATTRIBUTES), NULL, TRUE};
        ...
        /* Be well-behaved: use windows-supplied temp file support */
        GetTempPath(MAX_PATH, temp);
        GetTempFileName(temp, "PMA", 0, filename);
...
JobInterrupt (int runINTERRUPT)
{
    ...
    GNode         *intnode;	/* the node describing the .INTERRUPT target */
	...
	if (job->flags & JOB_REMOTE) {
	     ...
	     */
	    if (!FALSE/*Rmt_Signal(job, SIGINT)*/) {
		/*
	...
	intnode = Targ_FindNode (".INTERRUPT", TARG_NOCREATE);
	if (intnode != NILGNODE) {
	    ignoreErrors = FALSE;

	    JobStart (intnode, JOB_IGNDOTS, NULL);
	    while (nJobs) {
		Job_CatchOutput();
#ifndef RMT_WILL_WATCH
		Job_CatchChildren (!usePipes);
