 ...
 *	    	  	    third argument is non-zero, Pars_Error is
...
#ifndef lint
static char     *rcsid = "$Id: var.c,v 1.7 96/06/24 15:07:50 tbradley Exp $ SPRITE (Berkeley)";
#endif lint
...
/* this is the callback type for VarModify*/
typedef Boolean VarModifyCallback(char *, Boolean, Buffer, ClientData);
...
/***********************protoypes for static routines**********************/
static Var	*VarAlloc      (char *name);
static int	 VarCmp        (Var *v, char *name);
static Var	*VarFind       (char *name, GNode *ctxt, int flags);
static void	 VarAdd        (char *name, char *val, GNode *ctxt);
static Boolean	 VarHead       (char *myword, Boolean addSpace, Buffer buf,
				ClientData unUsed);
static Boolean   VarTail       (char *myword, Boolean addSpace, Buffer buf,
				ClientData unUsed);
static Boolean	 VarSuffix     (char *myword, Boolean addSpace, Buffer buf,
				ClientData unUsed);
static Boolean	 VarRoot       (char *myword, Boolean addSpace, Buffer buf,
				ClientData unUsed);
static Boolean	 VarMatch      (char *myword, Boolean addSpace, Buffer buf,
				ClientData pattern);
static Boolean	 VarNoMatch    (char *myword, Boolean addSpace, Buffer buf,
				ClientData pattern);
static Boolean	 VarSubstitute (char *myword, Boolean addSpace, Buffer buf,
				register ClientData pattern);
static char	*VarModify     (char *str, VarModifyCallback noProc,
				ClientData datum);
static int	 VarPrintVar   (Var *v);
 ...
 *-----------------------------------------------------------------------
 * VarAlloc --
 *	Allocate a Var record given its name.
 *
 * Results:
 *	An initialized Var record with the val field set to 0
 *
 * Side Effects:
 *	None
 *
...
VarAlloc(char *name)
{
    ...
    MallocCheck(v, sizeof(Var) + strlen(name) + 1);
 ...
 *      int    flags : FIND_GLOBAL set means to look in the VAR_GLOBAL
 *                     context as well.  FIND_CMD set means to look in the
 *                     VAR_CMD context also.  FIND_ENV set means to look in
 *                     the environment.
 *
 * Side Effects:
 *	None
...
VarFind (char *name, GNode *ctxt, int flags)
{
	...
	char *env;
	     ...
	     * its value (since eVarVal was allocated on the stack). The name
 	     * wouldn't need duplication since it's always in the environment,
 	     * if we had the pointer into the environment rather than
 	     * just some random buffer.  Instead, we have to allocate
 	     * space for a copy of it.
  	     */
	    int	  	len;
 ...
 *			Var_SetFromFile
 *********************************************************************
 * SYNOPSIS: same as Var_Set but gets the info from a file rather than
 *	     a string
 * CALLED BY:	DOS stuff
 * RETURN:
 * SIDE EFFECTS:
 * STRATEGY:	get info from a file, put it in a string and call Var_Set
 * REVISION HISTORY:
 *	Name	Date		Description			     
 *	----	----		-----------			     
 *	jimmy	8/13/92		Initial version			     
 * 
 *********************************************************************/
void
Var_SetFromFile(char	*name,	/* name of variable to set */
		char	*filename, /* file to get data from */
		GNode	*ctxt) /* context in which to set */
{
    FILE    *fp;
    char    *buf,*tbuf;
    long    fsize;
    ...
    MallocCheck(buf, fsize);
    tbuf = buf;

    /*	
     *	read file into buffer ignoring carriage returns and
     *	converting newlines into spaces
     */

    while((c = (char) getc(fp)) != (char)EOF)
    {
	if (c != '\r')
	{
	    if (c == '\n')
    ...
    /*
     * space have been put in in place of all the newlines, now we must
     * backup and write a null over the last space
     */
    --buf;
    *buf = '\0';
    Var_Set(name, tbuf, ctxt);
}
#endif
/*-
 *-----------------------------------------------------------------------
...
Var_Set (char *name, char *val, GNode *ctxt)
{
    ...
    if (ctxt == VAR_CMD) {
	/*
	 * Make sure the shell won't choke
	 */
	if (isalpha(name[0])) {
	    char	*cp;
	    ...
	    if (*cp == '\0') {
		/*
		 * Characters are all alpha-numeric and the name begins with
		 * an alphabetic character, so the shell should be happy with
		 * this name.
		 */
#if defined(__WATCOMC__)
 ...
 ***********************************************************************/
char *
Var_LastPathSep (char *path)
{
#ifdef unix
    return strrchr(path, PATHNAME_SLASH);
#else
    char *lastSlash;
    char *lastBackSlash;
 ...
 ***********************************************************************/
char *
Var_FirstPathSep (char *path)
{
#ifdef unix
    return strchr(path, PATHNAME_SLASH);
#else
    char *firstSlash;
    char *firstBackSlash;
 ...
 *
 *Arguments:
 *      char    *myword   : Word to trim
 *      Boolean  addspace : TRUE if need to add a space to the buffer before
 ...
 */
static Boolean
VarHead (char *myword, Boolean addSpace, Buffer buf, ClientData unUsed)
{
    register char *slash;

    slash = Var_LastPathSep(myword);

    if (slash != NULL) {
	/*
	 * Silly under Unix, but under Win32, avoids changing
	 * /'s to \'s.
	 */
	char originalSlash = *slash;

	if (addSpace) {
	    Buf_AddByte (buf, (Byte)' ');
	}
	*slash = '\0';
	Buf_AddBytes (buf, strlen (myword), (Byte *)myword);
	*slash = originalSlash;
	return (TRUE);
    } else {
	/*
 ...
 * Arguments:
 *      char    *myword   : Word to trim
 *      Boolean  addSpace : TRUE if need to stick a space in the buffer
 ...
 */
static Boolean
VarTail (char *myword, Boolean addSpace, Buffer buf, ClientData unUsed)
{
    register char *slash;
    ...
    if (slash != (char *)NULL) {
	/*
	 * Silly under Unix, but under Win32, avoids changing /'s
	 * to \'s.
	 */
	char originalSlash = *slash;
 ...
 * Arguments:
 *      char    *myword   : Word to trim
 *      Boolean  addSpace : TRUE if need to add a space before placing the
 ...
 */
static Boolean
VarSuffix (char *myword, Boolean addSpace, Buffer buf, ClientData unUsed)
{
    register char *dot;
 ...
 */
static Boolean
VarRoot (char *myword, Boolean addSpace, Buffer buf, ClientData unUsed)
{
    register char *dot;
 ...
 *			VarExtract
 *********************************************************************
 * SYNOPSIS: like VarMatch but only returns the piece inside of /( /)
 * CALLED BY:	Callback function for VarModify used for the :X modifier
 * RETURN:  
 * SIDE EFFECTS:
 *
 * Arguments:
 *      char    *myword     : Word to examine
 *      Boolean  addSpace   : TRUE if need to add a space to the buffer before
 ...
 *********************************************************************/
static Boolean
VarExtract (char *myword, Boolean addSpace, Buffer buf, ClientData pat)
{
    char    *index1, *index2;
    char    *pattern = (char *) pat;

    if (Str_Match(myword, pattern, &index1, &index2)) {
	if ((unsigned long)index1 > (unsigned long)index2) {
	    return FALSE;
	}
	...
	addSpace = TRUE;
	Buf_AddBytes(buf, index2 - index1, (Byte *)(index1));
    }
    return(addSpace);
}
 ...
 */
static Boolean
VarMatch (char *myword, Boolean addSpace, Buffer buf, ClientData pat)
{
    char	*index1, *index2;
    char        *pattern = (char *) pat;
 ...
 */
static Boolean
VarNoMatch (char *myword, Boolean addSpace, Buffer buf, ClientData pat)
{
    char    *index1, *index2;
    char    *pattern = (char *) pat;
 ...
 *
 * Arguments:
 *      char                *myword   : Word to modify
 *      Boolean              addSpace : TRUE if space should be added before
 *                                      other characters
 *      Buffer               buf      : Buffer for results
 ...
 */
static Boolean
VarSubstitute (char *myword, Boolean addSpace, Buffer buf,
	       register ClientData pat)
{
    register int  	wordLen;    /* Length of word */
    register char 	*cp;	    /* General pointer */
    register VarPattern *pattern = (VarPattern *) pat;
	...
	if ((pattern->flags & VAR_MATCH_START) &&
	    (strnicmp(myword, pattern->lhs, pattern->leftLen) == 0)) {
		/*
	     ...
	     * to use strncmp.
	     */
	    cp = myword + (wordLen - pattern->leftLen);
	    if ((cp >= myword) &&
		(strnicmp(cp, pattern->lhs, pattern->leftLen) == 0)) {
		/*
		 ...
		 */
		if (((cp - myword) + pattern->rightLen) != 0) {
		    if (addSpace) {
			Buf_AddByte(buf, (Byte)' ');
		    }
		    addSpace = TRUE;
		}
		Buf_AddBytes(buf, cp - myword, (Byte *)myword);
		Buf_AddBytes(buf, pattern->rightLen, (Byte *)pattern->rhs);
	    } else {
		/*
	    ...
	    while (!done) {
#if defined(_WIN32)
		cp = Str_FindSubstringI(myword, pattern->lhs);
#else /* unix or _MSDOS */
		cp = Str_FindSubstring(myword, pattern->lhs);
#endif /* defined(_WIN32) */
		if (cp != (char *)NULL) {
		    if (addSpace && (((cp - myword) + pattern->rightLen) != 0)){
			Buf_AddByte(buf, (Byte)' ');
			addSpace = FALSE;
		    }
		    Buf_AddBytes(buf, cp-myword, (Byte *)myword);
		    Buf_AddBytes(buf, pattern->rightLen, (Byte *)pattern->rhs);
		    wordLen -= (cp - myword) + pattern->leftLen;
		    myword = cp + pattern->leftLen;
		    if (wordLen == 0) {
			done = TRUE;
		    }
		...
		Buf_AddBytes(buf, wordLen, (Byte *)myword);
	    }
	    /*
 ...
 *      Boolean    (*modProc)(void) : Function to use to modify them
 ...
 */
static char *
VarModify (char *str, VarModifyCallback modProc, ClientData datum)
{
 ...
 *			VarParsePattern
 *********************************************************************
 * SYNOPSIS: expand variables in a pattern string, and unescape escaped things
 * CALLED BY:	VarParse
 * RETURN:  cp pointer's new position
 * SIDE EFFECTS: buf has its variables expanded
 * STRATEGY: look for '\' and '$' and deal with them
 * REVISION HISTORY:
 *	Name	Date		Description			     
 *	----	----		-----------			     
 *	jimmy	8/14/92		Initial version			     
 * 
 *********************************************************************/
static char *
VarParsePattern(char 	    *tstr,  /* string to expand */ 
		char 	    *buf,   /* buffer to put expanded string into */
		char	    *ctxt,  /* context of parse */
		char 	    delim,  /* delimiter character */
		VarPattern  *pattern,/* pattern flag */
		Boolean	    err,    /* error flag */
    	    	Boolean	    stopOnColon) /* should we stop on colon */
{
    char    *cp;

    /*
     * Pass through the lhs looking for 1) escaped delimiters,
     * '$'s and backslashes (and maybe colons) (place the escaped character in
     * uninterpreted) and 2) unescaped $'s that aren't before
     * the delimiter (expand the variable substitution).
     * The result is left in the Buffer buf.
     */
    for (cp = tstr; 
	 *cp != '\0' && *cp != delim && !(*cp == ':' && (stopOnColon==TRUE)); 
	 cp++) 
    {
	if ((*cp == '\\') &&
	    ((cp[1] == delim) ||
	...
	else if (*cp == '$') 
	{
		...
		cp2 = Var_Parse(cp, (GNode *) ctxt, err, &len, &freeIt);
 ...
 *      Boolean  err       : TRUE if undefined variable are an error
	   ...
	   Boolean *freePtr)
{
    ...
    *freePtr = FALSE;
	...
	if (*tstr == ':') {
	     ...
	     */
	    Error("Unclosed variable specification %.*s",
		  (unsigned long)str, (unsigned long)(tstr - str), 0);
	    *lengthPtr = tstr - str;
	    return (var_Error);
	}
	*tstr = '\0';
	...
	Fatal("Variable %s is recursive.", (unsigned long)v->name, 0);
	/*NOTREACHED*/
    } else {
	v->flags |= VAR_IN_USE;
    }
	...
	*tstr++ = ':';
	while (*tstr != '\0')
	{
		...
		{
		    char    *pattern;
#if defined(OLD_PARSE)
		    Boolean copy;
#endif /* defined(OLD_PARSE) */
		    Buffer  buf;
		    VarPattern	vp;
		    ...
		    vp.flags = 0;
		    buf = Buf_Init(0);
		    cp = VarParsePattern(tstr, buf, (char *)ctxt, 
					 endc, &vp, err,TRUE);
		    tstr = (char*) Buf_GetAll(buf, NULL);
		    Buf_Destroy(buf, FALSE);
#if defined(OLD_PARSE)
		    copy = FALSE;
#endif /* defined(OLD_PARSE) */
			...
			*cp2 = '\0';
			...
			case 'X':
			case 'x':
			    newStr = VarModify(str, VarExtract,
						    (ClientData)pattern);
			    break;

			case 'M':
			case 'm':
			    newStr = VarModify(str, VarMatch,
						    (ClientData)pattern);
			    break;
			case 'N':
			case 'n':
		...
		case 'T':
		    /*
		     * We process the modifier even if the variable is not
		     * properly terminated (tstr[1] == '\0'), to avoid
		     * misleading 'Unknown modifier' warnings on the next
		     * iteration.
		     */
		    if (tstr[1] == '\0' || tstr[1] == endc || tstr[1] == ':') {
		...
		default: {
			...
			*cp = '\0';
			...
			/*
			 * Looks like random gibberish.  Note that this
			 * includes the case of an empty modifier $(VAR:).
			 */
			for (cp = tstr;
			     *cp != ':' && *cp != endc && *cp != '\0';
			     cp++) {
			...
			if (termc != '\0') {
			    /*
			     * Will be flagged as unclosed variable --
			     * avoid double error messages.  
			     */
			    *cp = '\0';
			    Error ("Unknown modifier '%s'", 
				   (unsigned long)tstr, 0, 0);
			}
			newStr = var_Error;
		    }
		}
		...
		*cp++ = termc;
		...
		/*
		 * This is where we end up when a modifier was implicitly
		 * terminated, such as 'S'.  Be forgiving and continue.
		 * XXX: Is this the right thing to do?
		 */
		if ((termc != endc) && !noWarnings) {
		    Error("warning: Missing ':' after modifier for %s",
			  (unsigned long)v->name, 0, 0);
		}
		*cp = termc;
	    }
	    tstr = cp;
	    /*
	     * Check if we're done with modifiers.  We don't quit yet when
	     * an empty modifier follows since that should be left to the
	     * 'Unknown modifier' error handling above.
	     */
	    if ((termc != ':') && (*tstr == endc)) {
		break;
	    }
 ...
 */
char *
Var_Subst (register char *str, GNode *ctxt, Boolean undefErr)
{
	...
	if ((*str == '$') && (str[1] == '$')) {
		...
		if (oldVars) {
			...
			Parse_Error (PARSE_WARNING,
				     "Undefined variable \"%.*s\"",length,(unsigned long)str);
		    }
#endif
		    str += length;
		} else if (undefErr) {
