...
#ifndef lint
static char rcsid[] = "$Id: hash.c,v 1.2 96/06/24 15:03:51 tbradley Exp $ SPRITE (Berkeley)";
#endif  not lint
...
static int		Hash(Hash_Table *tablePtr, char *key);
...
Hash_CreateEntry(register Hash_Table *tablePtr,	/* Hash table to search. */
		 Address key,			/* A hash key. */
		 Boolean *newPtr)      		/* Filled in with TRUE if new
						 * entry created, FALSE
						 * otherwise. */
{
    register Hash_Entry *hashEntryPtr;
		...
		int clientData)
{
    ...
    (*proc)( (ClientData) clientData, msg);
    for (i = 0;  i < 10; i++) {
     ...
     register char *key)
{
    ...
    return((int)(((i*1103515245 + 12345) >> tablePtr->downShift) & tablePtr->mask));
}

/*
 *---------------------------------------------------------
	    ...
	    register List_Links *hashList)
{
	...
	switch (numKeys) {
		...
		break;
	    default: 
		if (strncmp((Address) hashEntryPtr->key.words,
