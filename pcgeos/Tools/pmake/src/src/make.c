...
#if !defined(lint)
static char     *rcsid = "$Id: make.c,v 1.7 96/06/24 15:06:06 tbradley Exp $ SPRITE (Berkeley)";
#endif lint
...
static int	MakeAddChild(GNode *gn, Lst l);
static int	MakeAddAllSrc(GNode *cgn, GNode *pgn);
...
#if defined(_MSDOS)
static Boolean	MakeStartLowlyDosWorldJobs(void);
#endif /* defined(_MSDOS) */

static int	MakePrintStatus(GNode *gn, Boolean cycle);
 ...
 *      register GNode *cgn : the child we`ve jsut examined
 *
 * Side Effects:
 ...
 */
int
Make_TimeStamp (register GNode *pgn, register GNode *cgn)
{
...
Make_OODate (register GNode *gn)
{
	...
	/* WHY? */
	/* 
	 * Actually, there does seem to be a reason why.  If
	 * a dependency is out-of-date, but pmake has no
	 * idea of how to remake it (say, in the case of
	 * a system header file like stdapp.goh), then it's
	 * bad idea to remake the "parent" dependency just
	 * because this one is out-of-date.
	 */
	if (DEBUG(MAKE)) {
...
Make_HandleUse (register GNode *cgn, register GNode *pgn)
{
	...
	if (Lst_Open (cgn->children) == SUCCESS) {
		...
		if (Lst_Member (pgn->children, (ClientData)gn) == NILLNODE) {
		    (void) Lst_AtEnd (pgn->children, (ClientData)gn);
 ...
 */
void
Make_Update (register GNode *cgn)
{
	...
	while ((ln = Lst_Next (cgn->parents)) != NILLNODE) {
		...
		if (pgn->unmade == 0) {
		    ...
		    Error ("Graph cycles through %s", (unsigned long)pgn->name, 0, 0);
		}
	    }
	}
...
/*********************************************************************
 *			MakeStartLowlyDosWorldJobs
 *********************************************************************
 * SYNOPSIS: run the jobs on the toBeMade list one at a time
 * CALLED BY:	Make_Run
 * RETURN:  nothing
 * SIDE EFFECTS: nodes are removed from the toBeMade list
 * STRATEGY:
 * REVISION HISTORY:
 *	Name	Date		Description			     
 *	----	----		-----------			     
 *	jimmy	7/ 9/92		Initial version			     
 * 
 *********************************************************************/
static Boolean
MakeStartLowlyDosWorldJobs(void)
{
    register GNode	*gn;
#if defined(_MSDOS)
    char    *suffix;
...
#if defined(_MSDOS)
	/* deal with DOS 8.3 name space and EC names */
	if (!strcmp((suffix=strchr(gn->name, '.')), ".GEO"))
	{
	    if (strlen(gn->name) > 12)
	    {
		strcpy(gn->name+7, "E.GEO");
	    }
	}
#endif /* defined(_MSDOS) */
 ...
 *      Boolean cycle : TRUE if gn->unmade being non-zero implies a cycle
 *                      in the graph, not an error in an inferior.
 * Results:
 *	Always returns 0.
 *
 * Side Effects:
...
MakePrintStatus(GNode *gn, Boolean cycle)
{
	...
	if (cycle) {
	     ...
	     */
	    if (gn->made == CYCLE) {
		Error("Graph cycles through `%s'", (unsigned long)gn->name, 0, 0);
		gn->made = ENDCYCLE;
...
Make_Run (Lst targs)
{
    ...
    int	    	    errors = 0;	/* Number of errors the Job module reports */
	...
	if (!gn->make) {
	    ...
	    Suff_FindDeps (gn);
	    /*
	     * Now we get to deal with transformation rules that have .USE
	     * rules as sources (don't laugh -- it's happened before). If
	     * the search for dependencies added sources (likely), we need to
	     * process any .USE rules in the list of new sources that we didn't
	     * get to before. This also takes care of .USE rules hidden in
	     * dynamic sources.
	     */
	    Lst_ForEach(gn->children, Make_HandleUse, (ClientData)gn);
...
#else /* MSDOS */
    /* 
     * since running many jobs isn't an option, we just go through
     * the toBeMade list doing one lousy job at a time
     */
    MakeStartLowlyDosWorldJobs();
    if (queryFlag) {
	return TRUE;
