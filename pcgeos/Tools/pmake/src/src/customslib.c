...
#ifndef lint
static char *csid =
"$Id: customslib.c,v 1.3 93/02/01 11:59:04 adam Exp $ SPRITE (Berkeley)";
#endif lint
 ...
 */
Rpc_Stat
Customs_Host(short flags, ExportPermit *permitPtr)
{
    Host_Data	  data;
 ...
 */
Rpc_Stat
Customs_AvailInterval(struct timeval *interval)
{
    if (customs_Socket == -1) {
	CustomsInit();
    }
 ...
 */
Rpc_Stat
Customs_Master(struct sockaddr_in *masterAddrPtr)
{
    if (customs_Socket == -1) {
	CustomsInit();
    }
 ...
 */
Rpc_Stat
Customs_SetAvail(Avail_Data *criteria)
{
    if (customs_Socket == -1) {
	CustomsInit();
    }
 ...
 *
 * Note:
 *      buf should be of size MAX_INFO_SIZE
 *
 * Side Effects:
 *	Not really.
 *
 ...
 */
Rpc_Stat
Customs_Info(struct sockaddr_in *masterAddrPtr, char buf[])
{
    if (customs_Socket == -1) {
	CustomsInit();
    }
 ...
 */
int
Customs_MakeWayBill(ExportPermit *permitPtr, char *cwd, char *file,
                    char **argv, char **environ, unsigned short port,
		    char *buf)
{
    register char 	*cp;
    register int  	i;
 ...
 *      char         *cwd        : Current Directory.  NULL if not determined
 *      int           flags      : Flags to pass to Customs_Host
 *      int          *retSockPtr : Socket on which return call should be made
 *                                 when process exits.  If < 0, will return a
 ...
 */
int
Customs_RawExport(char *file, char **argv, char *cwd, int flags,
		  int *retSockPtr, ExportPermit *permitPtr)
{
    ...
    bzero(buf,MAX_DATA_SIZE);       /* Zeroes the buffer so we don't get
				     * unitialized memory read with purify */
