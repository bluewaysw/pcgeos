...
/* we have our own version of ffs(), so don't use config.h's */
#define HAVE_FFS

#include <config.h>

#ifndef lint
static char     *rcsid = "$Id: suff.c,v 1.7 96/06/24 15:07:20 tbradley Exp $ SPRITE (Berkeley)";
#endif lint
...
#if defined(unix)
#define cmpi_char(c1,c2) ((c1) == (c2))
#else
#define cmpi_char(c1,c2) (islower(c1) ? \
	(islower(c2) ? \
		(c1) == (c2) : \
		toupper(c1) == (c2)) : \
	(islower(c2) ? \
...
static char	*SuffStrIsPrefix(register char *perf, register char *str);
static char	*SuffSuffIsSuffix(register Suff *s, char *str);
static int	SuffSuffHasNameP(Suff *s, char *sname);
static int	SuffSuffIsPrefix(Suff *s, char *str);
static int	SuffGNHasNameP(GNode *gn, char *name);
static void	SuffFree(Suff *s);
static void	SuffInsert(Lst l, Suff *s);
static Boolean	SuffParseTransform(char *str, Suff **srcPtr, Suff **targPtr);
static int	SuffRebuildGraph(GNode *transform, Suff *s);
static int	SuffAddSrc(Suff *s, LstSrc *ls);
static void	SuffAddLevel(Lst l, Src *targ);
static void	SuffFreeSrc(Src *s);
static Src	*SuffFindThem(Lst srcs);
static Src	*SuffFindCmds(Src *targ);
static int	SuffExpandChildren(GNode *cgn, GNode *pgn);
static Boolean	SuffApplyTransform(GNode *tGn, GNode *sGn, Suff *t, Suff *s);

#if defined (unix)
static void	SuffFindArchiveDeps(GNode *gn);
#endif /* defined (unix) */

static void	SuffFindNormalDeps(GNode *gn);
static int	SuffPrintSuff(Suff *s);
static int	SuffPrintTrans(GNode *t);
 ...
 */
static char    *
SuffStrIsPrefix (register char *pref, register char *str)
{
    while (*str && cmpi_char(*pref,*str)) {
	pref++;
	str++;
    }
 ...
 */
static char *
SuffSuffIsSuffix (register Suff *s, char *str)
{
    ...
    while (p1 >= s->name && p2 >= str && cmpi_char(*p1,*p2)) {
	p1--;
	p2--;
    }
    return (p1 < s->name ? p2 : NULL);
}

/*-
 *-----------------------------------------------------------------------
...
Suff_ClearSuffixes (void)
{
    ...
    /*
     * Now nuke the various lists bound to the empty suffix, so we don't
     * have errant pointers attached to the null suffix that refer to
     * now-deceased suffixes from a past life.
     */
    Lst_Destroy (emptySuff->parents, NOFREE);
    Lst_Destroy (emptySuff->children, NOFREE);
    Dir_ClearPath (emptySuff->searchPath);

    emptySuff->children =    Lst_Init (FALSE);
    emptySuff->parents =     Lst_Init (FALSE);
}

/*-
 *-----------------------------------------------------------------------
...
Suff_AddSuffix (char *str)
{
	...
	MallocCheck (s, sizeof (Suff));

	s->name =   	Str_New (str);
#if defined(_MSDOS) || defined (_WIN32)
	/*
	 * upcase the suffix under DOS so we can use it when looking for
	 * files.
	 */
	for (cp = s->name; *cp; cp++) {
	    *cp = (char) toupper(*cp);
...
Suff_DoPaths(void)
{
    ...
    /*
     * The empty suffix isn't part of the list (it would cause excessive
     * searches if it were, since it is a suffix of everything, and we
     * only want to use it as a last resort).
     */
 ...
 *-----------------------------------------------------------------------
 * SuffSrcCycle  --
 *	Check the src chain for a given suffix.  Used to
 *	detect cycles in suffix dependencies.
 *
 * Results:
 *	TRUE if suffix is used somewhere in the src chain.
 *
 * Side Effects:
 *	None.
 *-----------------------------------------------------------------------
 */
static Boolean
SuffSrcCycle (Src *src, Suff *suff)
{
    for (; src != (Src *)0; src = src->parent) {
	if (src->suff == suff) {
	    return (TRUE);
	}
    }
...
SuffAddSrc (Suff *s, LstSrc *ls)
{
    ...
    if (SuffSrcCycle (targ, s)) {
	Error ("Suffix dependencies cycle through `%s%s'", 
		     (unsigned long)targ->pref, (unsigned long)s->name, 0);
	return (0);
    }
...
SuffExpandChildren(GNode *cgn, GNode *pgn)
{
	 ...
	 */
	ln = Lst_Find(sufflist, (ClientData)cgn->name, SuffSuffIsSuffixP);
...
SuffApplyTransform(GNode *tGn, GNode *sGn, Suff *t, Suff *s)
{
    ...
    /*
     * When not operating in compatibility mode, if the thing has commands,
     * we do *not* want to apply any transformations. I remember having
     * encountered makefiles that needed the search for sources even
     * when the thing has commands (this is what make does, even if it
     * doesn't use the commands), so I am leaving the search in in
     * compatibility mode (we also need to search to figure the proper
     * suffix so we can expand any dynamic sources). When being normal,
     * we do not want to apply transformations to such targets, however.
     */
#if defined(unix)
    if (OP_NO_CMDS(tGn) || amMake) {
#else
    /* no compatibility in DOS since make is a unix thing anyways, and it
     * causes certain cases for geodes not to work
     */
    if (OP_NO_CMDS(tGn)) {
#endif
...
SuffFindArchiveDeps(GNode *gn)
{
    ...
    *eoarch = '\0';	  /* So a suffix can be found */
	 ...
	 */
	ln = Lst_Find(ms->parents, (ClientData)gn->name, SuffSuffIsSuffixP);
...
SuffFindNormalDeps(GNode *gn)
{
	 ...
	 */
	ln = Lst_FindFrom(sufflist, ln,
			  (ClientData)gn->name, SuffSuffIsSuffixP);
	...
	if (ln != NILLNODE) {
	    ...
	    MallocCheck(targ->pref, prefLen + 1);
...
Suff_Init (void)
{
    ...
    MallocCheck (suffNull, sizeof (Suff));
    emptySuff = suffNull;
