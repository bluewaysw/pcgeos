...
#ifndef lint
static char *rcsid = "$Id: cond.c,v 1.5 96/06/24 15:05:10 tbradley Exp $ SPRITE (Berkeley)";
#endif lint
...
static int	CondGetArg    (char **linePtr, char **argsPtr, char *func,
			       Boolean parens);
static Boolean  CondDoDefined (int arglen, char *arg);
static int	CondStrMatch  (char *string, char *pattern);
static Boolean	CondDoMake    (int argten, char *arg);
static Boolean	CondDoExists  (int argten, char *arg);
static Boolean	CondDoTarget  (int argten, char *arg);
static double	CondCvtArg    (register char *str);
 ...
 *
 * parens should be TRUE if arg should be bounded by parens
 *
 *
 * Side Effects:
 ...
 */
static int
CondStrMatch(char *string, char *pattern)
{
    char    *i, *j;
    return(!Str_Match(string,pattern, &i, &j));
}

/*-
 *-----------------------------------------------------------------------
	...
	switch (*condExpr) {
		...
		if (*condExpr == '\0') {
		    Parse_Error(PARSE_WARNING,
				"Missing right-hand-side of operator", 0, 0);
		    goto error;
		}
		rhs = condExpr;
do_compare:
		if (*rhs == '"') {
		    ...
		    if (((*op != '!') && (*op != '=')) || (op[1] != '=')) {
			Parse_Error(PARSE_WARNING,
		"String comparison operator should be either == or !=", 0, 0);
			goto error;
		    }
...
#else /* _MSDOS or _WIN32 is case insensitive */
			t = stricmp(lhs, string) ? False : True;
#endif /* defined(unix) */
		    } else {
#if defined(unix)
			t = strcmp(lhs, string)  ? True : False;
#else /* _MSDOS or _WIN32 is case insensitive */
			t = stricmp(lhs, string) ? True : False;
#endif /* defined(unix) */
		    }
		    free(string);
		    if (rhs == condExpr) {
...
Cond_Eval (char *line)
{
	...
	if (skipIfLevel != 0) {
	    ...
	    if (condTop == MAXIF) {
		Parse_Error (level, "if-less endif", 0, 0);
		return (COND_INVALID);
	    } else {
		skipLine = FALSE;
		condTop += 1;
	...
	if (isElse && (line[0] == 's') && (line[1] == 'e')) {
	    if (condTop == MAXIF) {
		Parse_Error (level, "if-less else", 0, 0);
		return (COND_INVALID);
	...
	if (isElse) {
	    if (condTop == MAXIF) {
		Parse_Error (level, "if-less elif", 0, 0);
		return (COND_INVALID);
	    } else if (skipIfLevel != 0) {
		/*
	 ...
	 */
	Parse_Error (PARSE_FATAL, "Too many nested if's. %d max.", MAXIF, 0);
	return (COND_INVALID);
    } else {
