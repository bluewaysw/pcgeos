...
static int func_entry(int message);
void skip_comment(void);
static int str_entry(register int c);
...
endtok:			if (sp > tok) {
		...
		case ',':
			...
			if (goc) {
				...
				if (strcmp(tok,"instance") == 0) {
					instvar = YES;
				} else if (strcmp(tok,"vardata") == 0) {
					instvar = YES;
				} else if (strcmp(tok, "message") == 0) {
					message = YES;
				} else if (strcmp(tok, "prototype") == 0) {
					message = YES;
