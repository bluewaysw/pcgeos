...
#  define DayNoFloor(t) (JSE_FP_DIV(t,msPerDay))
...
MakeDay(jsenumber year,jsenumber month,jsenumber date)
{
   ...
   for( x=0;x<r6;x++ )
   {
      JSE_FP_ADD_EQ(t,JSE_FP_MUL(JSE_FP_CAST_FROM_SLONG(days_in_month[x]),msPerDay));
      if( x==1 && InLeapYear(t))
         JSE_FP_ADD_EQ(t,msPerDay);
   }
   ...
   t = (t + (double)d)-1.0 ;
   return t ;
//   return JSE_FP_SUB(JSE_FP_ADD(DayNoFloor(t),JSE_FP_CAST_FROM_SLONG(d)),jseOne);
}
...
#  endif
   ...
   a = MakeDay(now.TDAT_year, now.TDAT_month-1, now.TDAT_day-1) ;
   a *= msPerDay ;
		  b = MakeTime(now.TDAT_hours, now.TDAT_minutes, now.TDAT_seconds, 0);
   a += b ;
   return a ;
