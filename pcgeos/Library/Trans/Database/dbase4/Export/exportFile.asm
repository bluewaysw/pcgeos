...
DESCRIPTION:
	...
	$Id: exportFile.asm,v 1.1 97/04/07 11:43:26 newdeal Exp $
...
skip:
	...
	mov	es, ax
	clr	di
	mov	es:[di].DBH_version, DBASE4_NO_MEMO	; write out ver. num.
...
skip:
	...
	call	ExportGetFieldLength		; al - dbase IV data type 
	mov	es:[di].FD_fieldType, al	; save it
	cmp	al, 'F'				; float format?
...
SYNOPSIS:	Converts FieldDataType to dBase IV field type.

CALLED BY:	(INTERNAL) ExportGetDefaultFieldInfo

PASS:		al - FieldDataType

RETURN:		al - dBase IV field data type
...
checkReal:
	cmp	al, FDT_REAL
	jne	checkDate
	mov	al, 'F'				; convert it to float field
	jmp	exit
...
checkDate:
	cmp	al, FDT_DATE
	jne	checkTime
	mov	al, 'D'				; convert to date field
	jmp	exit
...
SYNOPSIS:	Given a field data type, returns field length and dBase IV
		data type.
...
checkReal:
	cmp	al, FDT_REAL
	jne	checkDate
	mov	al, 'F'				; convert it to numeric field
...
ExportIntegerToAscii	proc	near
	...
	; if float format, use the default decimal offset
