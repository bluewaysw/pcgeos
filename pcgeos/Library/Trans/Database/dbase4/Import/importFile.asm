...
DESCRIPTION:
	...
	$Id: importFile.asm,v 1.1 97/04/07 11:43:30 newdeal Exp $
...
SYNOPSIS:	Determines if the file is of dBase IV format.	
...
TransGetFormat	proc	far	uses	bx, dx, ds
	versionNo	local	byte
	.enter

	mov	cx, 1			; read in only one byte
	...
	; check to see if this is dbase IV file

	cmp	versionNo, DBASE4_NO_MEMO
	je	okay

	cmp	versionNo, DBASE4_MDX
	je	okay

	cmp	versionNo, DBASE4_MEMO
	jne	notFormat
okay:
...
RETURN:         ax - TransError
		bx - handle of error msg if ax = TE_CUSTOM
		   -  else clipboaedFormat = CIF_SPREADSHEET
...
ImportParseHeader	proc	near
	...
	; first check to see if this is indeed dBase IV file
...
notLogic:
	...
	jne	checkFloat		; if not, skip

	; now check to see if decimal count is non-zero

	tst	al
	je	integer			; if zero, then integer field
...
checkFloat:
	cmp     IGFD_Local.ISF_fieldType, 'F'
	jne	integer			; if not, skip	

	; now check to see if decimal count is zero

	tst	al
	jne	integer			; if not zero, then real number

	; if zero, then treat this field as an integer field

	mov	es:[di].FIB_fieldType, FDT_INTEGER 
	mov	IGFD_Local.ISF_fieldType, 'N'
integer:
	mov	cx, RESERVED2
	call	ImportSkipBytes			; skip four bytes
	jc	fileErr				; exit if file error
	...
	; check to see if map block exists

	tst	IGFD_Local.ISF_mapBlock		
	jne	mapExists			; if exists, skip

	; if no map block, add the new entry to the end
...
checkNum:
	...
	; check to see if float number field type
checkFloat:
	cmp	al, 'F'
	jne	checkLogic

	mov	al, FDT_REAL			; convert it to float field
	mov     es:[di].FIB_minValue.F_exponent, FP_NAN
	mov     es:[di].FIB_maxValue.F_exponent, FP_NAN
	jmp	common
...
date:
	cmp	al, 'D'
	je	noError
	stc					; if not a date field,
	jmp	exit				; then exit with carry set
noError:
	mov	al, FDT_DATE
	mov     es:[di].FIB_minValue.F_exponent, FP_NAN
	mov     es:[di].FIB_maxValue.F_exponent, FP_NAN
...
nextEntry:
	...
	stc					; if not, '*'
	jmp	fileErr				; then exit with carry set
delete:
...
checkFloat:
	cmp	al, 'F'
	jne	checkLogic
	call	ImportNumericField	; if float field, handle it
	jmp	common
...
date:
	...
	je	noError
	stc				; if not a date field
	jmp	common			; set carry flag and exit
noError:
...
ImportAddFieldToArray		proc	near	uses	ds, bp
	...
	tst	IAFTA_Local.ISF_mapBlock	; does the map block exist?
