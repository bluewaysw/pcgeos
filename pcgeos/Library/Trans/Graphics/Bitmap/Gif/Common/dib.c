...
ReadHeaderInfo(FILE * Infile)
{
   ...
   if (fread(buf,1,SIZEOFBMPFILEHDR,Infile ) <= 0 )
       {
       err = HSI_EC_SRCCANTREAD;
       goto cu0;
       }
 
   copybmf4buf(&bmfhdr,buf);       
   ...
   if (fread(buf,1,SIZEOFBMPINFOHDR, Infile) <= 0 )
       {
       err = HSI_EC_SRCCANTREAD;
       goto cu0;
       }
   ...
   else
   {
       if (*buf==0x0c)
	   {
	       ...
	       if (fread(buf,1,SIZEOFBMPCOREHDR,Infile) <= 0)
		   {
		       err = HSI_EC_SRCCANTREAD;
		       goto cu0;
		   }

	       copybmc4buf(&bmchdr,buf);
...
WriteHeaderInfo(VOID)
   ...
   n =  fwrite(buf2,                     /* write out file header */
          1,
          SIZEOFBMPFILEHDR,
          Outfile);
     if( n <= 0)
   ...
   n = fwrite(buf2,                     /* write out header info  */
          1,
          SIZEOFBMPINFOHDR,
          Outfile);
   ...
   for (i=0;i< (SHORT)nColors;i++)
       ...
       buf2[0]    = clrtbl[i].rgbBlue;
       buf2[1]    = clrtbl[i].rgbGreen;
       buf2[2]    = clrtbl[i].rgbRed;
       buf2[3]    = clrtbl[i].rgbReserved;
 
       n=fwrite(buf2,   1,    SIZEOFRGBQUAD,    Outfile);
...
*/
   ...
   nRead = fread ((LPSTR)buf3,1,40,infile);
   ...
   wHeader = *(LPWORD)&buf3[0];         /* get 1st two bytes */
 
   if ( wHeader == BFT_BITMAP )        /* Windows 3.0 or PM DIB */

       {
       wHeader = (WORD)*(LPDWORD)&buf3[14];   /* get biSize */
   ...
   if ( buf3[11] == 1 ||                /* check for window 2.0 bitmap */
        buf3[11] == 4 ||                /* assuming BitsPixel should be */
        buf3[11] == 8 )                 /* 1, 4 or 8 */
        {
        err = BMP_WIN20;
        goto cu0;
