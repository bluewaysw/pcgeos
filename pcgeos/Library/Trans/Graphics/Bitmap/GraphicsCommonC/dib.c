...
ReadHeaderInfo(FILE * Infile)
{
   ...
   if (fread(buf5,1,SIZEOFBMPFILEHDR,Infile ) <= 0 )
       {
       err = HSI_EC_SRCCANTREAD;
       goto cu0;
       }
 
   copybmf4buf(&bmfhdr,buf5);       
   ...
   if (fread(buf5,1,SIZEOFBMPINFOHDR, Infile) <= 0 )
       {
       err = HSI_EC_SRCCANTREAD;
       goto cu0;
       }
   ...
   else
   {
       if (*buf5==0x0c)
	   {
	       ...
	       if (fread(buf5,1,SIZEOFBMPCOREHDR,Infile) <= 0)
		   {
		       err = HSI_EC_SRCCANTREAD;
		       goto cu0;
		   }

	       copybmc4buf(&bmchdr,buf5);
   ...
   if ( nColors > 0 )
       ...
       for (i=0;i<nColors;i++)
           ...
           clrtbl[i].rgbBlue      = s[0];
           clrtbl[i].rgbGreen     = s[1];
           clrtbl[i].rgbRed       = s[2];
           clrtbl[i].rgbReserved  = s[3];
	 }
   }
...
WriteHeaderInfo(VOID)
   ...
   copybmf2buf(&bmfhdr,buf1);
 
   n =  fwrite(buf1,                     /* write out file header */
          1,
          SIZEOFBMPFILEHDR,
          Outfile);
     if( n <= 0)
   ...
   copybmi2buf(&bmihdr,buf1);
 
   n = fwrite(buf1,                     /* write out header info  */
          1,
          SIZEOFBMPINFOHDR,
          Outfile);
   ...
   for (i=0;i< (SHORT)nColors;i++)
       ...
       buf1[0]    = clrtbl[i].rgbBlue;
       buf1[1]    = clrtbl[i].rgbGreen;
       buf1[2]    = clrtbl[i].rgbRed;
       buf1[3]    = clrtbl[i].rgbReserved;
 
       n=fwrite(buf1,   1,    SIZEOFRGBQUAD,    Outfile);
...
WriteRawHeaderInfo(void)
   ...
   {
   LPSTR s6 = (LPSTR)malloc(nColors * SIZEOFRGBQUAD); 
 
   copyclrtbl2buf(clrtbl,s6,nColors);
...
LoadSystemColorTable( RGBQUAD *clr, SHORT *n )
   ...
   GetObject(hbm, sizeof(BITMAP), (LPSTR)s3);
 
   copybmp4buf(&bm,s); /* copy the buffer data to BITMAP */
...
copyfile(LPSTR szInfile,LPSTR szOutfile)
   ...
   while ((n= _lread(infile,buf6,size))>0)    /* copy the file body */
       _lwrite(outfile,buf6,n);
...
*/
   ...
   nRead = fread ((LPSTR)buf7,1,40,infile);
   ...
   wHeader = *(LPWORD)&buf7[0];         /* get 1st two bytes */
 
   if ( wHeader == BFT_BITMAP )        /* Windows 3.0 or PM DIB */

       {
       wHeader = (WORD)*(LPDWORD)&buf7[14];   /* get biSize */
   ...
   if ( buf7[11] == 1 ||                /* check for window 2.0 bitmap */
        buf7[11] == 4 ||                /* assuming BitsPixel should be */
        buf7[11] == 8 )                 /* 1, 4 or 8 */
        {
        err = BMP_WIN20;
        goto cu0;
