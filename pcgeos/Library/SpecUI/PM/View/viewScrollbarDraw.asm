...
DESCRIPTION:
	...
	$Id: viewScrollbarDraw.asm,v 1.8 94/12/21 11:26:43 adam Exp $
...
10$:
	...
	mov	dx, PM_SPIN_ARROW_HEIGHT*2+3	;dx = height of spin gadget
...
DrawUpArrow	proc	near
	...
	push	darkColor			;save darkColor
	test	es:[di].OLSBI_state, mask OLSS_AT_TOP	;see if at the top
	jz	checkSelectionState		;skip if no
	mov	darkColor, C_WHITE		;darkColor is temporarily white
...
draw:	push	di
	...
	pop	di
	pop	darkColor			;restore darkColor
exit:
	.leave
	ret
DrawUpArrow	endp
...
notFullSize:
	...
	jnz	drawThumb			;slider, don't invert

	;inverting thumb for now...
	push	ax
	mov	ah, es:[di].OLSBI_state		;get current state
	and	ah, mask OLSS_DOWN_FLAGS
...
DrawDownArrow	proc	near
	...
	push	ax
	test	es:[di].OLSBI_state, mask OLSS_AT_BOTTOM ;see if at the bottom
	jnz	whiteArrow			;if yes, draw white arrow

	mov	ax, es:[di].OLSBI_scrArea	;we must double check to see if
	sub	ax, es:[di].OLSBI_elevLen	;scrollbar is at bottom because
	sub	ax, es:[di].OLSBI_elevOffset	;OLSS_AT_BOTTOM is not reliable
	tst	ax
	jnz	checkSelectionState	
...
spinGadget	label word
	nptr	offset HVUpDownArrow	; left, top, right, then bottom
	word	1, 2, 1, PM_SPIN_ARROW_HEIGHT
	word	1, 1, MO_ARROW_WIDTH+1, 1
	word	MO_ARROW_WIDTH+1, 2, MO_ARROW_WIDTH+1, PM_SPIN_ARROW_HEIGHT-1
	word	2, PM_SPIN_ARROW_HEIGHT, \
		MO_ARROW_WIDTH+1, PM_SPIN_ARROW_HEIGHT
...
spinDnArrow	label	word
	nptr	offset HVUpDownArrow
	word	1, PARAM_3-PM_SPIN_ARROW_HEIGHT, 1, PARAM_3-2
	word	1, PARAM_3-PM_SPIN_ARROW_HEIGHT-1, \
		MO_ARROW_WIDTH+1, PARAM_3-PM_SPIN_ARROW_HEIGHT-1
	word	MO_ARROW_WIDTH+1, PARAM_3-PM_SPIN_ARROW_HEIGHT, \
		MO_ARROW_WIDTH+1, PARAM_3-3
	word	2, PARAM_3-2, MO_ARROW_WIDTH+1, PARAM_3-2
	word	1, PARAM_3-PM_SPIN_ARROW_HEIGHT-1, offset spinDownInside
	word	1, PARAM_3-PM_SPIN_ARROW_HEIGHT-1, offset spinDownOutside
	word	1, PARAM_3-PM_SPIN_ARROW_HEIGHT-1, \
...
if MO_ARROW_WIDTH eq 12
	...
	byte	01100110b, 01100000b
	byte	01001111b, 00100000b
	byte	01111111b, 11100000b
	byte	01111111b, 11100000b
	byte	01111111b, 11100000b
	byte	01111111b, 11100000b
...
if MO_ARROW_WIDTH eq 12
	...
	byte	00001100b, 11000000b
	byte	00011000b, 01100000b
endif
endif
...
HVUpDownArrow	proc	near
	...
	add	si, 12
	call	SetDarkColor		; set fill color
	call	FillSBBitmap
...
HVPageUpDn	proc	near
	...
	mov	al, SDM_50
	call	GrSetAreaMask
	call	SetDarkColor		;And again using dark color
	call	FillSBRect
