...
DESCRIPTION:
	...
	$Id: copenButtonColor.asm,v 1.3 98/05/04 06:38:19 joon Exp $
...
if not DRAW_STYLES
if (not _ISUI)	; no optimization for ISUI -- need to redraw everything
...
ISU <	call	DrawColorButtonBackground ;if not an UPDATE, then flush BG >
...
afterMarkDrawn:
	...
	call	GetDarkColor		;al <- dark color, ah <- CF_INDEX
if not DRAW_STYLES
if (not _ISUI)
...
50$:	test	ds:[di].OLBI_moreAttrs, mask OLBMA_IN_TOOLBOX
	...
	test	bx, mask OLBSS_MENU_DOWN_MARK	; If we have a menu down mark
	jz	55$				; in a toolbox, then we need
	inc	dl				; one more pixel
...
60$:	test	bx, mask OLBSS_IN_MENU_BAR or mask OLBSS_IN_MENU or \
		    mask OLBSS_MENU_DOWN_MARK	; Don't shift moniker if we
	jnz	70$				; are menu related.

65$:	test	bx, mask OLBSS_BORDERED		; If bordered and depressed,
	jz	70$				; we shift the moniker to the
	test	bx, mask OLBSS_DEPRESSED	; right one pixel.
	jz	70$
	inc	dl
...
if CURSOR_OUTSIDE_BOUNDS
	...
	call	GetLightColor			;al <- light ah <- CF_INDEX
	call	GrSetAreaColor
MO <	call	VisGetBounds						>
MO <	call	GrFillRect						>
ISU <	call	VisGetBounds						>
ISU <	call	GrFillRect						>
endif
...
if	SELECTION_BOX 
	...
	call	GetLightColor
	mov	ah, al
	jmp	drawBorder		;pass al = colors to use
isBordered:
...
MO   <	mov	ax, (C_BLACK shl 8) or C_WHITE	;ah = colorLT, al = colorRB  >
	...
	push	ax
	call	GetDarkColor
	mov	ah, C_WHITE			;ah = colorLT, al = colorRB
endif ; (not _ISUI)
...
if _ISUI
	test	bx, mask OLBSS_BORDERED
	jz	drawLtRb		;use passed colors if not bordered
	mov	ax, ds:[bp].CBR_ltRbColors
	test	bx, mask OLBSS_DEPRESSED
	jz	drawLtRb
	mov	ax, ds:[bp].CBR_depressedLtRbColors
drawLtRb:
endif
	push	ax
...
if _ISUI ;--------------------------------------------------------------------
	;
	; now to the inner lt/rb borders
	;
	test	bx, mask OLBSS_BORDERED
	jz	drawInnerLtRb		;use passed colors if not bordered
	mov	ax, ds:[bp].CBR_innerLtRbColors
	test	bx, mask OLBSS_DEPRESSED
	jz	drawInnerLtRb
	mov	ax, ds:[bp].CBR_depressedInnerLtRbColors
drawInnerLtRb:
	...
	clr	ah
	call	GrSetAreaColor
	mov	si, ds:[bp].CBR_innerLeftTop	;pass ds:si = region
	call	GrDrawRegionAtCP
	pop	ax

	clr	ah			;ax = colorRB
	call	GrSetAreaColor
	mov	si, ds:[bp].CBR_innerRightBottom
	call	GrDrawRegionAtCP
	;
...
if _ISUI
	test	ds:[si].OLBI_specState, mask OLBSS_IN_MENU_BAR or \
					mask OLBSS_IN_MENU or \
...
if _MOTIF   	;--------------------------------------------------------
	push	ax
	call	GetDarkColor		;  & use the dark color
...
if _ISUI	;--------------------------------------------------------------
	push	ax
	mov	ax, C_BLACK
	test	bx, mask OLBSS_MENU_DOWN_MARK	; use black if menu down mark
	jnz	20$
	test	bx, mask OLBSS_DEPRESSED	; use black if not depressed
	jz	20$
	mov	ax, C_WHITE			; else use white
20$:
	call	GrSetAreaColor
	pop	ax
endif		;--------------------------------------------------------------
...
if DRAW_STYLES
	...
	;
	mov	ah, C_BLACK
	test	bx, mask OLBSS_IN_MENU_BAR or mask OLBSS_IN_MENU or \
		    mask OLBSS_MENU_DOWN_MARK
	jz	textColor
	mov	ah, C_WHITE
else
