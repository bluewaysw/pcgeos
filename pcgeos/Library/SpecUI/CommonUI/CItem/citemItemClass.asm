...
DESCRIPTION:
	$Id: citemItemClass.asm,v 1.2 98/03/11 05:54:29 joon Exp $
...
ISU <	test	ds:[di].OLII_state, mask OLIS_IS_CHECKBOX		      >
ISU <	jnz	exit				;exit (z=0) if not scroll item>
...
if (_MOTIF or _ISUI) and ALLOW_TAB_ITEMS
	...
	and	al, mask OLIER_TAB_STYLE
	jz	notTab
	add	cx, (2*TABS_INSET_X)	; Add padding around moniker
	add	dx, (2*TABS_INSET_Y)
	CheckHack <OLITS_TAB_RIGHT eq OLITS_TAB_LEFT + 1>
	cmp	al, OLITS_TAB_LEFT
	jb	notTab
	cmp	al, OLITS_TAB_RIGHT
	ja	notTab
	add	cx, (2*TABS_INSET_X)	; Extra padding
notTab:
endif
