...
DESCRIPTION:
	$Id: citemItemGroupClass.asm,v 1.2 98/03/11 05:54:37 joon Exp $
...
if (_MOTIF or _ISUI) and ALLOW_TAB_ITEMS
	...
	mov	ax, HINT_ITEM_GROUP_DONT_CENTER_TABBED_CHILDREN
	call	ObjVarFindData
	jnc	done
	...
	jne	checkVert
	mov	al, ds:[di].VCI_geoDimensionAttrs
	and	al, not	mask VCGDA_WIDTH_JUSTIFICATION
	CheckHack < WJ_LEFT_JUSTIFY_CHILDREN eq 0 >
	mov	ds:[di].VCI_geoDimensionAttrs, al
...
checkVert:
	...
	jne	done
	mov	al, ds:[di].VCI_geoDimensionAttrs
	and	al, not	mask VCGDA_HEIGHT_JUSTIFICATION
	CheckHack < HJ_TOP_JUSTIFY_CHILDREN eq 0 >
	mov	ds:[di].VCI_geoDimensionAttrs, al
done:
endif
	.leave
