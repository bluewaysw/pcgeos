...
DESCRIPTION:
	...
	$Id: cviewPane.asm,v 1.2 98/03/11 06:02:57 joon Exp $
...
10$:
	...
	; don't bother if detaching
	mov	ax, MSG_GEN_APPLICATION_GET_STATE
	call	UserCallApplication
	test	ax, mask AS_DETACHING
	jnz	noBeep
	; don't bother if just beeped
	mov	ax, segment dgroup
	mov	ds, ax
	call	TimerGetCount
	pushdw	bxax
	subdw	bxax, ds:beepTime
	cmpdw	bxax, 2*60		; 2 seconds between beeps
	popdw	bxax
	jb	noBeep
	movdw	ds:beepTime, bxax
	mov	ax, SST_NO_INPUT
	call	UserStandardSound
noBeep:
	pop	ax, bx, cx, dx, bp, ds
exit:
	ret
...
;	call 	CallScrollbarParent	;call parent of vertical bar
	...
	call	CallVertScrollbar
	;
	; update position of newly added scrollbars
	;
	push	bp
	mov	di, ds:[si]
	add	di, ds:[di].Gen_offset
	movdw	dxcx, ds:[di].GVI_origin.PDF_y.DWF_int
	tst	ds:[di].GVI_origin.PDF_y.DWF_frac.high
	jns	gotY
	adddw	dxcx, 1
gotY:
	pushdw	dxcx
	movdw	dxcx, ds:[di].GVI_origin.PDF_x.DWF_int
...
gotX:
	...
	pop	bp
	;
	; make sure scrollbar knows how big the document is
	;
	push	bp				; preserve BP as done above
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	mov	ds:[di].OLPI_pageWidth, 0xffff	; force page size setting
	mov	ds:[di].OLPI_pageWidth, 0xffff
	call	OLPaneSetNewPageSize
	pop	bp
exit:
	ret
