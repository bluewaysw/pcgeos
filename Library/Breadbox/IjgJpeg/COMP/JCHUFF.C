 ...
 *
 * The jchuff_savable_state subrecord contains fields that change within an MCU,
...
typedef huff_entropy_encoder * jchuff_entropy_ptr;
...
{
  jchuff_entropy_ptr entropy = (jchuff_entropy_ptr) cinfo->entropy;
  int ci, dctbl, actbl;
  jpeg_component_info * compptr;

  if (gather_statistics) {
#ifdef ENTROPY_OPT_SUPPORTED
    MASSIGN(entropy->pub.encode_mcu, encode_mcu_gather);
    MASSIGN(entropy->pub.ee_finish_pass, finish_pass_gather);
#else
    ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
  } else {
    MASSIGN(entropy->pub.encode_mcu, encode_mcu_huff);
    MASSIGN(entropy->pub.ee_finish_pass, finish_pass_huff);
  }
...
EC( if (dctbl < 0 || dctbl >= NUM_HUFF_TBLS ||
	(cinfo->dc_huff_tbl_ptrs[dctbl] == NULL && !gather_statistics))
      ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, dctbl); )
EC( if (actbl < 0 || actbl >= NUM_HUFF_TBLS ||
...
/* Empty the output buffer; return TRUE if successful, FALSE if must suspend */
{
  ...
  if (! MCALL1(empty_output_buffer, dest, state->cinfo))
    return FALSE;
...
{
  jchuff_entropy_ptr entropy;
  int i;

  entropy = (jchuff_entropy_ptr)
    MCALL3(alloc_small, cinfo->mem, (j_common_ptr) cinfo, JPOOL_IMAGE,
				SIZEOF(huff_entropy_encoder));
  cinfo->entropy = (struct jpeg_entropy_encoder *) entropy;
  MASSIGN(entropy->pub.ee_start_pass, start_pass_huff);
