...
/* Emit a byte */
{
  ...
  *(dest->next_output_byte)++ = (JOCTET) val;
  if (--dest->free_in_buffer == 0) {
    if (! MCALL1(empty_output_buffer, dest, cinfo))
...
EC(if ((long) cinfo->image_height > 65535L ||
...
jinit_marker_writer (j_compress_ptr cinfo)
{
  ...
  MASSIGN(cinfo->marker->write_any_marker, write_any_marker);
  MASSIGN(cinfo->marker->write_file_header, write_file_header);
  MASSIGN(cinfo->marker->write_frame_header, write_frame_header);
  MASSIGN(cinfo->marker->write_scan_header, write_scan_header);
  MASSIGN(cinfo->marker->write_file_trailer, write_file_trailer);
  MASSIGN(cinfo->marker->write_tables_only, write_tables_only);
}
