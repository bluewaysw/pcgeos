...
typedef huff_entropy_decoder * jdhuff_entropy_ptr;
...
{
  jdhuff_entropy_ptr entropy = (jdhuff_entropy_ptr) cinfo->entropy;
  int ci, dctbl, actbl;
...
/* Load up the bit buffer to a depth of at least nbits */
{
  ...
  while (bits_left < MIN_GET_BITS) {
    ...
    if (bytes_in_buffer == 0) {
      if (! MCALL1(fill_input_buffer, state->cinfo->src, state->cinfo))
	return FALSE;
...
static const int jdhuff_extend_test[16] =   /* entry n is 2**(n-1) */
  { 0, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
    0x0100, 0x0200, 0x0400, 0x0800, 0x1000, 0x2000, 0x4000 };

static const int jdhuff_extend_offset[16] = /* entry n is (-1 << n) + 1 */
...
{
  jdhuff_entropy_ptr entropy;
  int i;

  entropy = (jdhuff_entropy_ptr)
    MCALL3(alloc_small, cinfo->mem, (j_common_ptr) cinfo, JPOOL_IMAGE,
