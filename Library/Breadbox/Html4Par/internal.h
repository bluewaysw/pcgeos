...
/*
 * Html4Par script features by actual build variant:
 *
 * Mode  Text  JS build  Product  J    A    H    Collect  Analyze  Exec code
 * ----  ----  --------  -------  ---  ---  ---  -------  -------  ---------
 * EC    DBCS  yes       JSDBCS   on   off  on   yes      no       yes
 * EC    DBCS  no        DBCS     off  off  off  no       no       no
 * EC    DBCS  no        ABDBCS   off  on   on   yes      yes      no
 * EC    SBCS  yes       JS       on   off  on   yes      no       yes
 * EC    SBCS  no        regular  off  off  off  no       no       no
 * EC    SBCS  no        AB       off  on   on   yes      yes      no
 * NC    DBCS  yes       JSDBCS   on   off  on   yes      no       yes
 * NC    DBCS  no        DBCS     off  off  off  no       no       no
 * NC    DBCS  no        ABDBCS   off  on   on   yes      yes      no
 * NC    SBCS  yes       JS       on   off  on   yes      no       yes
 * NC    SBCS  no        regular  off  off  off  no       no       no
 * NC    SBCS  no        AB       off  on   on   yes      yes      no
 *
 * J = JAVASCRIPT_SUPPORT. A = COMPILE_OPTION_AUTO_BROWSE.
 * H = derived HTML_SCRIPT_SUPPORT. Collect includes scripts and events.
 * Analyze means AutoBrowse analysis. JS and JSDBCS are parser-only products;
 * BbxBrow has no matching JS product, so no complete browser build executes JS.
 * No current product sets both J and A.
 */
