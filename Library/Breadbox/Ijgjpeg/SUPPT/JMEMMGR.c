...
// These are the "real" pointer types for control blocks, in
// contrast to the public jvirt_Xarray_ptr types which are just
// used as opaque pointers. This split is necessary to avoid having to
// include the Xarray structures in every module...
// This is necessary to keep symbolic debuggers like Swat happy.

typedef struct jvirt_sarray_control * jmemmgr_sarray_ptr;
typedef struct jvirt_barray_control * jmemmgr_barray_ptr;
...
struct jvirt_sarray_control {
  ...
  JDIMENSION cur_rows_in_mem;   /* number of rows in the buffer */
  JDIMENSION first_undef_row;	/* row # of first uninitialized row */
  boolean pre_zero;		/* pre-zero mode requested? */
  boolean dirty;		/* do current buffer contents need written? */
  boolean b_s_open;		/* is backing-store data valid? */
  jmemmgr_sarray_ptr * next; /* link to next virtual sarray control block */
...
struct jvirt_barray_control {
  ...
  boolean dirty;		/* do current buffer contents need written? */
  boolean b_s_open;		/* is backing-store data valid? */
  jmemmgr_barray_ptr * next;    /* link to next virtual barray control block */
  backing_store_info b_s_info;	/* System-dependent control info */
...
static const size_t first_pool_slop[JPOOL_NUMPOOLS] = 
{
	1600,			/* first PERMANENT pool */
        2000                    /* first IMAGE pool */
};

static const size_t extra_pool_slop[JPOOL_NUMPOOLS] = 
{
	0,			/* additional PERMANENT pools */
        1000                    /* additional IMAGE pools */
};
...
EC(if (sizeofobject > (size_t) (MAX_ALLOC_CHUNK-SIZEOF(small_pool_hdr)))
...
/* Allocate a 2-D sample array */
{
  ...
  ltemp = (PREFERRED_ALLOC_CHUNK-SIZEOF(large_pool_hdr)) /
	  ((long) samplesperrow * SIZEOF(JSAMPLE));
EC(if (ltemp < 0)
    ERREXIT(cinfo, JERR_WIDTH_OVERFLOW); )
  if (ltemp == 0)                       // Byte the bullet, make it larger...
    ltemp = 1;
...
/* Allocate a 2-D coefficient-block array */
{
  ...
  /* Calculate max # of rows allowed in one allocation chunk */
  ltemp = (PREFERRED_ALLOC_CHUNK-SIZEOF(MemHandle)) /
...
{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  jmemmgr_sarray_ptr result;

  /* Only IMAGE-lifetime virtual arrays are currently supported */
EC(if (pool_id != JPOOL_IMAGE)
    ERREXIT1(cinfo, JERR_BAD_POOL_ID, pool_id); /* safety check */ )

  /* get control block */
  result = (jmemmgr_sarray_ptr) alloc_small(cinfo, pool_id,
...
EC(if (pool_id != JPOOL_IMAGE)
  ...
  /* get control block */
  result = (jmemmgr_barray_ptr) alloc_small(cinfo, pool_id,
...
{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  jmemmgr_sarray_ptr sptr;
  jmemmgr_barray_ptr bptr;

  /* Allocate the in-memory buffers and initialize backing store as needed. */

  for (sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
    if (sptr->mem_buffer == NULL) { /* if not realized yet */
      sptr->rows_in_mem = (JDIMENSION) sptr->maxaccess;
  ...
  for (bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
    if (bptr->mem_buffer == NULL) { /* if not realized yet */
      bptr->rows_in_mem = (JDIMENSION) bptr->maxaccess;
...
METHODDEF(JSAMPARRAY)
access_virt_sarray (j_common_ptr cinfo, jvirt_sarray_ptr _ptr,
...
METHODDEF(JBLOCKARRAY)
access_virt_barray (j_common_ptr cinfo, jvirt_barray_ptr _ptr,
		    JDIMENSION start_row, JDIMENSION num_rows,
		    boolean writable)
...
EC(size_t test_mac; )
   ...
   */
EC(if ((SIZEOF(ALIGN_TYPE) & (SIZEOF(ALIGN_TYPE)-1)) != 0)
   ...
   */
EC(test_mac = (size_t) MAX_ALLOC_CHUNK; )
EC(if ((long) test_mac != MAX_ALLOC_CHUNK ||
  ...
  MASSIGN(mem->pub.alloc_small, alloc_small);
  MASSIGN(mem->pub.alloc_large, alloc_large);
  MASSIGN(mem->pub.alloc_sarray, alloc_sarray);
  MASSIGN(mem->pub.alloc_barray, alloc_barray);
  MASSIGN(mem->pub.request_virt_sarray, request_virt_sarray);
  MASSIGN(mem->pub.request_virt_barray, request_virt_barray);
  MASSIGN(mem->pub.realize_virt_arrays, realize_virt_arrays);
  MASSIGN(mem->pub.access_virt_sarray, access_virt_sarray);
  MASSIGN(mem->pub.access_virt_barray, access_virt_barray);
  MASSIGN(mem->pub.free_pool, free_pool);
  MASSIGN(mem->pub.self_destruct, self_destruct);
