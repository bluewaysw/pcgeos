...
DESCRIPTION:
	...
	$Id: cmainCode.asm,v 2.85 97/03/11 18:06:34 cassie Exp $
...
addSimpleHelp::
	;
	; Add a simple help control object to the UIApp
	;
		clr	ax, cx
		mov	bx, handle SimpleHelpResource
		call	ObjDuplicateResource
		mov	cx, bx
		mov	dx, offset SimpleHelp
		...
		mov	di, mask MF_FORCE_QUEUE
		GOTO	ObjMessage
else				; else of ifdef WIZARDBA

if (not _REDMOTIF)
IKBD<	call	InitFloatingKbd		>
endif				; end of if (not _REDMOTIF)

endif				; end of else of ifdef WIZARDBA
done:
		ret
LibraryEntry	endp
...
; Set UIWO_KBD_NAVIGATION true if advanced intermediate.
	...
	;
ifdef WIZARDBA	; for wizard we always want pinnable menus
	or	bl, mask UIWO_MAXIMIZE_ON_STARTUP or \
...
;
	...
	tst	ax
	jnz	haveWinMenu
if (not _ODIE)		; no window menu for ODIE
	or	bl, mask UIWO_WINDOW_MENU
endif
haveWinMenu:
PMAN <	or	bl, mask UIWO_WINDOW_MENU	; default on for PM	>
...
ifdef WIZARDBA
	;
	; default is based on user type (student, teacher, office worker, etc.)
	;
	mov	bx, UIEP_TOP_PRIMARY shl offset UIEO_POSITION

	call	IclasGetCurrentUserType
	cmp	ah, UT_STUDENT
	je	haveDefaultExpressPrefs
	cmp	ah, UT_GENERIC
	je	haveDefaultExpressPrefs
...
haveDefaultExpressPrefs:
	...
	mov	bx, UIEP_NONE shl offset UIEO_POSITION
...
haveDecision:
	...
	mov	bx, mask UIEO_UTILITIES_PANEL or \
			UIEP_TOP_PRIMARY shl offset UIEO_POSITION
endif
...
if not _RUDY					; just use the outline
...
if _JEDIMOTIF
	;
	; 6/15/95 -jw
	; Don't do this for Jedi. Jedi never wants this bit set.
	;
else
