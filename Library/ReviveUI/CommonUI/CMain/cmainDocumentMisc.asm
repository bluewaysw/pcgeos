...
DESCRIPTION:
	...
	$Id: cmainDocumentMisc.asm,v 1.31 97/02/13 22:09:49 kho Exp $
...
if _RUDY
	;
	; if the file is deleted somehow, we just make a new one
	; -- kho, 11/8/95
	;
...
SYNOPSIS:	destroy and delete document

CALLED BY:	MSG_OL_DOCUMENT_DESTROY
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if _JEDIMOTIF
OLDocumentDestroy	method dynamic OLDocumentClass, 
					MSG_OL_DOCUMENT_DESTROY
	mov	cx, TRUE
	call	OLDocDestroyDocument
	;
	...
	call	FinishDocOperation
	ret
OLDocumentDestroy	endm
endif
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLDocumentCloseForNew
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	close this document before creating new one

CALLED BY:	MSG_OL_DOCUMENT_CLOSE_FOR_NEW
PASS:		*ds:si	= OLDocumentClass object
		ds:di	= OLDocumentClass instance data
		ds:bx	= OLDocumentClass object (same as *ds:si)
		es 	= segment of OLDocumentClass
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	brianc	5/ 6/95   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if _JEDIMOTIF
OLDocumentCloseForNew	method dynamic OLDocumentClass, 
					MSG_OL_DOCUMENT_CLOSE_FOR_NEW
	mov	ax, TEMP_OL_DOCUMENT_CLOSE_FOR_NEW
	clr	cx
	call	ObjVarAddData
	mov	ax, MSG_GEN_DOCUMENT_CLOSE
	call	ObjCallInstanceNoLock
	ret
OLDocumentCloseForNew	endm
endif	; _JEDIMOTIF
...
if not _RUDY
	;
	; if _RUDY, we save automatically
	;
...
if _JEDIMOTIF
	;
	; create new document, or return to list screen
	;
	call	FinishDocOperation
else
...
if (not _JEDIMOTIF)		; don't ask for JEDI, GDA_SAVE_FAILED is
				;	set on failed auto-save
	test	ax, mask GDA_SAVE_FAILED
	LONG	jnz	askSave
endif
...
if _JEDIMOTIF
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	test	ds:[di].OLDI_attrs, mask OLDA_AUTO_SAVE_ERROR
	mov	ax, MSG_GEN_DOCUMENT_CONTROL_INITIATE_SAVE_DOC
	jz	haveMsg				; no auto save error
	mov	ax, MSG_OLDC_INITIATE_SAVE_DOC_FOR_AUTO_SAVE_ERROR
haveMsg:
else
	mov	ax, MSG_GEN_DOCUMENT_CONTROL_INITIATE_SAVE_DOC
endif
...
if _JEDIMOTIF
	andnf	ds:[di].OLDI_attrs, not mask OLDA_AUTO_SAVE_ERROR
endif
...
if _JEDIMOTIF	;------------------------------------------------------------
	...
	;
	; We'll only be doing a Save As while closing an untitled document.
	; Handle this by just saving and renaming, dealing with conflicting
	; name first.
	;
	mov	di, ds:[si]
	add	di, ds:[di].Gen_offset
	...
	jc	noFile
	;
	; file exists, delete if overwrite, else error handling
	;
	test	ss:[bp].DCP_flags, mask DOF_SAVE_AS_OVERWRITE_EXISTING_FILE
	jnz	overwrite
	mov	ax, ERROR_FILE_EXISTS
	jmp	showError		; will put up Save As if not overwrite
					;	or will doSaveAs if overwrite
					;	with OVERWRITE flag set
...
noFile:
	;
	; save changes to untitled doc
	;
	push	bp
	mov	ax, MSG_GEN_DOCUMENT_PHYSICAL_SAVE
	call	ObjCallInstanceNoLock	; returns error
	pop	bp
	LONG jc	handleError
	;
	; saved successfully -- set rename-after-close flag
	;
...
exit:
	...
	jnc	reallyExit
if (not _JEDIMOTIF)	; since JEDI only does Save As on closing untitled,
			;	no reason to ever clear closing
if HANDLE_DISK_FULL_ON_SAVE_AS
	;
...
if _NIKE
	...
	; HACK: The DOF_NO_ERROR_DIALOG flag is set in
	; OLDocumentControlInitiateSaveDoc if want to save a default document.
	; We check the flag here to put up a different error dialog.

	test	ss:[bp].DCP_flags, mask DOF_NO_ERROR_DIALOG
	jz	notFileExists

	mov	ax, SDBT_FILE_SAVE_AS_DEFAULT_DOCUMENT_EXISTS
	call	CallUserStandardDialog
	cmp	ax, IC_YES		;if IC_YES, then overWrite exisiting
	je	overWrite
	cmp	ax, IC_NO		;if IC_NO, then back to Save As dialog
	jne	stillAnError		; else signal error (cx=CLOSE_CANCELLD)
	jmp	backToSaveAs
notFileExists:
endif	; if _NIKE
...
SYNOPSIS:	finish closing of document

CALLED BY:	OLDocumentSaveAs, OLDocumentDestroy, OLDocumentClose
PASS:		*ds:si = OLDocumentClass
		ax = message to send to doc control
RETURN:		nothing
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if _JEDIMOTIF
FinishDocOperation	proc	far
	mov	di, ax				; save message
	;
	; If we save-as/closed this document in order to create new one,
	; do that now
	;
	mov	ax, TEMP_OL_DOCUMENT_CLOSE_FOR_NEW
	call	ObjVarFindData
	...
	mov	ax, MSG_OLDC_INITIATE_NEW_DOC_NOW
	push	si
	mov	bx, segment OLDocumentControlClass
	mov	si, offset OLDocumentControlClass
	mov	di, mask MF_RECORD
	call	ObjMessage
	pop	si
	push	si
	call	GenFindParent			; ^lbx:si = document group
	mov	cx, di
	mov	dx, TO_GEN_PARENT		; to document control
	mov	ax, MSG_META_SEND_CLASSED_EVENT
	mov	di, mask MF_FORCE_QUEUE
	call	ObjMessage
	pop	si
...
notCloseForNew:
	mov	ax, di				; ax = message for doc control
	call	SendToDocControlWithFlushQ
done:
	ret
FinishDocOperation	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		SendToDocControlWithFlushQ
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Flush the process queue, and then dispatch the recorded
		message.

CALLED BY:	FinishDocOperation()
PASS:		*ds:si	= GenDocumentClass instance
		ax	= message sent to doc control
RETURN:		nothing
DESTROYED:	ax, bx, cx, dx, di
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	CL	7/ 6/95    	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
SendToDocControlWithFlushQ	proc	near
		uses	bp
		.enter
		Assert	objectPtr dssi, GenDocumentClass
	;
	; Record the message to send to the document control.
	;
		push	si
		mov	bx, segment GenDocumentControlClass
		mov	si, offset GenDocumentControlClass
		...
		pop	si
	;
	; Dispatch the recorded message through the gen parent.
	;
		push	si
		...
		mov	ax, MSG_META_SEND_CLASSED_EVENT
		mov	di, mask MF_RECORD
		call	GenFindParent			;^lbx:si = parent obj
		call	ObjMessage			;di = event handle
		pop	si
	;
	; Flush the queue to make sure that the recorded event will be
	; received late enough.
	;
		mov	cx, di				;cx = event handle
		mov	dx, ds:[LMBH_handle]
		mov	bp, OFIQNS_INPUT_OBJ_OF_OWNING_GEODE
		mov	ax, MSG_META_OBJ_FLUSH_INPUT_QUEUE
		call	ObjCallInstanceNoLock
		.leave
		ret
SendToDocControlWithFlushQ		endp
endif
...
if _JEDIMOTIF
	;
	; shouldn't need this, since there'll be no "Cancel" trigger if
	; this is set, but...
	;
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	andnf	ds:[di].OLDI_attrs, not mask OLDA_AUTO_SAVE_ERROR
	;
	; no longer closing current file for "New" operation
	;
	mov	ax, TEMP_OL_DOCUMENT_CLOSE_FOR_NEW
	call	ObjVarDeleteData
endif
...
if _RUDY
	;
	; Assume that if the data file has been deleted in the interim, we
	; can just reopen another one under Rudy.  -cbh 10/13/95
	;
	mov	ss:[bp].DCP_flags, mask DOF_CREATE_FILE_IF_FILE_DOES_NOT_EXIST
endif
