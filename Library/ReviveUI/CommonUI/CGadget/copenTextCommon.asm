...
ROUTINES:
  ...
  ----				-----------
    ...
    INT RudyGetBaselineOffset   Gets the baseline offset of the text
				object.

    INT RudyGetSystemFontInfo   Get's the height, accent, and baseline
				offset of the system default font
...
DESCRIPTION:
	...
	$Id: copenTextCommon.asm,v 1.46 97/01/06 16:10:23 brianc Exp $
...
OLTextDraw	method dynamic OLTextClass, MSG_VIS_DRAW
	push	cx				;
	test	ds:[di].OLTDI_specState, mask TDSS_IN_VIEW
	jnz	drawText			; in view, no borders!
...
10$:
	...
	add	di, ds:[di].Vis_offset		;
	test	ds:[di].OLTDI_specState, mask TDSS_IN_FRAME
	jz	drawText			; skip if not

	;
	; We'll draw the frame in black, and clear with the fill color.
	;
CUA <	mov	ax, C_WHITE or (C_BLACK shl 8)	; al has fill color, ah frame >
MAC <	mov	ax, C_WHITE or (C_BLACK shl 8)	; al has fill color, ah frame >
   
	test	ds:[di].OLTDI_specState, mask TDSS_COLOR_SET
	jz	20$				; no special color, branch
	
	mov	di, ds:[si]			; point to instance
	add	di, ds:[di].Vis_offset		; ds:[di] -- VisInstance
	test	ds:[di].VTI_washColor.SCP_index.SCPI_info, mask VTCMM_COLOR_FLAG
	jnz	20$				; some RGB color, forget it
						; else use as wash color here
	mov	al, ds:[di].VTI_washColor.SCP_index.SCPI_index
	cmp	al, C_BLACK			; is the wash color black?
	jne	20$				; no, branch
	mov	ah, C_WHITE			; else we'll use a white frame
20$:
	push	ax				; save line color
	clr	ah				; ax has fill color
	mov	di, bp				; di = GState
	call	GrSetAreaColor			;			      
	pop	ax				; get line color in ax
	mov	al, ah
	clr	ah
	call	GrSetLineColor			; set as line color
...
if _JEDIMOTIF
	;
	; text objects (non-scrollable ones handled here) have a 50% frame
	;
	mov	al, SDM_50
	call	GrSetLineMask
endif
...
if _JEDIMOTIF
	;
	; Clear out title-bar greebles.
	;
	call	MaybeClearTitleBarArea
endif
	pop	cx				; restore draw mode
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		MaybeClearTitleBarArea
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	If we're in the title bar do special drawing.
...
MaybeClearTitleBarArea	proc	near
		uses	ax,bx,cx,dx,si,di,bp
		.enter

EC <		call	ECCheckGenTextObject				>
	;
	;  The user has to have the hint specifically on this
	;  text object, but we'll document it as such.  Too bad
	;  OLTextClass isn't a subclass of OLCtrlClass...
	;
...
inTitleBar:
	;
	;  If it's single-line, clear out the area under it.
	;  Should we allow multi-line text objects in the title
	;  bar?
	;
		mov	bp, di			; bp = gstate
		mov	di, ds:[si]
...
noToughGuy:
endif	;BUBBLE_DIALOGS and (not (_ODIE or _DUI))
...
if (not _ODIE)			; let's not do this for ODIE as it messes
				;	up for multi-line, multi-font objects
...
if _RUDY
	push	dx, bp
	push	ax				; save left
	call	RudyGetBaselineOffset		; bx -> text baseline
	jnc	geometryNotReady		; failed, quit
	call	RudyGetSystemFontInfo		; bp -> system height
						; - system accent
						; dx -> system baseline
						; ax -> system accent
	call	OLTextDerefVis			; ds:di = our Vis instance

	cmp	dx, bx				; baselines equal?
	je	exit				; YES, no calc., quit
	sub	bx, dx
	add	cx, bx				; top = top + text
						; baseline - system baseline
	neg	ax 
	add	ax, ds:[di].VI_bounds.R_top	; ax = artificial top
	cmp	cx, ax				; new top >= artificial top?
	jae	checkBottom			; YES, ok now checkBottom
	mov	cx, ax				; reset top to artificial top
	jmp	exit
checkBottom:
	mov	si, cx
	add	si, bp
	cmp	si, ds:[di].VI_bounds.R_bottom	; new bottom <= bottom bound?
	jbe	exit				; YES, all OK
	sub	si, ds:[di].VI_bounds.R_bottom
	sub	cx, si				; new top = new top -
						; new bottom - bottom bound
	jmp	exit
geometryNotReady:
	clc
	jmp	quit
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		RudyGetBaselineOffset
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:       Gets the baseline offset of the text object.

CALLED BY:      OLTextMkrPos
PASS:           *ds:si  = OLTextClass
RETURN:         bx      = baseline offset
		carry set - geometry OK
DESTROYED:     	nothing
SIDE EFFECTS:
...
REVISION HISTORY:
        Name    Date            Description
        ----    ----            -----------
        reza	12/14/94        Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
RudyGetBaselineOffset	proc	near
vtglipBuffer    local   VisTextGetLineInfoParameters
lineinfoBuffer  local   LineInfo
	uses	ax, cx, dx, bp
        .enter
EC <            Assert  objectPtr dssi OLTextClass                     >

        lea     dx, lineinfoBuffer      ; why use si here?
        movdw   vtglipBuffer.VTGLIP_buffer, ssdx
        mov     vtglipBuffer.VTGLIP_bsize, MAX_LINE_INFO_SIZE
        clrdw   vtglipBuffer.VTGLIP_line

        push    bp
        mov     dx, ss
        lea     bp, vtglipBuffer        ; dx:bp = vtglipBuffer
        mov     ax, MSG_VIS_TEXT_GET_LINE_INFO
        call    ObjCallInstanceNoLock   ; cx -> # bytes of data
        pop     bp
	jc	getInfoFail
        mov     bx, lineinfoBuffer.LI_blo.WBF_int
	stc
	jmp	exit
getInfoFail:
	clc
exit:
        .leave
        ret
RudyGetBaselineOffset	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		RudyGetSystemFontInfo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Get's the height, accent, and baseline offset of the
		system default font

CALLED BY:	OLTextMkrPos
PASS:		*ds:si	= OLTextClass object
		es	= segment of OLTextClass
RETURN:		bp	= system height
		ax	= system accent
		dx	= system baseline
DESTROYED:	nothing
SIDE EFFECTS:	
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	RH	12/15/94    	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
RudyGetSystemFontInfo	proc	near
	uses	di, si
	.enter
EC <            Assert  objectPtr dssi OLTextClass                     >
	mov	di, offset OLTextClass
	mov     ax, MSG_VIS_VUP_CREATE_GSTATE
	call    ObjCallSuperNoLock      	; bp -> gstate handle
EC <    Assert  gstate, bp                                      >
	mov     di, bp

	mov	si, GFMI_ROUNDED or GFMI_ACCENT
	call	GrFontMetrics
	mov	ax, dx				; ax = system accent

	mov	si, GFMI_ROUNDED or GFMI_HEIGHT
	call	GrFontMetrics
	mov	bp, dx				; bp = system height

	mov	si, GFMI_ROUNDED or GFMI_BASELINE
	call	GrFontMetrics			; dx -> system baseline
	call	GrDestroyState

	.leave
	ret
RudyGetSystemFontInfo	endp
endif
...
if _RUDY
	;
	; See notes above -- kho, 6/23/95
	;
	; Commented altogether -- kho, 9/14/95
	;
	; test	dl, mask CF_RELEASE		;ignore releases
	; jnz	noNavigate
endif		; _RUDY
...
if _RUDY
	;
	; We need to ignore releases of C_ENTER if we're
	; singleLineText, yet preventing our text object from
	; receiving the keystroke, otherwise the apply messge will
	; be sent out. -- reza 12/30/96
	;
...
if _RUDY
	;
	; Need to force-queue a message for 2 reasons:
	;   1) The Vis linkage for a parent Content might not be
	;      complete, so CallOLWin will fail.
	;   2) We want to be able to pre-empt earlier update messages,
	;      to avoid flashing change triggers.
	; 
	mov	bx, ds:[LMBH_handle]
	mov	cx, MSG_GEN_SET_NOT_ENABLED
	mov	ax, MSG_SPEC_TEXT_UPDATE_CHANGE_TRIGGER
	mov	di, mask MF_FORCE_QUEUE or mask MF_CHECK_DUPLICATE or \
			mask MF_REPLACE or mask MF_MATCH_ALL
	call	ObjMessage
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLTOlTextUpdateChangeTrigger
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Sends a message to the IC_CHANGE trigger to update
		it's state.

CALLED BY:	MSG_SPEC_TEXT_UPDATE_CHANGE_TRIGGER
PASS:		*ds:si	= OLTextClass object
		ds:di	= OLTextClass instance data
		ds:bx	= OLTextClass object (same as *ds:si)
		es 	= segment of OLTextClass
		ax	= message #
		cx	= message to send to change trigger
RETURN:		
DESTROYED:	
SIDE EFFECTS:	
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	CT	1/ 9/96   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if _RUDY
OLTOlTextUpdateChangeTrigger	method dynamic OLTextClass, 
					MSG_SPEC_TEXT_UPDATE_CHANGE_TRIGGER
	.enter
...
nope:
	.leave
	ret
OLTOlTextUpdateChangeTrigger	endm
endif ; _RUDY
...
if _RUDY
	;
	; We want the cursor to be as tall as the font height.
	;
	mov	si, GFMI_BASELINE or GFMI_ROUNDED
	call	GrFontMetrics				;dx = baseline
	mov	cx, dx					;cx = baseline
else	
...
if _RUDY
	;
	; 2 pixel thick cursor.
	;
	sub	bx, cx					;top = y - baseline
	mov	cx, ax
	add	cx, 2					;right = left + 2
else
