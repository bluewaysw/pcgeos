...
DESCRIPTION:
	...
	$Id: copenTextBuild.asm,v 1.33 97/03/06 02:42:14 brianc Exp $
...
if _RUDY
	;
	; We need to set this bit first before calling superclass,
	; because our superclass (UnderlinedGenText) checks this flag.
	;
	...
	jz	100$
	ornf	ds:[bx].VTI_state, mask VTS_ONE_LINE

100$:
	push	ax
	mov	ax, HINT_TEXT_UNDERLINE
	mov	di, ATTR_UNDERLINED_VIS_TEXT_NO_UNDERLINES
	call	copyVarDataIfNotPresent
	pop	ax
endif
...
if _RUDY
	mov	ax, ATTR_GEN_TEXT_XOR_SELECTION
	mov	di, ATTR_VIS_TEXT_XOR_SELECTION
	call	copyVarData
endif
...
if (not _DUI) and (not _ODIE)		; don't make larger on DUI and ODIE
	call	SysGetPenMode
	tst	ax
...
if _JEDIMOTIF
	mov	ds:[bx].VTI_lrMargin, LR_FRAME_TEXT_MARGIN
else
...
if _JEDIMOTIF
	mov	ds:[di].VTI_lrMargin, LR_FRAME_TEXT_MARGIN
else
...
setVertAttrs:
if _PCV							; no scroll bars on
	ornf	al, mask GVDA_DONT_DISPLAY_SCROLLBAR	;  GenTexts for PCV
endif							;  ron 2/21/96
 	mov	ds:[bx].GVI_vertAttrs, al	; store view attributes

if _RUDY
	;
	; Don't set GVA_DONT_SEND_KBD_RELEASES for Rudy, because of the
	; Charset function in VisText.
	;
...
if _RUDY
	; And indent the text edit field to line up with popup list monikers
	; if the text object has HINT_ALIGN_WITH_CENTERED_MONIKERS.

	mov	di, ax				;di <- compHandle
	mov	ax, HINT_ALIGN_WITH_CENTERED_MONIKERS
	call	ObjVarFindData
	jnc	noIndent
