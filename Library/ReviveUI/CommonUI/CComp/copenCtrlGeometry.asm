...
ROUTINES:
    ...
    INT RudyHandleChildIndent   Handles indenting children, if the hint
				exists.

    INT RudyCtrlExtraLeftMargin Returns the amount of extra space to leave
				in the left margin because the OLCtrl is
				either an OLCRF_SETTINGS_TRIGGER or has the
				OLCRF_DRAW_RIGHT_ARROW attribute.
...
DESCRIPTION:
	...
	$Id: copenCtrlGeometry.asm,v 1.31 97/01/02 23:06:05 joon Exp $
...
if _JEDIMOTIF
	;
	; 1 pixel spacing for objects in interactions in title bar,
	; (2 pixel spacing for interactions/objects in title bar handled
	; in OLTitleGroupClass), actually we do small cheat/optimization
	; here and just check for OLTitleGroupClass
	;
	push	di
	...
	pop	di
	mov	cx, 1			;1 pixel spacing
	jnc	haveSize
	mov	cx, 2			;2 pixel spacing in OLTitleGroup
haveSize:
else
...
if	_RUDY or _JEDIMOTIF
	;Sign extend the returned value
	test	cx, 1 shl (offset SSS_TYPE-1)
	jz	isPositive
	ornf	cx, mask SSS_TYPE	
isPositive:
else
EC <	tst	cx							>
...
if _RUDY
	;
	; If a settings trigger, add extra top and bottom margin
	;
	test	ds:[di].OLCI_rudyFlags, mask OLCRF_SETTINGS_TRIGGER
	jz	notSettingsTrigger
	add	bp, CTRL_SETTINGS_TRIGGER_TOP_MARGIN
	add	dx, CTRL_SETTINGS_TRIGGER_BOTTOM_MARGIN
notSettingsTrigger:
endif
...
if	 _RUDY
	;
	; For RUDY, the extra space (used by a bitmap of some kind) is part
	; of the margin but is NOT part of the moniker space.  So, if we are
	; going to use the moniker space of the parent (max of childrens'
	; moniker space), we need to add back in the extra margin.
	;
	push	bp
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
...
if	 _RUDY and ASSUME_BOLD_STYLE_FOR_MONIKERS
    	;
	; if ASSUME_BOLD_STYLE_FOR_MONIKERS is true, then
	; we want to check OLCRF_FORCE_BOLD_MONIKER.  If this is true, then
	; we need to be sure to pass a gstate to SpecGetGenMonikerSize that
	; includes the bold font and the FOAM_LARGE_FONT_SIZE.
	; We destroy it after the call.
	;
	clr	bp				;assumes normal font
...
else	;_RUDY and ASSUME_BOLD_STYLE_FOR_MONIKERS is FALSE
...
if _RUDY
	mov	ax, MO_CONTROL_MKR_Y_SPACING+RUDY_SPACE_BELOW_BOXED_MONIKER
else
...
if _RUDY
	;
	; We always want to add in the box margins, in addition to any
	; moniker margins.  I admit to not understanding why
	; the comparison is being done in the first place.
	;
	add	bp, OL_CTRL_BOXED_MARGIN_TOP
else ; not _RUDY
...
if _RUDY
	;
	; Add in margins for shadows & focus leaves
	;
	...
	pop	ax
	jnc	noShadows
	add	cx, RUDY_SHADOW_THICKNESS
	add	dx, RUDY_SHADOW_THICKNESS
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		RudyHandleChildIndent
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Handles indenting children, if the hint exists.

CALLED BY:	CalcCtrlMargins

PASS:		*ds:si -- control
		ax -- left margin so far
		bp -- top margin so far

RETURN:		ax, bp -- possibly updated.
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	Chris	7/ 6/95       	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if _RUDY

RudyHandleChildIndent	proc	far		uses	cx, bx, di
	.enter
	clr	cx				;start with zero
	push	ax
	mov	di, ds:[si]			
	add	di, ds:[di].Vis_offset
	test	ds:[di].OLCI_rudyFlags, mask OLCRF_INDENT_CHILDREN
	jz	noIndent
	...
	test	ds:[di].OLCI_rudyFlags, mask OLCRF_DOING_SIZE_HINTS
	jnz	haveIndent			;if so, use cx=0
	mov	cx, CTRL_INDENT_LEFT_MARGIN	;default indent
haveIndent:
	tst 	ax
	jz	exit				;no hint, use default

	mov	bx, {word} ds:[bx]		;indent argument
	tst	bx
	jz	exit				;zero argument, use default

	jcxz	afterHintCustomChildSpacing	;doing size hints
...
EC <	test	cx, 1 shl (offset SSS_TYPE-1)				>
EC <	ERROR_NZ HINT_INDENT_TOO_LARGE					>

noIndent:
	;
	; GenItemGroup's built into popups aren't under the OLGadgetArea,
	; so we'll have to provide our own margins for the children,
	; and draw the moniker inward a bit.
	;
	mov	di, ds:[si]			
	add	di, ds:[di].Vis_offset
	test	ds:[di].OLCI_rudyFlags, mask OLCRF_USE_LARGE_FONT
	jz	exit
	add	cx, RUDY_POPUP_CTRL_LEFT_MARGIN	+ BUBBLE_LEFT_EXTRA_MARGIN
	add	bp, BUBBLE_TOP_EXTRA_MARGIN
						 
exit:
	pop	ax
	add	ax, cx				 ;add in the spacing
	.leave
	ret
RudyHandleChildIndent	endp
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		RudyCtrlExtraLeftMargin
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Returns the amount of extra space to leave in the left
		margin because the OLCtrl is either an
		OLCRF_SETTINGS_TRIGGER or has the OLCRF_DRAW_RIGHT_ARROW
		attribute.

CALLED BY:	CalcCtrlMargins
		OLCtrlGetLargestCenter

PASS:		ds:di	= pointer to OLCtrlClass instance data

RETURN:		bp	= extra space to leave in left margin
...
PSEUDO CODE/STRATEGY:
	Since the moniker width already contains the extra spacing
	MO_CONTROL_MKR_X_SPACING, we need to only add the amount of spacing
	above this default spacing.  If the amount of extra space is LESS
	than this default spacing, we don't do anything.

REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	JimG	6/27/95    	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if	 _RUDY

RudyCtrlExtraLeftMargin	proc	far
	.enter
    
	; Define local constants which just contain the width + extra offset
	; added for both the settings trigger and right mark.
	;
	_RUDY_ST_WIDTH = (RUDY_SETTINGS_TRIGGER_WIDTH + \
			  CTRL_SETTINGS_TRIGGER_X_OFFSET)
    
	_RUDY_RM_WIDTH = (RUDY_RIGHT_MARK_WIDTH + \
			  CTRL_RIGHT_MARK_X_OFFSET)

	CheckHack <_RUDY_ST_WIDTH eq _RUDY_RM_WIDTH>

	mov	bp, _RUDY_ST_WIDTH
	test	ds:[di].OLCI_rudyFlags, mask OLCRF_SETTINGS_TRIGGER or \
					mask OLCRF_DRAW_RIGHT_ARROW
	jnz	done

	; we need to shift the text edit field over a bit more to account
	; for popup list monikers being drawn with an inset value.

	add	bp, MO_BUTTON_INSET_X
	test	ds:[di].OLCI_rudyFlags, mask OLCRF_INDENT_TEXT_FIELD
	jnz	done
...
PMAN <	jnc	exit				; if not CGA, exit	>
PMAN <						; if CGA, compensate vert >
PMAN <	sub	bp, OL_REPLY_BAR_SPACE - OL_CGA_REPLY_BAR_SPACE		>
PMAN <	add	dx, OL_CGA_REPLY_BAR_BOTTOM_SPACE			>
...
if	 _RUDY
	;
	; For RUDY, subtract out the extra space in the margin.. that isn't
	; to be included in the "moniker space".  Since the extra space can
	; be different based on the type of object, we still want the
	; monikers to be "centered" on the right edge of the *text*, not the
	; (possible) bitmaps.
	;
	call	RudyCtrlExtraLeftMargin
	sub	ax, bp
...
if	 _RUDY
	;
	; If this interaction is "DRAW_IN_BOX", then use the new and
	; improved moniker offset.
	;
	mov	ds:[di].OLCI_monikerPos.P_y, RUDY_DRAW_IN_BOX_MKR_INSET_Y
	test	ds:[di].OLCI_optFlags, mask OLCOF_DISPLAY_BORDER
	jnz	yInsetSet
endif	;_RUDY
...
yInsetSet::					; RUDY conditional label
...
if _RUDY
	mov	ds:[di].OLCI_monikerPos.P_y, 0	;default: no moniker pos for 
						;   horizontal shme (conserve
						;   space)
endif
...
if _JEDIMOTIF
	;
	; should be in all SPUIs, but just for JEDI for now
	;
	; if we're a window, the moniker is in the title bar, which is
	; accounted for in the top margin returned above
	;	(cx = dx = 0 from above)
	;
	push	es, di
...
if _RUDY
	;
	; indicate that we are processing size hints (don't bother with
	; HINT_INDENT_CHILDREN)
	;
	mov	di, ds:[si]			;point to instance
	add	di, ds:[di].Vis_offset		;ds:[di] -- VisInstance
	ornf	ds:[di].OLCI_rudyFlags, mask OLCRF_DOING_SIZE_HINTS
endif
...
if _RUDY
	;
	; indicate that we are done processing size hints (don't bother with
	; HINT_INDENT_CHILDREN)
	;
	mov	di, ds:[si]			;point to instance
	add	di, ds:[di].Vis_offset		;ds:[di] -- VisInstance
	andnf	ds:[di].OLCI_rudyFlags, not mask OLCRF_DOING_SIZE_HINTS
endif
	pop	di
	call	ThreadReturnStackSpace
	ret
