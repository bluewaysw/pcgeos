...
DESCRIPTION:
	...
	$Id: copenContent.asm,v 1.82 97/02/13 19:19:47 cthomas Exp $
-------------------------------------------------------------------------------@
...
if _RUDY
 method VisSendToChildren, OLContentClass, MSG_SPEC_POPUP_LIST_APPLY
 method VisSendToChildren, OLContentClass, MSG_SPEC_POPUP_LIST_CANCEL

 ; Added 9/ 1/95 cbh, should be added for all products.
 method GenSendToChildren, OLContentClass, MSG_GEN_APPLY
 method GenSendToChildren, OLContentClass, MSG_GEN_RESET
 method GenSendToChildren, OLContentClass, MSG_GEN_PRE_APPLY
 method GenSendToChildren, OLContentClass, MSG_GEN_POST_APPLY
endif
...
navigate:
	...
	;
	; If we get back to the root, though, we need to accept responsibility
	; as something was focusable within us before, and now we have nothing
	; below us. For example, we might have a scrollable dynamic list that
	; is now empty (say, in the Document outbox after the last message has
	; been sent, speaking hypothetically, of course...). We accept the
	; focus initially when we have no children, so I see no reason not to
	; accept it again now we are childless once more -- ardeb 9/19/95
	;
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLContentGainedFocusExcl
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Intercepted to give the focus to a focusable child,
		when none already has the focus.
...
PSEUDO CODE/STRATEGY:
	...
	Seems like this should be in all UI's, unless there
	is some case in which automatically setting the focus
	within GenContent's is undesirable.  But those would
	be rare exceptions, and could be specified by some hint
	that turns this behavior off.  (Might the WWW app
	be one of these cases?)

	This code was taken from OpenWinGainedFocusExcl, which does
	this exact thing, at the window level.

	If our Focus hierarchical grab is NULL,
		give focus to first navigable object.

REVISION HISTORY:
	Name		Date		Description
	----		----		-----------
	cthomas 	2/13/97   	Initial version
	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLContentGainedFocusExcl	method dynamic OLContentClass, 
	...
	;
	; This message will make sure the focus is on something.
	;
...
OLContentChange	method dynamic	OLContentClass, \
				MSG_SPEC_CHANGE
	.enter
	mov	di, ds:[si]			
	add	di, ds:[di].Vis_offset
	movdw	bxsi, ds:[di].VCNI_focusExcl.FTVMC_OD
	tst	si
	jz	exit
	clr	di
	call	ObjMessage		;send on to focus
exit:
	.leave
	ret
OLContentChange	endm
