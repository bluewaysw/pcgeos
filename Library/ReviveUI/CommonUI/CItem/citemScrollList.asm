...
DESCRIPTION:
	...
	$Id: citemScrollList.asm,v 1.86 96/09/13 21:13:44 brianc Exp $
...
if _ODIE
	; No win frame around view if desired

	xchg	si, di				;*ds:si = view, *ds:di = list
	mov	ax, HINT_DRAW_IN_BOX
	call	ObjVarDeleteData		;remove from view
	xchg	si, di				;*ds:si = list, *ds:di = view
	mov	ax, HINT_DRAW_IN_BOX
	call	ObjVarFindData
	jc	keepFrame
	xchg	si, di				;*ds:si = view, *ds:di = list
	mov	ax, MSG_GEN_VIEW_SET_ATTRS
	mov	cx, mask GVA_NO_WIN_FRAME	;set no win frame flag
	clr	dx				;nothing to clear
	mov	bp, VUM_MANUAL
	call	ObjCallInstanceNoLock		;call view
	xchg	si, di				;*ds:si = list, *ds:di = view
keepFrame:
endif
...
if _RUDY
	;
	; Rudy, place moniker above for the view.
	;
...
if	 _RUDY
	; Force the newly created parent view to draw a bold moniker.
	;
	mov	di, ax
	mov	di, ds:[di]
	add	di, ds:[di].Vis_offset
	ORNF	ds:[di].OLCI_rudyFlags, mask OLCRF_FORCE_BOLD_MONIKER
 
 	mov	di, ax				; di = view, si = list
 	call	CopyComplexMonikerInfoIfNeeded
...
if _JEDIMOTIF
	;
	; This ain't pretty, but it works -- set double thick border flag
	; for view as JEDI scrolling lists have double border
	;
	xchg	di, si				;si = view, di = scrollList
	mov	ax, ATTR_OL_PANE_DOUBLE_BORDER
	mov	cx, 0
	call	ObjVarAddData
	xchg	di, si				;si = scrollList, di = view
endif
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if _RUDY
CopyComplexMonikerInfoIfNeeded	proc	near
	uses	ax, bx, cx
	.enter
EC <	call	VisCheckIfSpecBuilt					>
...
EC <	xchg	di, si							>
	;
	; If source is a ComplexMoniker, make note of that on the
	; view object.
	;
	xchg	si, di				; si = dest, di = source
	mov	bx, ds:[di]
	add	bx, ds:[bx].Vis_offset
	test	ds:[bx].OLCI_rudyFlags, mask OLCRF_IS_COMPLEX_MONIKER
	jz	done
	...
	mov	ds:[bx], di
	;
	; Set IS_COMPLEX_MONIKER on the destination, so it knows to
	; look for the vardata, even though it's not really a complex
	; moniker.
	;
	mov	bx, ds:[si]
	add	bx, ds:[bx].Vis_offset
	ornf	ds:[bx].OLCI_rudyFlags, mask OLCRF_IS_COMPLEX_MONIKER

done:
	xchg	si, di
	.leave
	ret
CopyComplexMonikerInfoIfNeeded	endp
endif ; _RUDY
...
if _RUDY
 if 0 ; do for all scrolling lists
      ; this doesn't work anyway as we are under a content -- brianc 2/11/96
	;
	; if in popup, expand width to fit
	;
	push	si
...
if _RUDY
	pop	dx				;*ds:dx <- list
	push	si				; save view
	mov	si, dx				;*ds:si <- list
	;
	; Give the Window our default focus, so that it can be
	; propigated to the activating button.
	;	*ds:si = *ds:dx = list
	;	*ds:di = popup
	;
	push	ax				; save build flags
	mov	ax, HINT_DEFAULT_FOCUS
	call	CopyScrollListHint
	;
	; Allow moniker clipping also, to control width of popup list
	; button.  This also requires the change in HandleGenericButtonHints.
	; -- brianc 2/11/96
	;
	...
	call 	SIB_DerefVisDI
	;
	; Force the View to use a bold moniker.  It's a spec thing.
	;
...
if _RUDY
	pop	ax				;restore popup lptr
	mov	ds:[di].OLIGI_popupInteraction, ax
endif
...
if _RUDY
CopyScrollListHint	proc	near
	;
	; *ds:si -- source, *ds:di -- dest.  cx = bytes of extra data
	; word of data to copy (either 2 or 0, currently)
	;
	push	dx
	call	ObjVarFindData			;look for hint in source
	jnc	exit				;not found, exit
	jcxz	10$
	mov	dx, {word} ds:[bx]
10$:
	xchg	si, di				;*ds:si = dest, *ds:di = src
	call	ObjVarAddData			;else add hint to parent
	xchg	si, di				;*ds:si = src, *ds:di = dest
	jcxz	exit
	mov	{word} ds:[bx], dx
exit:
	pop	dx
	ret
CopyScrollListHint	endp
endif
...
if _RUDY
	;
	; For scroll lists in Rudy with some size hint, we'll also enforce
	; the default width as the minimum width if there hasn't been a
	; previous minimum size hint -- brianc 2/11/96
	;	cx = init width
	;	al = ScrollListScanArgs
	;
	tst	desiredSize.SSA_minWidth
	jnz	haveMinWidth
	mov	desiredSize.SSA_minWidth, cx
haveMinWidth:
endif ; _RUDY
	jmp	short setView
...
if _RUDY
	;
	; If we set a minimum size about, we add a hint to the view.
	; We may also end up storing the same HINT_MINIMUM_SIZE as we
	; did in OLScrollListHandleSizeHints -- brianc 2/11/96
	;
	tst	desiredSize.SSA_minWidth	;did we set anything?
	jz	noMinWidthHint			;nope, don't bother
	mov	ax, MSG_GEN_SET_MINIMUM_SIZE	;what to call 
	mov	di, offset SSA_minWidth	
	call	CopySizeToViewIfSpecified
noMinWidthHint:
...
if _CUA_STYLE and not _JEDIMOTIF
	add	cx, MO_SCROLL_ITEM_INSET_X*2	; way...
	add	dx, MO_SCROLL_ITEM_INSET_Y*2
endif

JEDI <	add	cx, MO_SCROLL_ITEM_INSET_X	; way...		   >
JEDI <	add	dx, MO_SCROLL_ITEM_INSET_Y*2				   >

if	 _RUDY
	;
	; For Rudy, a file selector will need a little extra space because
	; the bitmaps are a little larger than the average bitmap.. :)
	;
	push	es
	segmov	es, <segment OLFSDynamicListClass>, di
	mov	di, offset OLFSDynamicListClass
	call	ObjIsObjectInClass
	jnc	notFileSelector
	
	add	dx, RUDY_FILE_SELECTOR_EXTRA_Y
notFileSelector:	
	pop	es
endif	;_RUDY
...
if DBCS_PCGEOS
  ...
  if _RUDY
    ...
    ScrollListKbdBindingsList	KeyboardShortcut \
		<1, 1, 1, 0, C_SYS_UP and mask KS_CHAR>,	;C-up
		<1, 1, 1, 0, C_SYS_DOWN and mask KS_CHAR>	;C-down
  ...
  if _RUDY
    ...
    ScrollListKbdBindingsList	KeyboardShortcut \
		<1, 1, 1, 0, 0xf, VC_UP>,	;C-up
		<1, 1, 1, 0, 0xf, VC_DOWN>	;C-down
...
if _RUDY
	;
	; If popup list, find the button that activates the
	; popup, and pass it directly there.
	;
	test	ds:[di].OLIGI_moreState, mask OLIGMS_POPUP_LIST
	jz	notPopup
