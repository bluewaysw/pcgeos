...
DESCRIPTION:
	$Id: citemScrollableItem.asm,v 1.45 97/02/18 20:46:11 reza Exp $
...
endif	; _OL_STYLE or _MOTIF or _PM ------------------------------------------
...
if _OL_STYLE or _MOTIF or _PM or _CUA or _MAC	;-------------------------------
...
if _RUDY
	;
	; In Rudy, we don't want to vertically center BW scrollable
	; items, because it messes up the vertical line drawing.
	;
...
endif	; _OL_STYLE or _MOTIF or _PM or _CUA or _MAC --------------------------
...
if _RUDY
	...
	;
	; being selected is forever, but cursoredness is fleeting,
	; so for those lists which are sensitive to losing the focus,
	; check the CURSORED attribute to determine whether to highlight
	; or not.
	;
	test	bh, mask OLBSS_CURSORED	;for other cases check cursored state.
	jz	notSelected
	jmp 	short selected
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		DrawSelectedItemMark
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Draw dot to signify that the item is selected.

CALLED BY:	DrawBWScrollableItemBackground
...
DrawSelectedItemMark	proc	near
		uses	ax,bx,cx,dx,si,di,bp,ds
		.enter
	;
	;  Determine the position at which to draw.
	;
		mov	ax, C_WHITE
		call	GrSetAreaColor
...
NOFXIP <	segmov	ds, cs, dx					>
	...
	;  Draw the mark.
	;
		mov	si, offset SelectedItemBitmap
		clr	dx			; no callback
		call	GrDrawBitmap
...
FXIP <		call	MemUnlock					>
		...
		.leave
		ret
DrawSelectedItemMark	endp

FXIP <  DrawBW	ends							>
FXIP <	DrawBWRegions	segment	resource				>

SelectedItemBitmap	label	byte
