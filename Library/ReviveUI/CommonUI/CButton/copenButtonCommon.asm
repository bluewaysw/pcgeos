...
DESCRIPTION:
	...
	$Id: copenButtonCommon.asm,v 1.37 97/04/02 21:35:48 brianc Exp $
...
if _RUDY
	;
	; If geometry not calculated yet, maybe now isn't the right
	; time to cause the thing to draw.  Seems like this
	; could be a problem on other platforms too. (see bug 54139 & 54448)
	;
	mov	di, ds:[si]
...
;MAC <	test	bp, (mask UIFA_SELECT) shl 8 	;are we dragging?	     >
;MAC <	jz	returnProcessed			;skip if not...		     >
...
if _ODIE
	; Odie menu buttons are always bordered so we only check depressed
...
if not _DUI	; DUI buttons never grab focus -- brianc 4/1/97 (no foolin')
if _CUA_STYLE	;------------------------------------------------------
	;MOTIF/PM/CUA:
...
OLButtonNavigate	method	OLButtonClass, MSG_SPEC_NAVIGATION_QUERY
	...
	call	OLButtonGetGenAndSpecState ;returns dh = VI_attrs
					   ;bx = OLBI_specState
if (not _KBD_NAVIGATION) or _DUI	; no focus for _DUI
...
RUDY <	test	ds:[di].OLBI_fixedAttrs, mask OLBFA_IN_REPLY_BAR 	>
RUDY <	jnz	menuRelated						>
...
if not _RUDY		;see comments in OLDialogWinHandleApplyable
...
if _RUDY
	;
	; nothing to draw for settings triggers
	;
...
if ( _OL_STYLE or _MOTIF or _PM ) and ( not _ASSUME_BW_ONLY )	;--------------
	jmp	short afterDraw
...
if _JEDIMOTIF
	;
	; hack: if HINT_MAXIMUM_SIZE, clip moniker
	;
...
if _JEDIMOTIF
	;
	; set DMF_UNDERLINE_ACCELERATOR if not menu bar button
	;
	; (we depend on DMF_UNDERLINE_ACCELERATOR only being clear for menu
	;  bar buttons as we use it to distinguish menu bar buttons from
	;  popup lists buttons (both have OLMA_DISP_DOWN_ARROW set))
	;
	ornf	ss:[bp].OMA_drawMonikerFlags, mask DMF_UNDERLINE_ACCELERATOR
	test	ds:[bx].OLBI_specState, mask OLBSS_IN_MENU_BAR
	jz	30$
	andnf	ss:[bp].OMA_drawMonikerFlags, not mask DMF_UNDERLINE_ACCELERATOR
30$:
endif
...
SYNOPSIS:	If this button activates a popup, dismiss the popup
		when we're closed, thus avoiding "child" dialogs
		that stay on screen when the parent has been
		dismissed.

CALLED BY:	MSG_MSG_VIS_CLOSE
PASS:		*ds:si	= OLButtonClass object
		ds:di	= OLButtonClass instance data
		ds:bx	= OLButtonClass object (same as *ds:si)
		es 	= segment of OLButtonClass
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLButtonVisClose	method dynamic OLButtonClass, 
					MSG_VIS_CLOSE
	mov	di, offset @CurClass
	call	ObjCallSuperNoLock

	;if this OLButtonClass object was fabricated to open a command window
	;or summons, then dismiss that window.
	...
	cmp	es:[di].Class_masterOffset, offset Vis_offset
					;is the specific part the highest-
					;level master class for this object?
	jne	done			;skip if not (has generic part)...
...
OLButtonChange	method dynamic	OLButtonClass, \
				MSG_SPEC_CHANGE

	mov	ax, ATTR_OL_BUTTON_OPENS_POPUP_LIST
	call	ObjVarFindData
	jnc	activateOurselves

	;
	; We're a popup list, send this there.
	;
	mov	di, ds:[si]			
	add	di, ds:[di].Vis_offset
...
EC <	ERROR_Z	OL_ERROR						>
	...
	;
	; This may be a view, in which case we have to look at the vis
	; child of its content.  Sigh.
	; 
	;
	push	es, di							
