...
DESCRIPTION:
	...
	$Id: copenButtonBuild.asm,v 1.30 96/10/23 23:04:52 brianc Exp $
...
if _RUDY
	ORNF	ds:[di].VI_geoAttrs, mask VGA_ONLY_RECALC_SIZE_WHEN_INVALID or\
				     ...
				     mask VGA_USE_VIS_SET_POSITION or \
				     mask VGA_NOTIFY_GEOMETRY_VALID
else
	ORNF	ds:[di].VI_geoAttrs, mask VGA_ONLY_RECALC_SIZE_WHEN_INVALID or\
				     mask VGA_USE_VIS_CENTER or \
...
if _JEDIMOTIF	; this may be needed for all products
	;
	; if in title bar, set in-toolbox as the we are under the toolbox
	; title bar group
	;
...
MAC <	ORNF	ds:[di].OLBI_specState, mask OLBSS_IN_MENU		>
...
if _RUDY
	;
	; In Rudy, menu buttons open popup lists.   This would be a better
	; solution in non-Rudy systems too, probably.
	;
	clr	cx
...
if _PM	; PM doesn't want the popup list's moniker centered -------------------
...
;this seems reasonable for PM also - brianc 1/20/93
if	(_MOTIF or _PM) and not _DETACHED_MENUS_DRAW_AS_NORMAL_BUTTONS
...
if _RUDY
	;
	; Rudy, only allow things that seek the menu bar to actually go
	; there.  (Also menuable things.  1/ 6/95 cbh) (removed
	; MENUABLE, because it's causing buttons for popups
	; to size & draw as if they were in the menu bar, even though
	; they aren't actually going there. 11/16/95 ct)
	;
	test	bp, mask OLBF_SEEK_MENU_BAR ; or mask OLBF_MENUABLE
	jz	checkForInMenu

elif _JEDIMOTIF
	;
	; JEDI: If explicity seeking menu bar, put in menu bar
	;
	test	bp, mask OLBF_SEEK_MENU_BAR
	jnz	inMenu
...
if (not _ODIE)	; menu buttons are bordered
MO   <	ANDNF	ds:[di].OLBI_specState, not mask OLBSS_BORDERED		>
endif

PMAN <	ANDNF	ds:[di].OLBI_specState, not mask OLBSS_BORDERED		>
OLS  <	ANDNF	ds:[di].OLBI_specState, not mask OLBSS_BORDERED		>
MAC  <	ANDNF	ds:[di].OLBI_specState, not mask OLBSS_BORDERED		>
...
if _RUDY
	;
	; if no moniker and HINT_TRIGGER_BRINGS_UP_WINDOW, mark this as a
	; settings trigger and notify parent GenInteraction
	;
	push	es
	mov	di, segment ComplexMonikerClass
	mov	es, di
	mov	di, offset ComplexMonikerClass
	call	ObjIsObjectInClass
	pop	es
	jc	notSettingsTrigger
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	test	ds:[di].VI_typeFlags, mask VTF_IS_GEN
	...
	jnz	notSettingsTrigger
	mov	ax, HINT_TRIGGER_BRINGS_UP_WINDOW
	call	ObjVarFindData
	jnc	notSettingsTrigger
	mov	ax, ATTR_OL_BUTTON_SETTINGS_TRIGGER
	clr	cx
	call	ObjVarAddData
	mov	ax, MSG_SPEC_NOTIFY_SETTINGS_TRIGGER
	call	GenCallParent
notSettingsTrigger:
endif
...
if _RUDY
	;
	; Most triggers will want arrows to point at them,
	; But not GenItem's.  The ancestor OLCtrl's probably won't
	; give them arrows anyway, but we'll prune the request here
	; to save time.
	;
	push	es
...
if _JEDIMOTIF	; sys icons are not bordered
	jz	maybeInMenu
	; actually, only the close button doesn't have a border
...
elseif (_ODIE and DRAW_STYLES)
	...
	mov	ax, HINT_CLOSE_BUTTON
	call	ObjVarFindData
	jc	notInMenu		; close button, leave raised, bordered
...
buildLegosButtonLookHintTable	label word
	word	0
if _PCV
	word	HINT_TRIGGER_COMMAND_STYLE_BUTTON
	word	HINT_TRIGGER_BLANK_STYLE_BUTTON
	word	HINT_TRIGGER_TOOL_STYLE_BUTTON
	word	HINT_TRIGGER_WINDOW_CONTROL_BUTTON
	word	HINT_TRIGGER_EDGE_STYLE_UPPER_RIGHT
	word	HINT_TRIGGER_EDGE_STYLE_LOWER_RIGHT
endif
...
OLButtonHintHandlers2	VarDataHandler \
	...
	<HINT_CAN_CLIP_MONIKER_WIDTH,
		offset OLButtonClipMonikerWidth>
else
OLButtonHintHandlers2	VarDataHandler \
	<HINT_TRIGGER_DESTRUCTIVE_ACTION, \
		offset Build:OLButtonSetIsDestructiveAction>,
...
if _RUDY
OLButtonClipMonikerWidth	proc	far
	class	OLButtonClass

	mov	di, cx				; ds:di = OLBI instance
	;
	; set clip flag, set expand flag so we expand or shrink as needed
	;
	ornf	ds:[di].OLBI_moreAttrs, mask OLBMA_CAN_CLIP_MONIKER_WIDTH or \
				mask OLBMA_EXPAND_WIDTH_TO_FIT_PARENT
	;
	; also need to get ourself to recalc as above attr is handled on
	; RECALC_SIZE -- brianc 2/12/96
	;
	andnf	ds:[di].VI_geoAttrs, not mask VGA_ONLY_RECALC_SIZE_WHEN_INVALID
	ret
OLButtonClipMonikerWidth	endp
endif
...
if _JEDIMOTIF
	;
	; Don't do this for App menu button either.  The genChunk is the
	; App Menu whose parent is the GenApp.
	;
...
if	ERROR_CHECK
	...
	call	ObjIsObjectInClass
	ERROR_NC	OL_ERROR	; If not, shit bricks :)
	pop	es
endif
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
...
if (_ODIE and DRAW_STYLES)
	;
	; for dialogs buttons in title bar:
	; sys icons are not bordered, though close button is bordered
	; sys icons are flat, though close button is raised
	;
	mov	di, ds:[si]
	...
	test	ds:[di].OLBI_specState, mask OLBSS_SYS_ICON
	jz	notSysIcon
;close button never gets here
;	mov	ax, HINT_CLOSE_BUTTON
;	call	ObjVarFindData
;	jc	notSysIcon		; close button, leave raised, bordered
					; sys icons, no bordered and flat
	andnf	ds:[di].OLBI_specState, not mask OLBSS_BORDERED
	mov	ds:[di].OLBI_drawStyle, DS_FLAT
notSysIcon:
