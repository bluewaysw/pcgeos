...
DESCRIPTION:
	...
	$Id: cviewPaneWindow.asm,v 1.73 96/12/27 21:09:04 brianc Exp $
...
if not _JEDIMOTIF
	...
	;
	; But: The JEDI jotter icon drag and drop requires that
	; potential destination objects grab the mouse.  And being
	; a pen based system, the MOVE_COPY messages are never generated,
	; so we must allow mouse grabs during the SELECT UIFunction.
	; - chrisT 9/13/94
	;
	test	bp, (mask UIFA_MOVE_COPY shl 8) or \
		    (mask UIFA_SELECT shl 8)
	jz	afterWanderDetect

		; But this test alone causes the grab to be taken away
		; from gadgets when the pen is dragged into a view, causing
		; popup menus to disappear.  So we'll limit a grab to
		; only the case in which a jotter drag and drop is
		; taking place (which uses the quick-transfer mechanism)
...
if	_MOTIF or _PM or _CUA or _MAC
...
if _JEDIMOTIF
	;
	; support double width border
	;
	push	di				;save gstate
	call	GetFrameWidth			;ax = width
	pop	di				;di = gstate
	cmp	ax, 1
	jbe	singleWidth			;single width frame
	call	GetFrameBounds			;get bounds
	inc	bx				;round corners
	dec	dx
	call	GrDrawVLine			;left line
	xchg	ax, cx				;do right side
	call	GrDrawVLine			;right line
	dec	bx				;restore corners for HLine
	inc	dx
	xchg	ax, cx				;restore left, right
	inc	ax				;round corners
	dec	cx
	call	GrDrawHLine			;top line
	xchg	bx, dx				;do bottom side
	call	GrDrawHLine			;bottom line
	xchg	bx, dx				;restore top, bottom
	inc	cx				;restore corners for inside
;cancels out
;	dec	ax
;	inc	ax				;inset and make fillRect-style
	inc	bx
	call	OpenDrawRect			;draw an inner rect
	jmp	short doneFrame			;all done
...
DrawColorPaneWinFrame	proc	far
	...
	mov	ax, cx				;move color scheme into ax
	and	ax, mask CS_darkColor		;use dark color
	call	GrSetLineColor
	mov	al, cl				
	mov	cl, 4
	shr	al, cl
...
if _JEDIMOTIF
	jg	10$
	jl	4$
	; if equal, check if at edge of screen, if so, scroll
	tst	cx
	jnz	10$				;not at left edge
4$:
else
...
if _JEDIMOTIF
	...
	jg	13$
	; if equal, check if at edge of screen, if so, scroll
	cmp	bx, 239				;no constant!?!
	jne	checkVert			;not at right edge
13$:
else
...
if _JEDIMOTIF
	...
	jg	24$
	; if equal, check if at edge of screen, if so, scroll
	cmp	dx, 239				;no constant!?!
	jne	scroll				;not at bottom edge
24$:
else
