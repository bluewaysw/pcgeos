...
DESCRIPTION:
	...
	$Id: copenAppAttDet.asm,v 1.10 96/05/01 03:18:06 joon Exp $
...
if _JEDIMOTIF
	call	RemoveAppMenuAndPrintScreen
endif
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if	_RUDY
SendFoamNotification	proc	far	uses	ax, cx, dx, bp, di
	.enter
	...
	mov	ax, GAGCNLT_FOAM_NOTIFICATIONS
	push	ax				; GCNLT_type
...
if 0	; now that indicator is a lot smarter..
	push	ds, si, di, es, ax		; required, from documentation
	mov	ax, MSG_GEN_LOWER_TO_BOTTOM
	call	ObjCallInstanceNoLock
	pop	ds, si, di, es, ax
endif	; 0
...
EC <	Assert	segment, es						>
	...
	;
	; Send out the foam auto-save notification since we want applications
	; to auto-save when they are lowered to bottom.
	;
	mov	dx, GWNT_FOAM_AUTO_SAVE
	call	SendFoamNotification

	; 
	; Let the app know it's losing the full screen excl
	;
	mov	dx, GWNT_FOAM_LOST_FULL_SCREEN_EXCL
	call	SendFoamNotification

;
;	We only want the top app to save its data on a suspend, so take apps
;	off the list when they lose the full-screen excl. In any case,
;	they should have already saved their data, anyhow...
;
