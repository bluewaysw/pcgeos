...
DESCRIPTION:
	...
	$Id: copenFileSelectorMiddle.asm,v 1.42 97/01/15 20:55:04 brianc Exp $
...
if _RUDY
	; If it's non-exclusive, don't bother copying any bogus
	; single selection back to gen data - the list of selections
	; are already there.
	call	OLFSIsNonExclusive		; carry if non-ex
	jc	done	
endif
...
if _FILE_TABLE
  ...
  if _RUDY
	pop	bp				; We need file selector
	pop	dx				; chunk in dx
	push	dx				; for the rudy version
	push	bp
  endif ; _RUDY
...
ifdef WIZARDBA
	call	IclasDriveGetStatus		; is drive present and allowed?
else
...
ifdef WIZARDBA
FileExtAttrDesc	<FEA_DESKTOP_INFO, offset OLFSE_desktopInfo, size OLFSE_desktopInfo>
endif
...
elif SORT_FILE_SELECTOR_ENTRIES_BY_MODIFICATION_DATE or _JEDIMOTIF
...
if _JEDIMOTIF
FileExtAttrDesc <FEA_SIZE, offset OLFSE_fileSize, size OLFSE_fileSize>
endif
...
if _RUDY
    OLFSEP_pathIsDocument byte			;non-zero if in SP_DOCUMENT
endif
	even
...
if _RUDY
	...
	sub	sp, cx
	mov	si, sp				;ds:si = buffer for path
	call	FileGetCurrentPath
	cmp	bx, SP_DOCUMENT
	mov	cl, 0				;assume not document
	jne	setIsDocument
	cmp	{byte} ds:[si], 0
	jne	setIsDocument
	mov	cl, 1
setIsDocument:
	mov	ss:[bp].OLFSEP_pathIsDocument, cl
	add	sp, FILE_LONGNAME_LENGTH
...
ifdef WIZARDBA
	;
	; IclasCheckSysFile is a function called from the FileEnum callback
	; function.  We need to make sure the function is resident in memory
	; before starting the FileEnum.  We also need to lock down
	; LocalCmpStrings which is called by IclasCheckSysFile.
	;
	push	ax, bx
	mov	bx, vseg IclasCheckSysFile
	mov	ax, offset IclasCheckSysFile
	call	MemLockFixedOrMovable
	mov	bx, vseg LocalCmpStrings
	mov	ax, offset LocalCmpStrings
	call	MemLockFixedOrMovable
	pop	ax, bx
endif
if HAVE_FAKE_FILE_SYSTEM
...
ifdef WIZARDBA
	;
	; We can now unlock the block containing IclasCheckSysFile and ...
	;
	push	ax, bx
	mov	bx, vseg LocalCmpStrings
	mov	ax, offset LocalCmpStrings
...
AddNoFilesIfNeeded	proc	near	uses	ax, es, di, si
	.enter
	;
	; In Rudy, we'll supply a "no files" moniker if there aren't
	; any.   This will create space for the moniker.
	;
	call	OLFSDeref_SI_Spec_DI
	and	ds:[di].OLFSI_state, not mask OLFSS_NO_FILES
	tst	cx
	jnz	exit
	...
	or	ds:[di].OLFSI_state, mask OLFSS_NO_FILES

EC <	tst	bx				; no block yet!		>
EC <	ERROR_NZ	OL_ERROR					>

	mov	ax, size OLFileSelectorEntry
	mov	cx, (mask HAF_LOCK shl 8) or (mask HAF_ZERO_INIT shl 8) or \
		     mask HF_SWAPABLE or mask HF_SHARABLE
	call	MemAlloc
	jc	memError			; no memory, give up
	...
	;
	clr	di
	segmov	es, ds					;es:di is our dest

	mov	al, mask FA_HIDDEN		;OLFSE_fileAttrs
	stosb
	clr	ax				;OLFSE_fileFlags
	stosw

	mov	bx, handle NoFilesMoniker
	call	ObjLockObjBlock
	mov	ds, ax
	mov	si, offset NoFilesMoniker
	mov	si, ds:[si]			;deref chunk
	call	storeFileNameAttrs		;store the filename
	segmov	ds, es				;restore ds <- file list block
	call	MemUnlock			;unlock moniker block

	pop	bx				; restore list block
	call	MemUnlock			; unlock list block
	mov	di, bx				; keep our list block here

	pop	cx
errRet:
	pop	bx				; now get ds <- fsel block
	call	MemDerefDS
	mov	bx, di
exit:	
	.leave
...
memError:
	pop	ax				; throw away file count
	clr	cx, di				; indicate no files, no buffer
	jmp	short errRet

storeFileNameAttrs	label	near
	;
	; ds:si - source name text
	; es:di - place to store
	; destroys ax
	;
	mov	cx, size OLFSE_name
	rep	movsb
	;
	; remaining fields have been zero-init'ed
	;
	retn

AddNoFilesIfNeeded	endp
...
ifdef WIZARDBA	;--------------------------------------------------------------
	;
	; For Wizard, we want to filter out certain links
	;
...
EC <		ERROR_C	OLFS_FILE_ATTR_MISSING_FROM_CALLBACK_DATA >
		...
		cmp	di, WOT_PRINTER
		je	reject
		cmp	di, WOT_WASTEBASKET
		je	reject
		cmp	di, WOT_DESKTOP
		je	reject
		cmp	di, WOT_PEOPLE_LIST
		je	reject
		cmp	di, WOT_SPECIALS_LIST
		je	reject
		cmp	di, WOT_OFFICEAPP_LIST
		je	reject
		cmp	di, WOT_COURSEWARE_LIST
		je	reject
		cmp	di, WOT_GEOS_COURSEWARE
		je	reject
		cmp	di, WOT_DOS_COURSEWARE
		jne	notRejected
reject:
		stc				;reject this file
		jmp	done
notRejected:
	;
	; Now check to see if this is a Wizard system file which should not
...
EC <		ERROR_C	OLFS_FILE_ATTR_MISSING_FROM_CALLBACK_DATA >
		mov	di, es:[di].FEAD_value.offset
		call	IclasCheckSysFile
		LONG jc	done			;reject this file
endif		;--------------------------------------------------------------
...
if _RUDY
	;
	; If file is in SP_DOCUMENT, filter out instead of disabling.
	; How odd, isn't it.
	;
		tst	ss:[params].OLFSEP_pathIsDocument
		stc				;assume isDocument
		jnz	haveResult		;
endif
...
if _RUDY
	...
	mov	si, offset OLFileSelectorChangeDirectoryPopup
if _RUDY
	; In this routine we don't care about the file selector (maybe?).
	; This routine is only called from the drive-selection
	; code anyway, which I don't think is used on Responder.
	clr	dx
endif ; _RUDY
	call	OLFSSetGenItemSelection
	.leave
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLFileSelectorSearch
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	builds the search filter for the file list

CALLED BY:	MSG_OL_FILE_SELECTOR_SEARCH
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	cthomas	12/04/96   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLFileSelectorSearch	method dynamic OLFileSelectorClass, 
					MSG_OL_FILE_SELECTOR_SEARCH

key		local	FileLongName		; search key
keyLength	local	word			; length of key string
files		local	hptr.OLFileSelectorEntry ; file buffer
numFiles	local	word
filterBlock	local	hptr.word
	...
	mov	ax, ds:[di].OLFSI_fileBuffer
	mov	ss:[files], ax
	mov	ax, ds:[di].OLFSI_numFiles
	mov	ss:[numFiles], ax

	; Get the search key from our search text object
	...
	mov	di, mask MF_CALL or mask MF_FIXUP_DS
	call	ObjMessage			; *ds:cx = chunk
						;  ax = length
	pop	si, bp				; *ds:si = file selector

	mov	ss:[keyLength], ax

	; If no search key, clear the search filter.

	tst	ax
	jz	setFilter

	;
	; Must search our file list for matching files
	;

	; Create a buffer big enough to hold the largest filter

	mov	ax, ss:[numFiles]
	shl	ax, 1
	inc	ax				; (in case no files)
	mov	cx, ALLOC_STATIC_LOCK
	call	MemAlloc			; ^hbx = ax = buffer
	jnc	haveBuf				; can't alloc: pretend
	clr	ss:[keyLength]			;  there is no filter
	jmp	setFilter

haveBuf:
	; At this point, we're doing a search.
	; If no files, or no file buffer, automatically use empty filter

	mov	ss:[filterBlock], bx
	mov	es, ax
	clr	di				; es:di = filter position
	mov	cx, ss:[numFiles]
	jcxz	setFilter
	mov	bx, ss:[files]
	tst	bx			; I guess sometimes fileBuffer can
	jz	setFilter		; be NULL even when numFiles <> 0

	; Lock OLFSI_fileBuffer for OLFileSelectorMatchFile's use,
	; then loop through all the entries, depositing matching
	; entries in the filter.

	push	bx
	call	MemLock
	clr	ax				; ax = current entry #
searchLoop:
	call	OLFileSelectorMatchFile
	jnc	next
	stosw
next:
	inc	ax
...
setFilter:
	; Use the resulting filter (or none, if searchLength == 0)
	; to initialize the list.

	push	bp				; save locals

	shr	di, 1
	mov	cx, di				; cx = # of filter entries

	call	OLFSDeref_SI_Spec_DI		; ds:di = spec instance
	mov	ax, ss:[keyLength]
	mov	ds:[di].OLFSI_searchLength, ax
	mov	dx, es
	clr	bp				; dx:bp = filter
	tst	ax
	jnz	useFilter
	clr	dx				; null filter if null
	mov	cx, ds:[di].OLFSI_numFiles	;   search key
useFilter:
	...
	; We're changing the list contents, which may move the selection,
	; so it needs to send out apply that our selection be updated.
	...
	; Free the filter block, if there was one.

	mov	bx, ss:[filterBlock]
	tst	bx
	jz	done
	call	MemFree
done:
	.leave
	ret
OLFileSelectorSearch	endm


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLFileSelectorMatchFile
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Attempts to match a file entry against a key

CALLED BY:	OLFileSelectorSearch
PASS:		*ds:si	= OLFileSelector
		frame of OLFileSelectorSearch
		ax	= file entry # to match
		OLFSI_fileBuf locked
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLFileSelectorMatchFile	proc	near
	
	uses	ax,bx,cx,si,di,ds,es
	.enter	inherit	OLFileSelectorSearch

	mov	cx, ax
	call	OLFSDeref_SI_Spec_DI		; ds:di = spec instance
	call	OLFSDerefIndexBuffer		; si = offset in file buffer

	mov	cx, ss:[keyLength]		; cx = key length
	mov	bx, ss:[files]
	call	MemDerefES
	lea	di, es:[si].OLFSE_name		; es:di = file name
	segmov	ds, ss, si
	lea	si, ss:[key]			; ds:si = key
...
if _RUDY
	; Set up the # of characters to underline to show matching
	; characters.  If there are no files, don't underline anythinng.

	test	ax, mask OLFSS_NO_FILES
	mov	ax, ds:[di].OLFSI_searchLength
	jz	setLength
	clr	ax
setLength:
	mov	matchLength, ax
...
if _RUDY ;--------------------------------------------------------------------
	...
	;
	; If the file is GFT_EXECUTABLE then we display the icon.
	;	di = gstate
	;	
	cmp	fileEntry.OLFSE_fileType, GFT_EXECUTABLE
	je	showIcon
		
	;
	; If GFHF_UNREAD clear, don't display icon
	;	di = gstate
	;
	test	fileEntry.OLFSE_fileFlags, mask GFHF_UNREAD
	jnz	showIcon
	mov	dx, GFS_NARROW_ICON_WIDTH
	test	olfsState, mask OLFSS_SHOW_PARENT_DIR
	jz	haveViewedWidth			; nope, use narrow icon spacing
	mov	dx, GFS_WIDE_ICON_WIDTH		; else, use wide icon spacing

haveViewedWidth:
	add	dx, GFS_ICON_SPACING		; dx = offset to name space
	push	ds				; put fs segment on stack
	jmp	afterBitmap			; (must have DS on stack)

showIcon:
	;
	; lookup icon in token.db and draw into gstring
	;	di = gstate
	;
	mov	ax, {word}fileEntry.OLFSE_token.GT_chars[0]
	mov	bx, {word}fileEntry.OLFSE_token.GT_chars[2]
	mov	si, fileEntry.OLFSE_token.GT_manufID
	mov	dh, DisplayType<DS_TINY, DAR_NORMAL, DC_GRAY_4>
	push	bp
	mov	bp, (VMS_TOOL shl offset VMSF_STYLE) or mask VMSF_GSTRING
	call	TokenLookupMoniker		; cx:dx = group/item, ax = file
	pop	bp
	jc	useDefault			; not found
	push	ds				; save fs segment
	call	TokenLockTokenMoniker		; *ds:bx = moniker
	mov	si, ds:[bx]			; ds:si = moniker
	test	ds:[si].VM_type, mask VMT_GSTRING
	jz	notIcon
	push	ds:[si].VM_width		; save width
...
notIcon:
	...
	popf
	jnz	afterBitmap			; icon found (with DS on stack)
	pop	ds				; restore fs segment
				; else, fall though to use default icon
...
if _RUDY
	;	
	; No token was found in the token db for this executable so we
	; use a default one.
	;
	mov	si, offset UnknownExecutableBitmap	
	cmp	fileEntry.OLFSE_fileType, GFT_EXECUTABLE
	je	gotIcon
	mov	si, offset UnknownUnreadDocBitmap
gotIcon:
else		
...
if	 _RUDY
	; We are going to have 4-bit grey bitmaps here, so we need to use
	; GrDrawBitmap.
	call	GrDrawBitmapAtCP		; draw into chunk
else	;_RUDY is FALSE
...
if	 _RUDY
	; Add in extra spacing for the text part of the moniker.
	
	add	dx, RUDY_FILE_SELECTOR_TEXT_X_OFFSET
	mov	bx, RUDY_FILE_SELECTOR_TEXT_Y_OFFSET
	clr	ax
else	;_RUDY is FALSE
	clrdw	bxax
endif	;_RUDY
...
if _RUDY
	test	fileEntry.OLFSE_fileAttrs, mask FA_HIDDEN
	jnz	disable				; hack to avoid selection
endif
	test	ds:[di].GFSI_attrs, mask FSA_SHOW_FILES_DISABLED
	jz	haveItemFlags
...
NRUDY <	mov	ax, MSG_GEN_DYNAMIC_LIST_REPLACE_ITEM_MONIKER		>
RUDY  <	mov	ax, MSG_FILTER_LIST_REPLACE_ITEM_MONIKER		>
...
if _RUDY
drawUnderlinedName	label	near
	lea	si, ss:[fileEntry].OLFSE_name	; ds:si = filename
	mov	cx, ss:[matchLength]
	jcxz	nonUnderlined

	; Draw underlined prefix of name

	push	ax
	mov	ax, (mask TS_UNDERLINE or mask TS_BOLD)
	call	GrSetTextStyle
	call	GrDrawTextAtCP
	mov	ax, (mask TS_UNDERLINE or mask TS_BOLD) shl 8
	call	GrSetTextStyle
	pop	ax
nonUnderlined:
	;
	; Draw non-underlined remainder of string
	;
	add	si, cx
	clr	cx				; null-terminated
	call	GrDrawTextAtCP
...
if ((not _JEDIMOTIF) and (not _DUI))
...
folderIconBitmap	label	byte
	Bitmap <GFS_WIDE_ICON_WIDTH, GFS_ICON_HEIGHT, 0, BMF_MONO>
	byte	00000000b, 00000000b, 00000000b, 00000000b
        byte    00000001b, 00000000b, 01111110b, 00000000b
        byte    00000000b, 00000000b, 10000001b, 11111110b
	byte    00000001b, 00000000b, 10111110b, 00000001b
	byte    00000000b, 00000000b, 10000000b, 00000001b
	byte    00000001b, 00000000b, 10000000b, 00000001b
	byte    00000000b, 00000000b, 10000000b, 00000001b
	byte    00000001b, 01010100b, 10000000b, 00000001b
	byte    00000000b, 00000000b, 10000000b, 00000001b
	byte	00000000b, 00000000b, 10000000b, 00000001b
	byte    00000000b, 00000000b, 10000000b, 00000001b
	byte    00000000b, 00000000b, 01111111b, 11111110b
endif

if _RUDY

include Art/mkrUnknownExecutable.def
include Art/mkrUnknownUnreadDoc.def

elif ((not _JEDIMOTIF) and (not _DUI))
...
if	 _RUDY
  ; This bad boy better have width of GFS_NARROW_ICON_WIDTH
  ;
include Art/mkrnarrowFolderIcon.def
...
narrowFolderIconBitmap	label	byte
	...
	byte    10000001b, 11111110b
	byte    10111110b, 00000001b
...
openFolderIconBitmap	label	byte
	...
	byte    01000001b, 11111111b
	byte    01000000b, 00000001b
	byte    01000011b, 11100001b
	byte    01111100b, 00010001b
	byte    10000000b, 00001001b
	byte    10000000b, 00001001b
	byte    10000000b, 00000101b
	byte    01000000b, 00000101b
	byte    01000000b, 00000011b
	byte    00111111b, 11111110b
...
ifdef WIZARDBA	;--------------------------------------------------------------
	;
	; Check to see if we are using Wizard's virtual root
	;
	test	ds:[di].OLFSI_state, mask OLFSS_USING_WIZARD_VIRTUAL_ROOT
	jz	noVirtualRoot

	mov	ax, SGIT_SYSTEM_DISK
	call	SysGetInfo		; get disk handle of system disk
	cmp	ax, bx			; did we change to the system disk
	jne	noVirtualRoot		; if not, then no wizard virtual root

	mov	cx, cs
	mov	dx, offset desktopDirectory
	mov	bp, SP_TOP
	jmp	short setPath
noVirtualRoot:
endif		;--------------------------------------------------------------
...
EC <	ERROR_C	OL_NON_EXCLUSIVE_FILE_SELECTOR_UNSUPPORTED_OPERATION	>
	jc	done
endif
...
OLFileSelectorGrabFocusExcl	method dynamic	OLFileSelectorClass, \
				MSG_META_GRAB_FOCUS_EXCL

	;
	; If the dynamic list for the file selector exists, pass the grab
	; down to it.
	;
	mov	bx, ds:[di].OLFSI_uiBlock
	tst	bx
...
callSuper:
	;
	; Otherwise call our superclass.
	;
	mov	di, offset OLFileSelectorClass
	GOTO	ObjCallSuperNoLock

OLFileSelectorGrabFocusExcl	endm
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLFSGenFileSelectorGetSelectionToken
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Returns the GeodeToken of the current file.  If the file
		is a DOS file the DOS extension is returned (including the
		dot).

CALLED BY:	MSG_GEN_FILE_SELECTOR_GET_SELECTION_TOKEN
PASS:		*ds:si	= OLFileSelectorClass object
		ds:di	= OLFileSelectorClass instance data
		ds:bx	= OLFileSelectorClass object (same as *ds:si)
		es 	= segment of OLFileSelectorClass
		ax	= message #
		cx:dx	= Address of GeodeToken buffer
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	SH	5/30/96   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLFileSelectorGetSelectionToken	method dynamic OLFileSelectorClass, 
				MSG_GEN_FILE_SELECTOR_GET_SELECTION_TOKEN
bufferPtr	local	fptr	push	cx,dx
		uses	ax, cx, dx, bp
		.enter
if _FXIP
	;
...
EC <		popdw	bxsi						>
	...
	;
		...
		je	done				; no selection, done
if _RUDY
		cmp	cx, OLFS_NO_MATCH_ENTRY_NUM
		je	done
endif
	;
	; Lock file buffer to get at the OLFileSelectorEntry, copy the
	; GeodeToken from the entry into the passed buffer.
	;
		call	OLFSDeref_SI_Spec_DI		; ds:di = spec instance
		call	OLFSDerefIndexBuffer	; si = offset in file buffer
		mov	bx, ds:[di].OLFSI_fileBuffer	; lock file buffer
		...
		mov	ds, ax				; ds:si = entry to get
	;
	; Check to see if the selected file is a GEOS file or not.
	;
		cmp	ds:[si].OLFSE_fileType, GFT_NOT_GEOS_FILE
		je	notGeosFile
		add	si, offset OLFSE_token
		movdw	esdi, ss:[bufferPtr]
...
notGeosFile:
	;
	; For RUDY we return the DOS extension if the file is not a GEOS file.
	;
		segmov	es, ds			; es:di <- name of file
		mov	di, si
		add	di, offset OLFSE_name
		push	di
		...
		pop	di
	;
	; Locate the "." character in the dos file name.  If it's not found
	; then the file doesn't have an extension.
	;
		mov	ax, C_PERIOD
		LocalFindChar			; es:di <- extenstion
		jne	noExtension		; jmp if no extension
	;
	; Copy the DOS extension to the buffer passed in.
	;
		dec	di			; points to "."
		segmov	ds, es			; ds:si <- source
		mov	si, di
		movdw	esdi, ss:[bufferPtr]	; es:di <- dest
		mov	cx, TOKEN_CHARS_LENGTH	; 4 characters
		LocalCopyNString SAVE_REGS
		mov	es:[di].GT_manufID, -1
		jmp	unlockBuffer
noExtension:
	;
	; The DOS file has no extension so we return the token as four spaces.
	;
		movdw	esdi, ss:[bufferPtr]
		segmov	ds, cs
		mov	si, offset cs:[dummyToken]
		copybuf <size TokenChars> SAVE_REGS
		mov	es:[di].GT_manufID, -1		
		jmp	unlockBuffer

OLFileSelectorGetSelectionToken	endm
