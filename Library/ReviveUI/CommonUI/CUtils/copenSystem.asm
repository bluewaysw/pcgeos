...
DESCRIPTION:
	...
	$Id: copenSystem.asm,v 2.72 96/08/28 18:14:20 brianc Exp $
...
if _JEDIMOTIF
	; Look for a calibration window, which has a layer priority of
	; MODAL-1.  If it exists, then don't allow changing the
	; flow.  This prevents sys-modal dialogs from asynchronously coming
	; up behind the calibration window and taking away input.
	;
	mov	cx, ((LAYER_PRIO_MODAL-1) shl offset WPD_LAYER) \
			or WIN_PRIO_MODAL
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
if _JEDIMOTIF
OLSystemFupKbdChar	method dynamic OLSystemClass, 
					MSG_META_FUP_KBD_CHAR
	test	dl, mask CF_FIRST_PRESS or mask CF_REPEAT_PRESS
	jz	done			; not press, just eat it silently
	test	dl, mask CF_STATE_KEY
	jnz	done			; state key, just eat it silently
	mov	ax, SST_NO_INPUT
	call	UserStandardSound
done:
	stc				; eat it
	ret
OLSystemFupKbdChar	endm
endif
