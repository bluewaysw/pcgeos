...
DESCRIPTION:
	...
	$Id: cwinClassCommonHigh.asm,v 1.24 97/02/24 11:26:36 grisco Exp $
...
if _REDMOTIF 
	call	HintWindowNoSysMenu
endif 

if _RUDY
	call	HintWindowNoTitleBar
endif
	ret
...
if _GCM
HintGeneralConsumerMode	proc	far
	class	OLWinClass
;NOTE: ERROR CHECK: test for OLBaseWinClass.
	mov	al, ds:[bx]		;al = GeneralConsumerModeFlags

	call	WinCommon_DerefVisSpec_DI
	ORNF	ds:[di].OLWI_fixedAttr, mask OWFA_GCM_TITLED
					;set flag for OLWinClass

	.warn	-private

	mov	ds:[di].OLBWI_gcmFlags, al

	.warn	@private

					;and save info on which icons to use
CUAS <	ANDNF	ds:[di].OLWI_attrs, not (mask OWA_HAS_SYS_MENU)		>
	ret
HintGeneralConsumerMode	endp
endif	; _GCM
...
if _RUDY
	...
	je	exit			;do nothing on Rudy base windows!
endif
	mov	cx, mask WPSF_SIZE_TYPE
...
PMAN <	je	findTitle						     >
PMAN <	cmp	ds:[di].OLWI_type, MOWT_DISPLAY_WINDOW			     >
PMAN <  jne	done							     >
PMAN <findTitle:							     >
PMAN <	call	OLMenuedWinFindTitleMonikerFar				     >
...
if (not _JEDIMOTIF)	; no need for this in JEDI
	call	CheckIfKeyboardRequired
...
if _RUDY
	;
	; Force margins in Rudy to be small, no harm done.
	; (Temporarily faked for demo 7/26/94 cbh)
	;
	tst	bp
	jz	afterForceMargins
	mov	bp, (RUDY_POPUP_RIGHT_INSET shl 8) or \
		     RUDY_POPUP_BOTTOM_INSET 
afterForceMargins:
...
if _JEDIMOTIF
	;
	; hack for JEDI to get correct size primaries
	;
	cmp	ds:[di].OLWI_type, MOWT_PRIMARY_WINDOW
	je	setFullSize
...
if _JEDIMOTIF
	;
	; outset only for Primaries and Displays
	;
	call	WinCommon_DerefVisSpec_DI
	cmp	ds:[di].OLWI_type, MOWT_PRIMARY_WINDOW
	je	inset
