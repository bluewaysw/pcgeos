...
DESCRIPTION:
	...
	$Id: cwinFieldOther.asm,v 1.15 97/04/02 02:57:18 joon Exp $
...
if _REDMOTIF
	LauncherInteractionClass	mask CLASSF_DISCARD_ON_SAVE
	LaunchThreadClass	mask CLASSF_DISCARD_ON_SAVE
endif
...
ifdef WIZARDBA
	EMCControlPanelClass	mask CLASSF_DISCARD_ON_SAVE
endif
...
if APPLICATION_MENU
	AppListItemClass	mask CLASSF_DISCARD_ON_SAVE or \
				mask CLASSF_NEVER_SAVED
endif
...
OLFieldAboutToDetachComplete	method	dynamic	OLFieldClass,
	...
	clr	ax
if _REDMOTIF
	xchg	ax, ds:[di].OLFI_expressMenu2
	tst	ax
	jz	noExpressMenu2
	call	clobber
noExpressMenu2:
...
ifdef WIZARDBA
	;
	;  clobber window list dialog control panel
	;
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	clr	ax
	xchg	ax, ds:[di].OLFI_windowListCtrlPnl
	tst	ax
	jz	noWindowListCtrlPnl
	call	clobber
noWindowListCtrlPnl:
endif
	;
...
if _REDMOTIF
	...
	; 
		...
		mov	di, ds:[si]
		add	di, ds:[di].OLField_offset
		mov	si, ds:[di].OLFI_expressMenu2
		mov	bx, dx
...
ifdef WIZARDBA
	;
	; If no WindowListDialog control panel for this field, just pass the
	; message to our superclass.
	;
		mov	di, ds:[si]
		add	di, ds:[di].OLField_offset
		tst	ds:[di].OLFI_windowListCtrlPnl
		jz	passItUp2
	;
	; We'll be sending a MSG_META_DETACH to the WindowListDialog
	; control panel, so record an ACK as needing to come in before
	; we finish detaching.
	;
		push	ax, cx, dx, bp, si
		call	ObjIncDetach
	;
	; Send the MSG_META_DETACH to the WindowListDialog control panel.
	;
		mov	dx, ds:[LMBH_handle]
		mov	bp, si
		mov	si, ds:[di].OLFI_windowListCtrlPnl
		mov	bx, dx
		mov	di, mask MF_FIXUP_DS or mask MF_CALL
		call	ObjMessage
		pop	ax, cx, dx, bp, si
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLFieldUpdateKbdStatusButtons
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Update the keyboard status buttons: CapsLock, NumLock, Ins/Over

CALLED BY:	MSG_OL_FIELD_UPDATE_KBD_STATUS_BUTTONS
PASS:		*ds:si	= OLFieldClass object
		ds:di	= OLFieldClass instance data
		ds:bx	= OLFieldClass object (same as *ds:si)
		es 	= segment of OLFieldClass
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	Joon	8/26/94   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if _NIKE	;--------------------------------------------------------------

OLFieldUpdateKbdStatusButtons	method dynamic OLFieldClass, 
					MSG_OL_FIELD_UPDATE_KBD_STATUS_BUTTONS
	uses	cx, dx, bp
	.enter
	...
	CheckHack <mask TS_CAPSLOCK eq mask KSF_CAPSLOCK>
	CheckHack <mask TS_NUMLOCK eq mask KSF_NUMLOCK>

	andnf	dl, mask KSF_CAPSLOCK or mask KSF_NUMLOCK

	call	UserGetOverstrikeMode
	jnz	setBooleans
	ornf	dl, mask KSF_INS_OVER
...
setBooleans:
	...
	.leave
	ret
OLFieldUpdateKbdStatusButtons	endm
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLFieldToggleKbdStatusButton
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Toggle one of the keyboard status buttons

CALLED BY:	MSG_OL_FIELD_TOGGLE_KBD_STATUS_BUTTON
PASS:		*ds:si	= OLFieldClass object
		ds:di	= OLFieldClass instance data
		ds:bx	= OLFieldClass object (same as *ds:si)
		es 	= segment of OLFieldClass
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	Joon	8/30/94   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if _NIKE	;--------------------------------------------------------------

OLFieldToggleKbdStatusButton	method dynamic OLFieldClass, 
					MSG_OL_FIELD_TOGGLE_KBD_STATUS_BUTTON
	test	bp, mask KSF_INS_OVER
	jz	capsNum

	mov	ax, MSG_META_KBD_CHAR
	mov	cx, VC_ISCTRL shl 8 or VC_INS
...
capsNum:
	...
	andnf	cl, mask KSF_CAPSLOCK or mask KSF_NUMLOCK
	andnf	dl, not (mask KSF_CAPSLOCK or mask KSF_NUMLOCK)
	ornf	dl, cl			; set new TS_CAPSLOCK and TS_NUMLOCK
	mov	cx, dx			; cl = ToggleState, ch = KbdTypematics
...
update:
	...
	ret
OLFieldToggleKbdStatusButton	endm
...
DBCS <	mov	bp, 4				;Space for 1st char + null> 
	call	InitFileReadString		;
...
;
; PM just selects the next item in the window list.
;
OLFieldNavigateToNextApp	method	dynamic	OLFieldClass, 
					MSG_OL_FIELD_NAVIGATE_TO_NEXT_APP
	;
...
METHOD:		OLFieldAboutToClose -- 
		MSG_GEN_FIELD_ABOUT_TO_CLOSE for OLFieldClass

DESCRIPTION:	Called when app field is about to close.  In Redwood, we'll
		try to ensure that geoWrite is at the front.
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	chris	10/ 6/93         Initial Version

------------------------------------------------------------------------------@

if	_REDMOTIF

LaunchAppParams	struc
	LAP_geodeToken		fptr		;pointer to geode to open
	LAP_field		optr		;field to open on (0 = default)
LaunchAppParams	ends



OLFieldAboutToClose	method dynamic	OLFieldClass, \
				MSG_GEN_FIELD_ABOUT_TO_CLOSE
	;	
	; Geowrite should already be up, we'll just move it to the front.
	;	
	sub	sp, size LaunchAppParams
	mov	bp, sp
if _FXIP
;	Copy data to stack on XIP systems.
	push	ds, si, cx
	mov	cx, size GeodeToken
	segmov	ds, cs, si
	mov	si, offset writeToken2
	call	SysCopyToStackDSSI
	mov	ss:[bp].LAP_geodeToken.segment, ds
	mov	ss:[bp].LAP_geodeToken.offset, si
	pop	ds, si, cx
else	
	mov	ax, cs
	mov	ss:[bp].LAP_geodeToken.segment, ax
	mov	ss:[bp].LAP_geodeToken.offset, offset writeToken2
endif	
	mov	ax, ds:[LMBH_handle]		;^lax:bp - field
	movdw	ss:[bp].LAP_field, axsi
	mov	dx, size LaunchAppParams
	call	LauncherInteractionLaunchApplNow

FXIP <	call	SysRemoveFromStack				>	
	
	add	sp, size LaunchAppParams
	ret

OLFieldAboutToClose	endm

writeToken2	char	"WP00", 0, 0
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		LauncherInteractionSetField -- 
		MSG_LAUNCHER_SET_FIELD for LauncherInteractionClass
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

DESCRIPTION:	Sets the field optr.

PASS:		*ds:si 	- instance data
		es     	- segment of MetaClass
		ax 	- MSG_LAUNCHER_SET_FIELD
		^lcx:dx - field
...
LauncherInteractionSetField	method dynamic	LauncherInteractionClass, \
				MSG_LAUNCHER_SET_FIELD
	.enter
	movdw	ds:[di].LIC_field, cxdx
	.leave
	ret
LauncherInteractionSetField	endm
...
SYNOPSIS:	Launches app given the token

CALLED BY:	
PASS:		cx:dx = GeodeToken
		*ds:si -- LaunchApplication object
RETURN:		
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	BC	7/ 6/93    	Initial version
	Chris	7/26/93		Changed for the proper delayed launch

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if _REDMOTIF

LaunchApplication	proc	far
	.enter
...
FXIP <	pop	bx, si						>	
	...
	sub	sp, size LaunchAppParams
	mov	bp, sp
	movdw	ss:[bp].LAP_geodeToken, cxdx
	clrdw	ss:[bp].LAP_field

	mov	bx, ds:[LMBH_handle]
	mov	ax, MSG_LAUNCHER_LAUNCH_APPLICATION_NOW
	mov	dx, size LaunchAppParams
...
if	1
	...
	; to the field, which will forward to the application with the full 
	; screen exclusive.  This code is more reliable than looking 
	; at the gcnlist, there seems to be some problems
	; with how fast the GCNSLT_TRANSPARENT_DETACH_FULL_SCREEN_EXCL
	; gcnlist gets updated, which leads to the wrong app getting the
	; message, and sometime crashes if the app is going away. 5/25/94 cbh
	;
	mov	ax, MSG_META_QUERY_SAVE_DOCUMENTS
	mov	di, ds:[si]			
	add	di, ds:[di].Gen_offset
	movdw	bxsi, ds:[di].LIC_field
	mov	di, mask MF_CALL
	call	ObjMessage
...
METHOD:		LauncherInteractionLostSysFocusExcl -- 
		MSG_META_LOST_SYS_FOCUS_EXCL for LauncherInteractionClass

DESCRIPTION:	Losing system focus excl.   Stuck in here so that express
		menus go away if they've lost the system focus.
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	chris	9/14/93         Initial Version

------------------------------------------------------------------------------@

if _REDMOTIF

LauncherInteractionLostSysFocusExcl method dynamic LauncherInteractionClass, \
				MSG_META_LOST_SYS_FOCUS_EXCL

	mov	di, offset LauncherInteractionClass
	call	ObjCallSuperNoLock

	clrdw	cxdx
	mov	bx, ds:[LMBH_handle]
	mov	ax, MSG_META_ENSURE_NO_MENUS_IN_STAY_UP_MODE
	mov	di, mask MF_FORCE_QUEUE		;queue to let lost-sys stuff
						;   finish...
	call	ObjMessage
	ret
LauncherInteractionLostSysFocusExcl	endm
...
METHOD:		LauncherInteractionLaunchApplNow -- 
		MSG_LAUNCHER_LAUNCH_APPLICATION_NOW for LauncherInteractionClass

DESCRIPTION:	Notifies for a file change.   We do this temporarily to keep
		querying for when all the document files have left the RAMdisk,
		so we can go ahead and switch apps.

PASS:		*ds:si 	- instance data
		es     	- segment of MetaClass
		ax 	- MSG_NOTIFY_FILE_CHANGE
		ss:bp   - LaunchAppParams
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	chris	7/22/93		Initial Version

------------------------------------------------------------------------------@

if _REDMOTIF

LauncherInteractionLaunchApplNow method LauncherInteractionClass,
			    	    MSG_LAUNCHER_LAUNCH_APPLICATION_NOW

	push	dx, bp
	mov	ax, MSG_PROCESS_CREATE_EVENT_THREAD
	call	GeodeGetProcessHandle
	mov	cx, segment LaunchThreadClass
	mov	dx, offset LaunchThreadClass
	push	bp
	mov	bp, 2048		; stack size for launching apps
	mov	di, mask MF_CALL
	call	ObjMessage		; ax = new thread handle
	pop	bp
	jc	exitPop			; if error, report it
	...
	mov	ax, MSG_LAUNCH_THREAD_LAUNCH_APPLICATION
...
LauncherInteractionLaunchApplNow	endm

endif


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		ThreadLaunchApplication
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Runs an application given its token

CALLED BY:	MSG_LAUNCH_THREAD_LAUNCH_APPLICATION
PASS:		*ds:si	= LaunchThreadClass object
		ds:di	= LaunchThreadClass instance data
		ds:bx	= LaunchThreadClass object (same as *ds:si)
wo ss:		es 	= segment of LaunchThreadClass
		ax	= message #
		ss:bp   = LaunchAppParams
RETURN:		
DESTROYED:	
...
ThreadLaunchApplication	method dynamic LaunchThreadClass, 
					MSG_LAUNCH_THREAD_LAUNCH_APPLICATION
	uses	ax, cx, dx, bp
	.enter

	movdw	esdi, ss:[bp].LAP_geodeToken
	movdw	axbp, ss:[bp].LAP_field
...
10$:
	...
	call	MyCreateDefaultLaunchBlock
	mov	bx, dx				; move ALB handle to bx
	...
	call	ThreadDestroy		; all done
	.leave
	ret
ThreadLaunchApplication	endm
...
MyCreateDefaultLaunchBlock proc	far
		uses	ds, ax, cx, bx, di
		.enter

		mov	di, ax				;field in ^ldi:bp
		...
		mov	ds, ax
		mov	ds:[ALB_genParent].handle, di
		mov	ds:[ALB_genParent].chunk, bp
		...
		.leave
		ret
MyCreateDefaultLaunchBlock endp
...
SYNOPSIS:	Launches GeoWrite

CALLED BY:	MSG_LAUNCHER_LAUNCH_WRITE
PASS:		*ds:si	= LauncherInteractionClass object
		ds:di	= LauncherInteractionClass instance data
		ds:bx	= LauncherInteractionClass object (same as *ds:si)
		es 	= segment of LauncherInteractionClass
		ax	= message #
RETURN:		
DESTROYED:	
SIDE EFFECTS:	
...
writeToken	char	"WP00", 0, 0
typeToken	char	"ETWs", 0, 0
geodexToken	char	"ADBK", 0, 0
bigcalcToken	char	"BCAL", 0, 0
plannerToken	char	"PLNR", 0, 0
scrapbookToken	char	"Scrp", 0, 0
geomanagerToken	char	"DESK", 0, 0
geocalcToken	char	"GCAL", 0, 0
drawToken	char	"DP00", 0, 0

LauncherLaunchWrite	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_WRITE
	mov	cx, cs
	mov	dx, offset writeToken
	call	LaunchApplication
	ret
LauncherLaunchWrite	endm

LauncherLaunchType	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_TYPE
	mov	cx, cs
	mov	dx, offset typeToken
	call	LaunchApplication
	ret
LauncherLaunchType	endm

LauncherLaunchGeodex	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_GEODEX
	mov	cx, cs
	mov	dx, offset geodexToken
	call	LaunchApplication
	ret
LauncherLaunchGeodex	endm

LauncherLaunchBigCalc	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_BIGCALC
	mov	cx, cs
	mov	dx, offset bigcalcToken
	call	LaunchApplication
	ret
LauncherLaunchBigCalc	endm

LauncherLaunchPlanner	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_PLANNER
	mov	cx, cs
	mov	dx, offset plannerToken
	call	LaunchApplication
	ret
LauncherLaunchPlanner	endm

LauncherLaunchScrapbook	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_SCRAPBOOK
	mov	cx, cs
	mov	dx, offset scrapbookToken
	call	LaunchApplication
	ret
LauncherLaunchScrapbook	endm

LauncherLaunchGeoManager	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_GEOMANAGER
	mov	cx, cs
	mov	dx, offset geomanagerToken
	call	LaunchApplication
	ret
LauncherLaunchGeoManager	endm

LauncherLaunchGeoCalc	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_GEOCALC
	mov	cx, cs
	mov	dx, offset geocalcToken
	call	LaunchApplication
	ret
LauncherLaunchGeoCalc	endm

LauncherLaunchDraw	method dynamic LauncherInteractionClass, 
					MSG_LAUNCHER_LAUNCH_DRAW
	mov	cx, cs
	mov	dx, offset drawToken
	call	LaunchApplication
	ret
LauncherLaunchDraw	endm
