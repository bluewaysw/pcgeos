...
DESCRIPTION:
	...
	$Id: cwinFieldCommon.asm,v 1.7 96/10/23 23:03:53 brianc Exp $
...
if _PM or _NIKE
	call	VisGetSize
	not	cx			; Move to "home" position - off screen
else
...
ifdef WIZARDBA
INITIAL_SLOT	= 2		; to prevent logout icon from being obscured
else
INITIAL_SLOT	= 1
endif
...
METHOD:		OLFieldEnableExpressMenu

DESCRIPTION:	Redwood-only, enables the express menu items.

PASS:
	*ds:si - instance data (for object in OLField class)
	ds:di - OLField instance data
	es - segment of OLFieldClass
	ax - MSG_OL_FIELD_ENABLE_EXPRESS_MENU
...
OLFieldEnableExpressMenu method dynamic OLFieldClass, 
					MSG_GEN_FIELD_ENABLE_EXPRESS_MENU

	;
	; I'm in a fiesty mood.  ax will resolve to MSG_GEN_SET_ENABLED,
	; big two word savings.  Ooooh.
	;
	mov	ax, MSG_GEN_SET_ENABLED - MSG_GEN_SET_NOT_ENABLED + \
		    MSG_GEN_FIELD_DISABLE_EXPRESS_MENU

	FALL_THRU	OLFieldDisableExpressMenu

OLFieldEnableExpressMenu endm

OLFieldDisableExpressMenu method OLFieldClass, 
					MSG_GEN_FIELD_DISABLE_EXPRESS_MENU

	;
	; ax will resolve to MSG_GEN_SET_NOT_ENABLED, unless we've fallen
	; through from the above routine.
	;
	add	ax, MSG_GEN_SET_NOT_ENABLED - MSG_GEN_FIELD_DISABLE_EXPRESS_MENU
...
OLFieldDisableExpressMenu endm
