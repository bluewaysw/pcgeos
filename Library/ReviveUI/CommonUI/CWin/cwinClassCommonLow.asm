...
DESCRIPTION:
	...
	$Id: cwinClassCommonLow.asm,v 1.46 97/04/02 21:37:08 brianc Exp $
...
if BUBBLE_DIALOGS and (_ODIE or _DUI)	;-------------------------------------
...
endif	; BUBBLE_DIALOGS and (_ODIE or _DUI) ;--------------------------------
...
endif	; if BUBBLE_DIALOGS and (not (_ODIE or _DUI)) -------------------------
...
CUA <	mov	al, C_WHITE		;assume is not a menu		>
...
PMAN <	mov	al, C_WHITE		;assume B&W display		>
PMAN <	test	ds:[moCS_flags], mask CSF_BW ; is this a B&W display?	>
PMAN <	jnz	OWOW_haveColor		;   skip if so...		>
PMAN <	mov	al, ds:[moCS_dsLightColor]	;else use light color	>
OWOW_haveColor:
	pop	ds
...
if _RUDY
	...
	;
	; Also, check to see if indicator position is good.
	; -- kho, 2/5/96
	;
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		RudyRememberIfHelpIsUp
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	If this window object is a help screen,
		store the fact that it does/doesn't exist.
		This info is used to help manage the indicator.

CALLED BY:	INTERNAL, OpenWinBringToTop, OpenWinVisClose
PASS:		*ds:si	= windowed object to test
		ax	= TRUE if window is coming up, FALSE if going away
RETURN:		nothing
DESTROYED:	nothing
SIDE EFFECTS:	
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
RudyRememberIfHelpIsUp	proc	far
	uses	es, di
	.enter
	;
	; Art thou the window we seeketh?
	;
		mov	di, segment HelpControlClass
		mov	es, di
		mov	di, offset  HelpControlClass
		call	ObjIsObjectInClass
		jnc	done			; nay? Then be gone with you!
	;
	; Let us sing of windows opened and closed!
	;
		mov	di, segment dgroup
		mov	es, di
		mov	es:[helpWindowIsUp], ax
done:
	.leave
	ret
RudyRememberIfHelpIsUp	endp
...
if _ROUND_THICK_DIALOGS
	...
	;
	;PCV needs to have si preserved, so don't use it to shuttle 
	; the constant onto the stack.
	;
if _PCV
	push	ax			; 'push' a placeholder on the stack
	push	si			; save our object handle on top of that
	mov	si, sp			; ss:si points to our object handle
	add	si, 2			; ss:si points to the placeholder
	mov	ss:[si], mask WPF_ABS	; set that word to the constant
...
EC <	pop	di							>
	...
	;
	; Odie: we only constrain menus that are not under the menubar
	;
ODIE <	call	CheckIfMenuButtonUnderMenuBar				>
ODIE <	jc	exit							>

	;
	; Another hack for PM: if the menu is not pressed, then don't worry
	; about overlapping the button.
	;
PMAN <	push	di							>
PMAN <	call	WinCommon_DerefVisSpec_DI				>
PMAN <	test	ds:[di].OLBI_specState, mask OLBSS_DEPRESSED		>
PMAN <	pop	di							>
PMAN <	jz	exit							>
...
if _JEDIMOTIF
	...
	mov	bx, 0
	jnc	notTitleBar
	mov	bh, -1			;indicate opening down/right
					;	instead of up/left
notTitleBar:
else
...
if _JEDIMOTIF
	;
	; adjust to avoid conflict with gadget area AND for title bar objects
	; being inset 1 pixel
	;
	dec	dx
	dec	dx
...
if (not _PM)
	dec	dx				; PM needs only 1 pixel adjust.
endif
done:
endif	; if _JEDIMOTIF else
	ret
...
if not _DUI	; always try to draw long term moniker for DUI even though
		; we're on a small screen -- brianc 4/1/97 (no foolin')
	;
