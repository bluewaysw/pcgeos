...
DESCRIPTION:
	...
	$Id: cwinClassCommonMiddle.asm,v 1.26 96/11/20 18:08:40 joon Exp $
...
haveObjectToDestroy:
	...
	ERROR	-1				;shouldn't be hit in Redwood
...
if _RUDY
if ERROR_CHECK
	; Dialogs launched on UI thread should be sysModal. Doesn't
	; make sense from API point of view to have non-sysModal
	; dialogs come up on UI thread.
	;
	pushf
	mov	bx, ds:[LMBH_handle]
	call	MemOwner
	cmp	bx, handle ui		; Running on UI thread?
	jne	notUI			; NO, done
	...
	pop	di
	ERROR_Z	OL_DIALOGS_INITIATED_ON_UI_THREAD_MUST_BE_SYS_MODAL
notUI:
	popf
endif	; ERROR_CHECK
endif	; _RUDY
...
if _JEDIMOTIF
	;
	; if doing ENTER navigation, set flag indicating no focus found yet
	;
	mov	ax, TEMP_OL_WIN_ENTER_NAVIGATION
	call	ObjVarFindData
	jnc	notEnterNav
	mov	({optr}ds:[bx]).handle, 0
notEnterNav:
endif
...
if _JEDIMOTIF ;---------------------------------------------------------------
	...
	call	checkInClass
	pop	si
	mov	di, 0				; di = 0 if VisText
	jc	isText				; yes
	dec	di				; di = -1 if not VisText
isText:
	;
	; Handle TEMP_OL_WIN_ENTER_NAVIGATION
	;	*ds:si = OLWin
	;	^lcx:dx = node to navigate to
	;	ax = original NavigateFlags
	;	di = 0 if VisText
	;
	push	ax
	mov	ax, TEMP_OL_WIN_ENTER_NAVIGATION
	...
	pop	ax
	jnc	notEnter
	cmp	({optr}ds:[bx]).handle, 0		; first time?
	jne	checkIfFirst
	mov	({optr}ds:[bx]).handle, cx		; save first object
	mov	({optr}ds:[bx]).offset, dx
	jmp	short notFirst				; skip first check

checkIfFirst:
	;
	; see if we've reached our starting point
	;
	cmp	({optr}ds:[bx]).handle, cx
	jne	notFirst
	cmp	({optr}ds:[bx]).offset, dx
	LONG je	done					; yes, bail

notFirst:
	tst	di
	jz	notEnter				; is text, give focus
	;
	; not a VisText, check if OLSpinGadget
	;
	push	si
	mov	bx, segment OLSpinGadgetClass
	mov	si, offset OLSpinGadgetClass
	call	checkInClass				; ^lbx:si = object
	pop	si
	jnc	notSpin					; not spin
	push	ax, si
	mov	si, dx					; ^lbx:si = object
	mov	ax, MSG_OL_SPIN_CHECK_FLOATING_TEXT
	call	WinCommon_ObjMessageCallFixupDS		; carry if floating
							; preserves cx, dx, bp
	pop	ax, si
	jnc	notEnter				; real text, give focus
notSpin:
	;
	; not a VisText, navigate from here to next obj
	;	*ds:si = OLWin
	;	^lcx:dx = non-VisText node to navigate to
	;	ax = original NavigateFlags
	;
	ornf	ax, mask NF_SKIP_NODE or mask NF_INITIATE_QUERY
	mov	bp, ax
	push	bp
	jmp	sendNavQueryToOD
...
if _JEDIMOTIF ;---------------------------------------------------------------
	;
	; grab target as well, if targetable text object
	;	di = 0 if VisText
	;
	tst	di
	jnz	noTarget
	...
	call	WinCommon_ObjMessageCallFixupDS	; cl = VisTextState
	test	cl, mask VTS_TARGETABLE
...
if _JEDIMOTIF ;---------------------------------------------------------------
	;
	; if we are navigating to a editable single-line text object,
	; select all its text after making it the focus
	;
	push	cx, dx, bp			; save OD, MAEF flags
	mov	ax, MSG_META_MUP_ALTER_FTVMC_EXCL
	call	WinCommon_ObjCallInstanceNoLock
	pop	cx, dx, bp			; restore OD, MAEF flags
	test	bp, mask MAEF_GRAB
	jz	notGrab
	;
	; make sure we wanted to do this
	;
	call	WinCommon_DerefVisSpec_DI
	test	ds:[di].OLWI_moreFixedAttr, mask OWMFA_SELECT_TEXT_ON_NAVIGATE
	jz	notGrab
	push	si
	mov	bx, segment VisTextClass
	mov	si, offset VisTextClass
	call	checkInClass			; ^lbx:si = object
	jnc	notSingleLineTextGrab
	mov	ax, MSG_VIS_TEXT_GET_STATE
	call	WinCommon_ObjMessageCallFixupDS	; cl = VisTextStates
	andnf	cl, mask VTS_EDITABLE \
		 or mask VTS_SELECTABLE \
		 or mask VTS_ONE_LINE
	cmp	cl, mask VTS_EDITABLE \
		 or mask VTS_SELECTABLE \
		 or mask VTS_ONE_LINE
	jne	notSingleLineTextGrab
	mov	ax, MSG_VIS_TEXT_SELECT_ALL
	call	WinCommon_ObjMessageCallFixupDS
notSingleLineTextGrab:
	pop	si
notGrab:
else ;------------------------------------------------------------------------
	mov	ax, MSG_META_MUP_ALTER_FTVMC_EXCL
...
if _JEDIMOTIF ;---------------------------------------------------------------
	call	WinCommon_DerefVisSpec_DI
	andnf	ds:[di].OLWI_moreFixedAttr, not \
			 mask OWMFA_SELECT_TEXT_ON_NAVIGATE
	mov	ax, TEMP_OL_WIN_ENTER_NAVIGATION
	call	ObjVarDeleteData
endif ;-----------------------------------------------------------------------
	ret
...
if _JEDIMOTIF ;---------------------------------------------------------------
; pass:	^lcx:dx = object to check
;	bx:si = class
; return: carry set if in class
;	^lbx:si = chunk handle of object
; destroyed: nothing
checkInClass	label	near
	push	ax, cx, dx, bp, di
	xchgdw	bxsi, cxdx			; ^lbx:si = obj, cx:dx = class
	mov	ax, MSG_META_IS_OBJECT_IN_CLASS
	call	WinCommon_ObjMessageCallFixupDS	; carry set if in class
	pop	ax, cx, dx, bp, di
	retn
endif ;-----------------------------------------------------------------------
...
if _PM	; to make sure resize borders are redrawn.
	call	OpenWinGetBounds
else
	call	OpenWinGetHeaderTitleBounds
ODIE <	dec	dx				;don't inval bottom pix	>
endif
	jmp	haveBounds
...
PMAN <	call	OpenWinDrawResizeBorder					>
	pop	dx
done:
	call	GrDestroyState
exit:
...
if _JEDIMOTIF
	;
	; let's encourage flashing when we get GEOMETRY_UPDATE for
	; no apparent reason -- if the title bar objects are invalid
	; or on the update path
	;
	call	WinCommon_DerefVisSpec_DI
	push	si
	movdw	bxsi, ds:[di].OLWI_titleBarLeftGroup
	tst	bx
	jz	noLeft
	mov	ax, MSG_VIS_GET_OPT_FLAGS
	call	WinCommon_ObjMessageCallFixupDS
	test	cl, VOF_PATH_BITS or VOF_INVALID_BITS
noLeft:
	pop	si
	jnz	updateGeo
	call	WinCommon_DerefVisSpec_DI
...
if _JEDIMOTIF
	;
	; JEDI modal dialogs have 2-pixel hatch border.  Need only to adjust
	; top.
	;
	cmp	ds:[di].OLWI_type, MOWT_COMMAND_WINDOW
	jb	notModal
	cmp	ds:[di].OLWI_type, MOWT_HELP_WINDOW
	ja	notModal
.warn -private
...
if _PCV
	push	cx, dx
	call	GetPCVSpecificWindowRegion
