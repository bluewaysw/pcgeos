...
DESCRIPTION:
	...
	$Id: cwinClassCUAS.asm,v 1.85 97/01/01 22:12:29 joon Exp $
...
PMAN <	cmp	ds:[di].OLWI_type, MOWT_PRIMARY_WINDOW			>
PMAN <	jne	notPrimary						>
PMAN <	tst	ds:[di].OLBWI_titleBarMenu.handle			>
PMAN <	jnz	next2							>
PMAN <notPrimary:							>
...
PMAN <	mov	ax, MSG_OL_WIN_SET_CUSTOM_SYSTEM_MENU_MONIKER		>
PMAN <	call	ObjCallInstanceNoLock					>
endif ;not _REDMOTIF ;------------------- Not needed for Redwood project
...
if not _REDMOTIF ;----------------------- Not needed for Redwood project
if not _NIKE ;----------------------- no system menu icons in nike
...
endif ;not _NIKE ;------------------- no system menu icons in nike
endif ;not _REDMOTIF ;------------------- Not needed for Redwood project
...
if not _REDMOTIF ;----------------------- Not needed for Redwood project
if not _NIKE ;----------------------- Not needed for Nike project
...
endif ;not _NIKE ;------------------- Not needed for Nike project
endif ;not _REDMOTIF ;------------------- Not needed for Redwood project
...
if _JEDIMOTIF
	;
	; save left and right bounds of window header to see if there end
	; up being any gadgets in the left and right areas.  If not, we
	; want to set up a 6 pixel gutter on each side that doesn't have
	; gadgets
	;
	push	ds:[di].OLWI_titleBarBounds.R_left
...
if (not _ODIE)	; close button is on the right side
...
if not _JEDIMOTIF				; no overlap in JEDI
	dec	cx
endif
	call	WinCommon_DerefVisSpec_DI
...
if not _JEDIMOTIF				; no overlap for JEDI
...
if _ODIE
	; dialogs always have close button
	...
	; set close button enable/~enabled based on OWA_CLOSABLE flag
...
if _JEDIMOTIF
	;
	; if dialog and no left or right stuff, add 6 pixel gutter, else
	; 3 pixel gutter (3 pixel gutter for primaries is digitizer dead
	; space)
	;
.assert (MOWT_NOTICE_WINDOW eq MOWT_PROPERTIES_WINDOW+1)
.assert (MOWT_PROPERTIES_WINDOW eq MOWT_COMMAND_WINDOW+1)
	pop	dx				; previous right
	pop	cx				; previous left
	call	WinCommon_DerefVisSpec_DI
	cmp	ds:[di].OLWI_type, MOWT_COMMAND_WINDOW
	jb	notDialog
	cmp	ds:[di].OLWI_type, MOWT_NOTICE_WINDOW
	ja	notDialog
	mov	ax, 3				; initial left gutter
	cmp	cx, ds:[di].OLWI_titleBarBounds.R_left
	jne	haveLeftStuff
	add	ax, 3				; no objects in left
haveLeftStuff:
	add	ds:[di].OLWI_titleBarBounds.R_left, ax
	mov	ax, 3				; initial right gutter
	cmp	dx, ds:[di].OLWI_titleBarBounds.R_right
	jne	haveRightStuff
	add	ax, 3				; no objects in right
haveRightStuff:
	sub	ds:[di].OLWI_titleBarBounds.R_right, ax
notDialog:
endif
...
if _JEDIMOTIF
	;
	; this matches JEDIMOTIF code in cwinGeometry.asm near
	; label 2$
	;
	inc	cx
	inc	dx
...
if (not _ODIE)	; close button on right side
	;
...
if _JEDIMOTIF
	;
	; adjust to avoid conflict with gadget area
	;
	dec	ax
else
...
if (not _PM)
	dec	ax				; PM needs only 1 pixel adjust.
endif
gotHeight:
endif
	;
...
if 0
	;
	; make size tweaks -- some unknown hacker
	;
	; nuked size tweaks -- stevey 10/6/94 (they screw up the
	; VCP_centerOffset for the title bar, so children get
	; positioned incorrectly).
	;
MO <	call	OpenCheckIfBW			; Is this a B&W display?>
MO <	jc	220$				; skip if so...		>
MO <	sub	dx, 2				; Nest icon inside resize bars>
MO <220$:					;  & title in a color display >

PMAN <	call	OpenCheckIfBW			; Is this a B&W display?>
PMAN <	jc	220$				; skip if so...		>
PMAN <	dec	dx				; Nest icon inside resize bars>
PMAN <220$:					;  & title in a color display >
...
if _JEDIMOTIF
	;
	; title bar groups only, need to be pushed down 1 pixel to
	; match exit button position (and sort-of to match ERS)
	;
	inc	dx
endif
...
readyWithWidthHeight:
	...
	push	di
	call	WinCommon_DerefVisSpec_DI
PMAN <	test	bl, mask OLWSI_SYS_MENU		;mask for Sys Menu icon	>
PMAN <	jz	normalSysMenu						>
PMAN <	call	GetSystemMenuBlockHandle	; return bx = sys menu  >
PMAN <	jmp	short haveSysMenu
normalSysMenu:
...
PMAN <	dec	dx				; Nest icon inside resize  >
PMAN <	dec	cx				;   bars & title in a color>
PMAN <20$:					;   display		   >
