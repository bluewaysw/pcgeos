...
DESCRIPTION:
	...
	$Id: cwinPopup.asm,v 2.163 97/04/02 21:56:34 brianc Exp $
...
if BUBBLE_DIALOGS and _ODIE
	; Add this hint here because our button doesn't get
	; MSG_VIS_NOTIFY_GEOMETRY_VALID (OLButtonNotifyGeometryValid).
...
if _JEDIMOTIF
	;
	; copy over any HINT_SEEK_SLOT info
	;	*ds:si = window
	;	*ds:dx = button
	;	bx = OLBuildFlags
	;	bp = SpecBuildFlags
	...
	call	ObjVarFindData		;carry set if found, preserves ax
	jnc	noSlot
	mov	di, ds:[bx]		;di = slot
	xchg	si, dx			;*ds:si = button, *ds:dx = window
	mov	cx, size word
	call	ObjVarAddData		;ds:bx = new vardata
	mov	ds:[bx], di		;save slot
	xchg	si, dx			;*ds:si = window, *ds:dx = button
noSlot:
	pop	bx
endif
...
if _RUDY
	;
	; Now that were part of the tree,
	; Tell any appropriate parent OLCtrls that we deserve a
	; right arrow.
	;
	push	ax,cx,dx,bp,si
	mov	cx, TRUE
	mov	ax, MSG_SPEC_NOTIFY_DESIRE_RIGHT_ARROW
	mov	si, bx				; *ds:si = OLButton
	call	VisCallParent
	pop	ax,cx,dx,bp,si
...
if _ODIE
	;
	; we do this after OLBSS_SYS_ICON has been used to determine the
	; vis parent
	;
	call	fixTitleBarMenus
endif
...
if _JEDIMOTIF
fixInMenuFlag	label	near
.warn	-private
	;
	; turn off OLBSS_IN_MENU_BAR if vis parent isn't a menu bar
	;
	call	WinClasses_DerefVisSpec_DI
	test	ds:[di].OLBI_specState, mask OLBSS_IN_MENU_BAR
	jz	fixed
	push	ax, cx, dx, bp
	mov	cx, segment OLMenuBarClass
	mov	dx, offset OLMenuBarClass
	mov	ax, MSG_META_IS_OBJECT_IN_CLASS
	call	VisCallParent
	jc	isMenuBar
	call	WinClasses_DerefVisSpec_DI
	andnf	ds:[di].OLBI_specState, not mask OLBSS_IN_MENU_BAR
	mov	cx, segment OLTitleGroupClass
	mov	dx, offset OLTitleGroupClass
	mov	ax, MSG_VIS_VUP_FIND_OBJECT_OF_CLASS
	call	ObjCallInstanceNoLock
	jnc	isMenuBar			; not in title, leave menu mark
	call	WinClasses_DerefVisSpec_DI
...
if _ODIE
;
; pass:	*ds:si = OLPopupWin
;
fixTitleBarMenus	label	near
	;
	; if in title bar, set some defaults
	;
	call	WinClasses_DerefVisSpec_DI
...
inTitleBar:
	...
	call	WinClasses_DerefVisSpec_DI
	ornf	ds:[di].OLBI_moreAttrs, mask OLBMA_IN_TOOLBOX or mask OLBMA_EXPAND_HEIGHT_TO_FIT_PARENT
...
if _OL_STYLE	;--------------------------------------------------------------
OLPopupWinSetPinnedIfLongTerm	proc	far
	;if already visible, do not change pinned status

	call	WinClasses_DerefVisSpec_DI
	test	ds:[di].VI_attrs, mask VA_VISIBLE
	jnz	done			;skip if already visible...

	ANDNF	ds:[di].OLWI_specState, not (mask OLWSS_PINNED)

	test	ds:[di].OLWI_attrs, mask OWA_PINNABLE
	jz	done
	test	ds:[di].OLWI_fixedAttr, mask OWFA_LONG_TERM
	jz	done

	ORNF	ds:[di].OLWI_specState, mask OLWSS_PINNED
done:
	ret
OLPopupWinSetPinnedIfLongTerm	endp
endif		;--------------------------------------------------------------
...
POPUP_MINIMUM_WIDTH	equ	250

OLPopupGetMinimumSize	method dynamic OLPopupWinClass, 
					MSG_VIS_COMP_GET_MINIMUM_SIZE
	mov	di, offset OLPopupWinClass	;call superclass for desired
	call	ObjCallSuperNoLock		;   size stuff
						; cx,dx <- min width,height
	;
	; If it's a special Rudy popup, enforce a minimum width
	;

	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	test	ds:[di].OLPWI_flags, mask OLPWF_IS_POPUP ; Rudy-only flag
	jz	notPopup

	;
	; Minimum width should be POPUP_MINIMUM_WIDTH + any margin
	; needed to render the spout
	;

	mov	ax, TEMP_OL_WIN_BUBBLE_MARGIN
	call	ObjVarFindData
	mov	ax, POPUP_MINIMUM_WIDTH		; ax = Min width
	jnc	noSpout
	add	ax, {word}ds:[bx]		; ax = Min width + the spout
noSpout:					; ax = min width + any spout
	cmp	cx, ax
	jge	notPopup
	mov	cx, ax
notPopup:
...
if _DUI
	;
	; If we have a keyboard, let's close it right now.
	;
	push	ax
	...
	je	noKbd
	mov	ax, mask FKF_NEEDS_SPACE
	cmp	{word}ds:[bx], KO_KEYBOARD_EMBEDDED
	je	haveFlags
haveFlags:
	clr	ax
	call	FloatingKbdSetState		; cx, dx not needed for close
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset		; just in case
noKbd:
	pop	ax
endif
...
if _ODIE
	;
	; for _ODIE, if this is a menu bar (being activated by F8 kbd
	; shortcut), we want menu bar button to grab focus and gadget excl
	; like it does when tapped on, so we'll send
	; MSG_OL_MENU_BUTTON_KBD_ACTIVATE instead of MSG_GEN_ACTIVATE.
	;
	; There may be undesirable side effects if modified the OLMenuButton
	; MSG_GEN_ACTIVATE handler instead.
	;
...
if _OL_STYLE	;START of OPEN LOOK specific code -----------------------------
	...
	call	WinClasses_DerefVisSpec_DI
	test	ds:[di].OLWI_fixedAttr, mask OWFA_IS_MENU
	jz	skipToDone			;skip if not a menu...
...
if _OL_STYLE	;START of OPEN LOOK specific code -----------------------------
	...
	;set the buttons as bordered, marking each as GEOMETRY_INVALID,
	;and this window as GEO_INVALID_PATH

	mov	cx, FALSE		; make them all un-bordered
	...
	call	ObjCompProcessChildren		; Go make all buttons bordered
endif		;--------------------------------------------------------------
...
if _ODIE
	;
	; the garbage value here is bad in ODIE
	;
	clr	bp			; fake UIFA flags
endif
	mov	ax, MSG_MO_MW_LEAVE_STAY_UP_MODE
	call	WinClasses_ObjCallInstanceNoLock
