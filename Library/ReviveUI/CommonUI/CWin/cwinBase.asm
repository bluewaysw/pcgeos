...
DESCRIPTION:
	...
	$Id: cwinBase.asm,v 2.188 97/04/01 22:35:03 joon Exp $
...
NKE <	ANDNF	ds:[di].OLWI_attrs, not mask OWA_HAS_SYS_MENU		>
ODIE <	ANDNF	ds:[di].OLWI_attrs, not mask OWA_HAS_SYS_MENU		>

CUAS <	mov	ds:[di].OLWI_type, MOWT_PRIMARY_WINDOW			>
				; Store is base window

    	; For RUDY, on base windows, please, no border.. thanks.
RUDY <	ANDNF	ds:[di].OLWI_attrs, not mask OWA_THICK_LINE_BORDER	>
	; For ODIE, no thick border on windows
ODIE <	ANDNF	ds:[di].OLWI_attrs, not mask OWA_THICK_LINE_BORDER	>
...
if _JEDIMOTIF	;--------------------------------------------------------------
	;
	;  No way to make this a feature, really.  We promised them
	;  they would own this.
	;
	pop	si
	call	EnsureAppMenu
	call	EnsureMoreTrigger
	;
	; Okay, more hideousness -- ensure a JotterControl
	;
	call	EnsureJotterControl
endif	;----------------------------------------------------------------------
...
PMAN <	mov	ax, HINT_ENSURE_TEMPORARY_DEFAULT			>
PMAN <	clr	cx				; hack to make trigger	>
PMAN <	call	ObjVarAddData			;  larger than moniker	>
...
afterGCM:
	...
	;
	; Add the keyboard accelerator F1 to it
	;
if _REDMOTIF
	push	cx, dx, si
	mov	si, dx				;*ds:si <- GenTrigger object
;	mov	cx, mask KS_PHYSICAL or ((VC_ISCTRL shl 8) and \
;		    mask KS_CHAR_SET) or VC_CTRL_H
...
haveMoniker:
	call	AA_DerefVisSpec_DI
	mov	bx, ds:[di].OLBWI_windowListEntry.handle
	...
	mov	di, dx
	mov	dx, ds:[di]		; deref
	add	dx, offset VM_data + offset VMT_text	; add offset to string
					; cx:dx is now fptr to null-term string
	pop	ax
	push	bx			; preserve bx for ObjSwapUnlock
	mov	bx, ax			; ^lbx:si = GenItem
	;
	; set found moniker in GenItem
	;	^lbx:si = GenItem
	;	cx:dx = far pointer to null-terminated text string
	;
	mov	bp, VUM_NOW
	mov	ax, MSG_GEN_REPLACE_VIS_MONIKER_TEXT
	mov	di, mask MF_CALL or mask MF_FIXUP_DS
...
OLBaseWinUpdateWindowEntry	method dynamic OLBaseWinClass, 
	...
	mov	di, mask MF_CALL or mask MF_FIXUP_DS
	call	ObjMessage		;identifier in ax
	mov	cx, ax
	clr	dx
	...
	;
	; Send MSG_GEN_LIST_SET_EXCL to parent of GenListEntry.
	;
...
if _PM	; NOTE: this is not OLBaseWin, It is OLWindowListItem -----------------
...
OLWindowListItemKeyboardChar	method dynamic OLWindowListItemClass,
	...
	cmp	cx, (VC_ISCTRL shl 8) or VC_ENTER
	jne	callSuper
...
EnsureFileMenu	proc	near
	...
	; for Jedi we *never* want a File menu
	; Don't want a file menu if _DISABLE_APP_EXIT_UI is true.
if not ( _JEDIMOTIF or _DISABLE_APP_EXIT_UI )
...
SYNOPSIS:	Make sure we have an app menu trigger.
...
EnsureAppMenu	proc	far
		uses	ax,bx,cx,dx,si,di,bp
		.enter
	;
	;  Custom window -> not necessary.
	;
		mov	di, ds:[si]
		add	di, ds:[di].Vis_offset
		test	ds:[di].OLWI_moreFixedAttr, mask OWMFA_CUSTOM_WINDOW
		LONG	jnz	done		; not for custom windows
	;
	; ensure that we have an App menu under the app object
	;
		mov	ax, MSG_OL_APP_ENSURE_APP_MENU
		call	UserCallApplication	; ^lcx:dx = App menu
	;
	; tell App menu to build a button for us
	;
		push	si
		push	si
		movdw	bxsi, cxdx		; ^lbx:si = app menu
		mov	cx, ds:[LMBH_handle]	; ^lcx:dx = OLBaseWin
		pop	dx
		mov	ax, MSG_OL_MENU_BUILD_APP_MENU_BUTTON
		mov	di, mask MF_CALL or mask MF_FIXUP_DS
		call	ObjMessage		; ^lcx:dx = menu button
		pop	si			; *ds:si = OLBaseWin
	;
	; save menu button optr
	;	*ds:si = OLBaseWin
	;	^lcx:dx = menu button
	;
		push	cx
		mov	ax, TEMP_OL_BASE_WIN_APP_MENU_BUTTON
		mov	cx, size optr
		call	ObjVarAddData
		pop	({optr}ds:[bx]).handle
		mov	({optr}ds:[bx]).offset, dx
done:
		.leave
		ret
EnsureAppMenu	endp
...
SYNOPSIS:	Add a "More" Button to the menu bar.

CALLED BY:	OLBaseWinUpdateSpecBuild

PASS:		*ds:si = OLBaseWin

RETURN:		nothing (saves lptr of trigger in instance data)

DESTROYED:	nothing (ds updated to point to same block)
...
EnsureMoreTrigger	proc	near
	...
	;
		...
		LONG	jnz	done		; not for custom windows
	;
	;  Already have app menu -> not necessary.
	;
		mov	ax, TEMP_OL_BASE_WIN_MORE_TRIGGER
		call	ObjVarFindData
		LONG	jc	done
	;
	;  Instantiate the trigger as a child of the PRIMARY,
	;  since OLMenuBarClass can't have Gen children, and this
	;  needs to be Generic in order to have an action message
	;  and a moniker.
	;
		push	es
		mov	dx, si			; *ds:dx = primary
		mov	di, segment GenTriggerClass
		mov	es, di
		mov	di, offset GenTriggerClass
		mov	al, -1			; init USABLE
		mov	ah, -1			; use one-way linkage
		mov	bx, TEMP_OL_BUTTON_MORE_TRIGGER
		mov	cx, ds:[LMBH_handle]	; put in our block
		mov	bp, CCO_LAST
		call	OpenCreateChildObject	; ^lcx:dx = new object
		pop	es
	;
	;  Save lptr in the primary.
	;
		mov	ax, TEMP_OL_BASE_WIN_MORE_TRIGGER
		mov	cx, size lptr
		call	ObjVarAddData
		mov	ds:[bx], dx		; save lptr of trigger
	;
	;  Give it a moniker.
	;
		mov	si, dx			; *ds:si = trigger
		...
		mov	dx, offset JediMoreTriggerMoniker
	...
	;
	;  Set action message and destination, AFTER we've spec-
	;  built it, so we have a destination to send the message.
	;
		mov	ax, MSG_GEN_TRIGGER_SET_ACTION_MSG
		mov	cx, MSG_OL_MENU_BAR_POSITION_CHILDREN
		call	ObjCallInstanceNoLock
	;
	;  Set the destination to be the vis-parent.
	;
		push	si				; save trigger
		call	VisFindParent			; ^lbx:si = parent
		movdw	cxdx, bxsi			; ^lcx:dx = parent
		pop	si				; *ds:si = trigger
		mov	ax, MSG_GEN_TRIGGER_SET_DESTINATION
		call	ObjCallInstanceNoLock
done:
		.leave
		ret
EnsureMoreTrigger	endp
...
REVISION HISTORY:
	Name	Date			Description
	----	----			-----------
	brianc	1/26/95			Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EnsureJotterControl	proc	near
		uses	ax,bx,cx,dx,si,di,bp
		.enter
	;
	;  Custom window -> not necessary.
	;
		...
		LONG	jnz	done		; not for custom windows
	;
	;  Already have it -> not necessary.
	;
		mov	ax, TEMP_OL_BASE_WIN_JOTTER_CONTROL
		call	ObjVarFindData
		LONG	jc	done
	;
	; see if we have a app-defined one
	;
		clr	bx			; initial child (first one)
		push	bx
		push	bx
		mov	bx, offset GI_link	; gen children
		push	bx
NOFXIP <	push	cs			; callback segment	>
FXIP <		mov	bx, SEGMENT_CS					>
FXIP <		push	bx			; XIP callback segment	>
		mov	bx, offset CheckForJotterControl
		push	bx			; callback offset
		mov	bx, offset Gen_offset	; master offset
		mov	di, offset GI_comp	; composite offset
		call	ObjCompProcessChildren
		LONG jc	done			; found JotterControl
	;
	;  Instantiate the JotterControl as a child of the PRIMARY
	;
		push	es
		mov	dx, si			; *ds:dx = primary
		mov	di, segment JotterControlClass
		mov	es, di
		mov	di, offset JotterControlClass
		mov	al, 0			; init not USABLE
		mov	ah, -1			; upward linkage
		mov	bx, 0
		mov	cx, ds:[LMBH_handle]	; put in our block
		mov	bp, CCO_LAST
		call	OpenCreateChildObject	; ^lcx:dx = new object
	...
	;
		...
		mov	cx, size word
		call	ObjVarAddData
		mov	{word} ds:[bx], 0	; seek leftmost spot
						;	in right title bar
		...
		call	ObjCallInstanceNoLock
	;
	;  If we currently have a stuck page, set ourselves
	;  usable.
	;
		call	ClipboardGetClipboardFile
		mov	ax, JOTTER_STUCK_PAGE_USER_ID
		clr	cx
		call	VMFind			       ; carry set if none
		jc	doneBuild
...
doneBuild:		
		...
		.leave
		ret
EnsureJotterControl	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		CheckForJotterControl
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	see if this child is a JotterControl

CALLED BY:	EnsureJotterControl via ObjCompProcessChildren
PASS:		*ds:si - child
		*es:di - composite
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
CheckForJotterControl	proc	far
	mov	di, segment JotterControlClass
	mov	es, di
	mov	di, offset JotterControlClass
	call	ObjIsObjectInClass
	ret
CheckForJotterControl	endp
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLBaseWinRebuildAppMenuButton
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	rebuild App menu button

CALLED BY:	MSG_OL_BASE_WIN_REBUILD_APP_MENU
PASS:		*ds:si	= OLBaseWinClass object
		ds:di	= OLBaseWinClass instance data
		ds:bx	= OLBaseWinClass object (same as *ds:si)
		es 	= segment of OLBaseWinClass
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	brianc	2/16/95   	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLBaseWinRebuildAppMenuButton	method dynamic OLBaseWinClass, 
					MSG_OL_BASE_WIN_REBUILD_APP_MENU_BUTTON
	;
	; destroy current App menu button
	;
	mov	di, -1			; don't update App menu
	call	RemoveAppMenuButton
	;
	; build App menu button
	;
	call	EnsureAppMenu
	ret
OLBaseWinRebuildAppMenuButton	endm
...
PMAN <	call	EnsureItemRemovedFromWindowList				>
	jmp	short done

notDetaching:

PMAN <	test	cx, mask UWF_ATTACHING					>
PMAN <	jz	done							>
PMAN <	call	EnsureItemAddedToWindowList				>
...
if _JEDIMOTIF	;--------------------------------------------------------------
	...
	;
	; nuke this OLBaseWin's App menu button, if any
	;
	mov	di, 0				;update App menu
	call	RemoveAppMenuButton
	...
	mov	si, ax				;*ds:si = Jotter control
	mov	ax, MSG_GEN_CONTROL_REMOVE_FROM_GCN_LISTS
	call	ObjCallInstanceNoLock
	mov	ax, si				;*ds:ax = Jotter control
	pop	si
	call	OLBWRSBO_genDestroy
noJotter:
...
if _JEDIMOTIF
	mov	dl, VUM_DELAYED_VIA_UI_QUEUE	;dl <- VisUpdateMode
else
	mov	dl, VUM_NOW
endif
...
SYNOPSIS:	remove creatd App menu button

CALLED BY:	OLBaseWinRemoveSpecBuildCreatedObjects
		OLBaseWinRebuildAppMenuButton
PASS:		*ds:si = OLBaseWin
		di = 0 if we should update the App menu
		di <> 0 otherwise
RETURN:		nothing
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
RemoveAppMenuButton	proc	near
	;
	; check if we have App menu button
	;
	mov	ax, TEMP_OL_BASE_WIN_APP_MENU_BUTTON
	call	ObjVarFindData
	jnc	noAppMenuButton
	;
	; we do, remove vardata
	;
	push	si
	push	({optr}ds:[bx]).handle, ({optr}ds:[bx]).offset
	call	ObjVarDeleteDataAt		;remove menu button flag
	;
	; then remove button
	;
	pop	bx, si				;^lbx:si = menu button
	call	ObjSwapLock			;*ds:si = menu button
	tst	di				;update App menu?
	jnz	noUpdate			;nope
	mov	ax, MSG_OL_MENU_UPDATE_APP_MENU
	clr	cx				;no more button
	call	OLButtonCallGenPart
noUpdate:
	call	VisCheckIfSpecBuilt
	jnc	unbuilt
	mov	ax, MSG_SPEC_UNBUILD_BRANCH
	clr	bp				;SpecBuildFlags
	mov	dl, VUM_NOW
	call	ObjCallInstanceNoLock
unbuilt:
	call	ObjSwapUnlock
...
EC <	ERROR_Z	OL_ERROR						>
	...
	call	WinClasses_DerefVisSpec_DI
if _ODIE
	add	ds:[di].OLWI_titleBarBounds.R_right, bp
if EVENT_MENU
	call	OLBaseWinGetEventMenuButtonWidth
	call	WinClasses_DerefVisSpec_DI
	add	ds:[di].OLWI_titleBarBounds.R_right, bp
...
if _JEDIMOTIF
        ;
        ;  Always move to back, to keep things around on heap.
        ;
        jmp     moveToBack
endif
ife ( _JEDIMOTIF or _DISABLE_APP_EXIT_UI )
	;
...
endif	; not	( _JEDIMOTIF or _DISABLE_APP_EXIT_UI )
...
if EVENT_MENU
	...
	;
	; push title bar right group over to make room for express menu
	;
	push	bx, si
	movdw	bxsi, ds:[di].OLWI_titleBarRightGroup
	tst	si
	jz	noRightGroup
if EVENT_MENU
	push	bp					; save event menu width
endif
	push	ax					; save title right
	mov	ax, MSG_VIS_GET_POSITION
	call	WinClasses_ObjMessageCallFixupDS	; cx, dx = cur position
	pop	ax					; ax, dx = new position
	push	ax
	cmp	ax, cx
	je	noMove
	mov	cx, ax					; cx, dx = new position
...
noMove:
	...
	call	WinClasses_ObjMessageCallFixupDS	; cx = width
	pop	ax					; ax = right group pos
if EVENT_MENU
	mov	dx, ax
	pop	bp					; bp = event width
	sub	dx, bp					; dx = event menu pos
endif
	add	ax, cx					; ax = express menu pos
noRightGroup:
	pop	bx, si
else
...
PMAN <	mov	ds, ax							>
PMAN <	test	ds:[moCS_flags], mask CSF_BW	; Is this a B&W display?>
PMAN <	pop	ds							>
PMAN <	jnz	20$				;   skip if so...	>
PMAN <	dec	cx				; Nest icon inside resize  >
PMAN <20$:					;   display		   >
...
endif	;not NO_EXPRESS_MENU_ON_PRIMARY
	ret
...
PMAN <	mov	bp, CUAS_GCM_HEADER_ICON_WIDTH-1			>
...
if _JEDIMOTIF
	;
	; check for ESCAPE on the way back up
	;
	...
	jne	notEscape
	;
	;  Simulate the exit button (MSG_GEN_DISPLAY_CLOSE)
	;
	mov	ax, MSG_GEN_DISPLAY_CLOSE
	mov	bx, ds:[LMBH_handle]
	mov	di, mask MF_FORCE_QUEUE		; (but do this after the window
...
if _USE_KBD_ACCELERATORS and not _JEDIMOTIF
	;
...
if RADIO_STATUS_ICON_ON_PRIMARY
	;
	; Check for F9 and shift-F9
	;
	test	dh, mask SS_LCTRL or mask SS_RCTRL or \
			mask SS_LALT or mask SS_RALT
	jnz	callSuper
SBCS <	cmp	cx, (VC_ISCTRL shl 8) or VC_F9				>
DBCS <	cmp	cx, C_SYS_F9						>
	jne	callSuper
	mov	bx, ds:[di].OLBWI_radioStatusIcon
	tst	bx
	jz	callSuper

	mov	ax, MSG_RADIO_STATUS_ICON_SHORT_TOUCH
	test	dh, mask SS_LSHIFT or mask SS_RSHIFT
	jz	10$
	mov	ax, MSG_RADIO_STATUS_ICON_LONG_TOUCH
10$:
	mov	si, bx			;*ds:si = RadioStatusIcon
	mov	bx, ds:[LMBH_handle]
