 ...
 * NOMBAS Changes *
 ******************/
/* Added #include "jseopt.h"         */
/* Added JSE_REGEXP_ANY around file  */
/* Renamed "pcre.c" to "regex.c"     */
/* Removed #include "internal.h"     */
/* Added #define JSE_REGEXP_CTABLES  */
/* define INT_MAX if not already defined */
/* Added (uschar) casts              */
/* min and max cannot be used as variable names,
 * conflict with macros on some systems. */
/* non-constant initialization gives warning on Solaris C,
 * so move it. i.e. int c = ++d; becomes int c; c = ++d;
 */

#include "jseopt.h"  /* NOMBAS */
#ifdef JSE_REGEXP_ANY /* NOMBAS */
...
/* #include "internal.h" NOMBAS - included with jseopt.h */
...
/* Nombas addition: if INT_MAX not defined then define it here */
#if !defined(INT_MAX)
#  define INT_MAX MAX_SINT
#endif
...
#define JSE_REGEXP_CTABLES /* NOMBAS */
#include "ctables.inc"
#undef JSE_REGEXP_CTABLES  /* NOMBAS */
...
while ((cd->ctypes[*p] & ctype_digit) != 0) minv = minv * 10 + *p++ - '0';
...
if (*p == '}') maxv = minv; else
  ...
  if (*(++p) != '}')
    {
    maxv = 0;
    while((cd->ctypes[*p] & ctype_digit) != 0) maxv = maxv * 10 + *p++ - '0';
    if (maxv < minv)
      {
      *errorptr = ERR4;
      return p;
...
/* NOMBAS addition:
 * If using watcom11 or borlanc under win16 or dos16 this check is always
 * false, so skip the check under these conditions.
 */
#if !( ( defined(__JSE_WIN16__) || defined(__JSE_DOS16__) || defined(__JSE_DOS32__) || defined(__JSE_GEOS__) ) && \
    ( defined(__BORLANDC__) || defined(__WATCOMC__) ) )
if (minv > 65535 || maxv > 65535)
  *errorptr = ERR5;
else
#endif
  {
    ...
    *code++ = OP_CLASS;
    ...
    do
      ...
      if (c == '\\')
        ...
        if (-c == ESC_b) c = '\b';
          ...
          switch (-c)
            ...
            case ESC_D:
            for (c = 0; c < 32; c++) class[c] |= (uschar)~cbits[c+cbit_digit];
            continue;
            ...
            case ESC_W:
            for (c = 0; c < 32; c++)
              class[c] |= (uschar)~(cbits[c+cbit_digit] | cbits[c+cbit_word]);
            continue;
            ...
            case ESC_S:
            for (c = 0; c < 32; c++) class[c] |= (uschar)~cbits[c+cbit_space];
            continue;
      ...
      if (ptr[1] == '-' && ptr[2] != ']')
        ...
        for (; c <= d; c++)
          {
          class[c/8] |= (uschar)(1 << (c&7));
          if ((options & PCRE_CASELESS) != 0)
            {
            int uc = cd->fcc[c];           /* flip case */
            class[uc/8] |= (uschar)(1 << (uc&7));
            }
    ...
    the code vector. */
      ...
      {
      if (negate_class)
        for (c = 0; c < 32; c++) code[c] = (uschar)~class[c];
      else
    ...
    repeats by adding a suitable offset into repeat_type. */
      ...
      if (repeat_min == 0)
        ...
        if (repeat_max == -1) *code++ = (uschar)(OP_STAR + repeat_type);
          ...
          *code++ = (uschar)(repeat_max >> 8);
          *code++ = (uschar)(repeat_max & 255);
          }
        }
      ...
      handled as an EXACT followed by an UPTO. An EXACT of 1 is optimized. */
        ...
        if (repeat_min != 1)
          ...
          *code++ = (uschar)(repeat_min >> 8);
          *code++ = (uschar)(repeat_min & 255);
          }
    ...
    cases. */
      ...
      register int i;
      int ketoffset = 0;
      size_t len = (size_t)(code - previous);
      uschar *bralink = NULL;
      ...
      if (repeat_min == 0)
        ...
        adjust the value or repeat_max, since one less copy is required. */
          ...
          *previous++ = OP_BRA;
          ...
          bralink = previous;
          *previous++ = (uschar)(offset >> 8);
          *previous++ = (uschar)(offset & 255);
          }
      ...
      if (repeat_max >= 0)
          ...
          *code++ = (uschar)(OP_BRAZERO + repeat_type);
          ...
          if (i != 0)
            ...
            *code++ = OP_BRA;
            ...
            bralink = code;
            *code++ = (uschar)(offset >> 8);
            *code++ = (uschar)(offset & 255);
            }
        ...
        while (bralink != NULL)
          ...
          *code++ = OP_KET;
          *code++ = bra[1] = (uschar)(offset >> 8);
          *code++ = bra[2] = (uschar)(offset & 255);
          }
        }
      ...
      else code[-ketoffset] = (uschar)(OP_KETRMAX + repeat_type);
      ...
      switch (*(++ptr))
        ...
        if (*ptr == ')')
          ...
          if ((options & PCRE_INGROUP) != 0 &&
            ...
            {
            *code++ = OP_OPT;
            *optchanged = newoptions & PCRE_IMS;
            *code++ = (uschar)(*optchanged);
            }
...
for (;;)
  ...
  last_branch[1] = (uschar)(length >> 8);
  last_branch[2] = (uschar)(length & 255);
  ...
  if (lookbehind)
    ...
    *code = OP_END;
    ...
    reverse_count[0] = (uschar)(length >> 8);
    reverse_count[1] = (uschar)(length & 255);
    }
  ...
  if (*ptr != '|')
    ...
    *code++ = OP_KET;
    *code++ = (uschar)(length >> 8);
    *code++ = (uschar)(length & 255);
    if (optchanged >= 0)
      {
      *code++ = OP_OPT;
      *code++ = (uschar)oldoptions;
      }
    *codeptr = code;
    *ptrptr = ptr;
    return TRUE;
  ...
  switch(c)
    ...
    if (c <= -ESC_REF)
      ...
      if (ptr[1] == '{' && is_counted_repeat(ptr+2, &compile_block))
        {
        ptr = read_repeat_counts(ptr+2, &minv, &maxv, errorptr, &compile_block);
        if (*errorptr != NULL) goto PCRE_ERROR_RETURN;
        if ((minv == 0 && (maxv == 1 || maxv == -1)) ||
          (minv == 1 && maxv == -1))
            length++;
        else length += 5;
        if (ptr[1] == '?') ptr++;
    ...
    ptr = read_repeat_counts(ptr+1, &minv, &maxv, errorptr, &compile_block);
    if (*errorptr != NULL) goto PCRE_ERROR_RETURN;
    if ((minv == 0 && (maxv == 1 || maxv == -1)) ||
      (minv == 1 && maxv == -1))
        length++;
      ...
      if (minv == 1) length++; else if (minv > 0) length += 4;
      if (maxv > 0) length += 4; else length += 2;
      }
    if (ptr[1] == '?') ptr++;
    continue;
...
/* NOMBAS addition:
 * If using watcom11 under win16 or dos16 this check is always
 * false, so skip the check under these conditions.
 */
#if !( ( defined(__JSE_WIN16__) || defined(__JSE_DOS16__) || defined(__JSE_DOS32__) || defined(__JSE_GEOS__) ) && \
    ( defined(__BORLANDC__) || defined(__WATCOMC__) ) )
...
size = length +
/* Nombas addition: msvc 1.52 doesn't like code[0] */
#if defined(_MSC_VER) && _MSC_VER == 800
   offsetof(real_pcre, code);
#else
   (int)offsetof(real_pcre, code[0]);
#endif

 re = jseMalloc(real_pcre,(size_t)size);
...
(void)compile_regex(options, -1, &bracount, &code, &ptr, errorptr, FALSE, -1,
  &compile_block);
re->top_bracket = (uschar)bracount;
re->top_backref = (uschar)top_backref;
  ...
  if (*code >= OP_BRA)
    ...
    case OP_CLASS:
      ...
      switch(*code)
        ...
        case OP_CRRANGE:
        case OP_CRMINRANGE:
        minv = (code[1] << 8) + code[2];
        maxv = (code[3] << 8) + code[4];
        if (maxv == 0) printf("{%d,}", minv);
        else printf("{%d,%d}", minv, maxv);
  ...
  switch(op)
    ...
    case OP_REF:
      ...
      switch (*ecode)
        ...
        c = *ecode++ - OP_CRSTAR;
        minimize = (c & 1) != 0;
        minv = rep_min[c];                 /* Pick up values from tables; */
        maxv = rep_max[c];                 /* zero for max => infinity */
        if (maxv == 0) maxv = INT_MAX;
        break;

        case OP_CRRANGE:
        case OP_CRMINRANGE:
        minimize = (*ecode == OP_CRMINRANGE);
        minv = (ecode[1] << 8) + ecode[2];
        maxv = (ecode[3] << 8) + ecode[4];
        if (maxv == 0) maxv = INT_MAX;
        ecode += 5;
        break;
      ...
      if (minimize)
        ...
        for (i = minv;; i++)
          ...
          if (match(eptr, ecode, offset_top, md, ims, FALSE, eptrb))
            return TRUE;
          if (i >= maxv || !match_ref(offset, eptr, length, md, ims))
            return FALSE;
          eptr += length;
          }
        /* Control never gets here */
    ...
    case OP_CLASS:
      ...
      /* If maxv == minv we can continue with the main loop without the
      need to recurse. */
      ...
      if (minimize)
        ...
        for (i = minv;; i++)
          ...
          if (match(eptr, ecode, offset_top, md, ims, FALSE, eptrb))
            return TRUE;
          if (i >= maxv || eptr >= md->end_subject) return FALSE;
          c = *eptr++;
          if ((data[c/8] & (1 << (c&7))) != 0) continue;
          return FALSE;
          }
    ...
    REPEATCHAR:
    if (minv > md->end_subject - eptr) return FALSE;
    c = *ecode++;
    ...
    /* Caseful comparisons */
      ...
      {
      for (i = 1; i <= minv; i++) if (c != *eptr++) return FALSE;
      if (minv == maxv) continue;
      if (minimize)
        {
        for (i = minv;; i++)
          ...
          if (match(eptr, ecode, offset_top, md, ims, FALSE, eptrb))
            return TRUE;
          if (i >= maxv || eptr >= md->end_subject || c != *eptr++) return FALSE;
          }
        /* Control never gets here */
        }
      else
    ...
    if (minv > md->end_subject - eptr) return FALSE;
    if (minv > 0) switch(ctype)
      {
      case OP_ANY:
      if ((ims & PCRE_DOTALL) == 0)
        { for (i = 1; i <= minv; i++) if (*eptr++ == '\n') return FALSE; }
      else eptr += minv;
      break;
    ...
    /* If minv = maxv, continue at the same level without recursing */
...
if (re->top_backref > 0 && re->top_backref >= ocount/3)
  ...
  match_block.offset_vector = jseMalloc(int,ocount * sizeof(int));
...
do
  ...
  if (using_temporary_offsets)
    ...
    jseMustFree(match_block.offset_vector);
    }
...
size_t
ecma_regerror(int errcode, const regex_t *preg, char *errbuf, size_t errbuf_size)
{
...
void
ecma_regfree(regex_t *preg)
{
  jseMustFree(preg->re_pcre);
}
...
int
ecma_regcomp(regex_t *preg, const char *pattern, int cflags)
{
   const char *errorptr;
   int erroffset;
   int options = 0;
   ...
   preg->re_erroffset = (size_t)erroffset;

   if (preg->re_pcre == NULL) return pcre_posix_error_code(errorptr);

   preg->re_nsub = (size_t)pcre_info(preg->re_pcre, NULL, NULL);
   return 0;
}
...
int
ecma_regexec(regex_t *preg, const char *string, size_t nmatch,
...
if (rc > 0)
  {
  size_t i;
  for (i = (size_t)rc; i < nmatch; i++) pmatch[i].rm_so = pmatch[i].rm_eo = -1;
  return 0;
  }
