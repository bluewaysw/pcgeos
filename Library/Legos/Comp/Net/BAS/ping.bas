sub duplo_ui_ui_ui()
 REM		$Id$
 REM	Copyright (c) New Deal 1999 -- All Rights Reserved
 REM	FILE:		STDINC.BH
...
Clear2.name="Clear2"
Dim Ping as button
Ping = MakeComponent("button",group2)
CompInit Ping
proto="Ping"
caption="Ping the host!"
sizeHControl=3
sizeVControl=3
visible=1
End CompInit
...
sub Ping_pressed(self as button)

'	DIM filename AS STRING
'	filename = filesel1.filename

	DIM hostname AS STRING
	hostname = "www.newdealinc.com"

	DIM socket AS COMPONENT
	socket = MakeComponent("telnet","top")
	socket.host = hostname
	socket.port = 7				' 7 = echo port
	socket.trap = 0				' 2 = verbose error dialogs
   	out.AppendString("\rPinging ",hostname,"...\r")
	socket!open()
	if (socket.error = 0) then
	   out.AppendString("   Host ",hostname," responding!...\r")
	else
	   out.AppendString("   Socket error: ",Str(socket.error),"\r")
	end if
	socket!close()
end sub
