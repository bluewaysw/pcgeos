...
DESCRIPTION:
	...
	$Id: cwinDisplayControl.asm,v 1.1 97/04/07 10:53:13 newdeal Exp $
...
afterMax:
	...
	;since we custom manage children, we have to always recalc size,
	;because we don't mark ourselves invalid, so the normal geometry
	;update will not work for us; fail case: drag resize a primary with
	;a display group to the same size as the original -- brianc 10/15/99
	ORNF	ds:[di].VI_geoAttrs, mask VGA_USE_VIS_SET_POSITION \
				  or mask VGA_NOTIFY_GEOMETRY_VALID \
				  or mask VGA_ALWAYS_RECALC_SIZE
