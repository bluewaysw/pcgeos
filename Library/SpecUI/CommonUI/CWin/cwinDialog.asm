...
DESCRIPTION:
	...
	$Id: cwinDialog.asm,v 1.2 98/03/11 06:07:56 joon Exp $
...
if _DUI
	...
	;
	; ISUI modal dialogs have titlebars, sysmenus, and are movable.
	;
...
doMonikers:
if not _ISUI
	; ISUI: help button is in the title bar
	;
...
if _ISUI ; If trigger is IC_DISMISS, then make dialog not closable ------
	cmp	cx, IC_DISMISS
	jne	afterICDismiss
	call	WinDialog_DerefVisSpec_DI
	ANDNF	ds:[di].OLWI_attrs, not mask OWA_CLOSABLE
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	brianc	1/21/92		Initial version

------------------------------------------------------------------------------@

if _ISUI		; ESC closes dialogs with single reply button
...
OLDialogWinFupKbdChar	method dynamic	OLDialogWinClass, \
	...
	;
	; If ESC, activate single reply bar trigger
	;
SBCS <	cmp	cx, (CS_CONTROL shl 8) or VC_ESCAPE	;ESC key?	>
DBCS <	cmp	cx, C_SYS_ESCAPE			;ESC key?	>
	jne	callSuper

	mov	di, ds:[si]			
	add	di, ds:[di].Vis_offset
	mov	di, ds:[di].OLDWI_triggerList
	tst	di
	jz	callSuper
	...
	clr	ax
	call	ChunkArrayElementToPtr		; ds:di = element
	cmp	ds:[di].NOICS_ic, IC_OK
	jne	popCallSuper
	pop	ax, ax, ax			; clean up stack
	movdw	bxsi, ds:[di].NOICS_optr
...
OLDialogWinAlterFTVMCExcl method dynamic OLDialogWinClass,
	...
	;
if not _ISUI  ; it seems we need this for modals also (e.g. sys tray dialogs)
