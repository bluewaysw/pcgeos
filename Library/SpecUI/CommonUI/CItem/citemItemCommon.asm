...
ROUTINES:
 ...
 ----			-----------
    ...
    INT ItemDrawISUICheck	Draw a simple check to represent radio
...
DESCRIPTION:
	$Id: citemItemCommon.asm,v 1.4 98/05/04 06:59:03 joon Exp $
...
12$:
	...
	;to hooey up the intermediate mode which was started by another item.)

	test	bp, mask OIENPF_MAINTAIN_INTERMEDIATE_MODE
	jnz	done

	;Motif, CUA/ISUI: exclusive items in menu can show SELECTION
...
OLItemEnsureIsPressed	method	OLItemClass,
	...
	;Motif and CUA/ISUI: exclusive items in menu can show
	;SELECTED emphasis independently of DEPRESSED emphasis, so no need
	;for Intermediate mode. This item will be drawn as DEPRESSED(CUA/ISUI)
...
CALLED BY:	ItemDrawColorISUIRadioButton
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	gene	2/25/02		Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if _ISUI	;--------------------------------------------------------------

ItemDrawISUICheck	proc	far
...
setColor:
	...
	test	bh, mask OLBSS_SELECTED			; nothing to draw if
	jz	done					;  not selected
...
OLItemGroupTabHints	VarDataHandler \
	 ...
	 <HINT_BOTTOM_JUSTIFY_CHILDREN, \
 			 offset ItemVeryCommon:OLItemGroupTabHintBottom>
endif
...
elseif (_MOTIF or _ISUI)
OLItemGroupTabHintBottom	proc	far
	andnf	dh, not (mask OLIER_TAB_STYLE)
	ornf	dh, OLITS_TAB_BOTTOM
	ret
OLItemGroupTabHintBottom	endp
endif
...
10$:	;See if this object should be drawn as SELECTED.
	...
	jnz	15$			;skip if so...
12$:
	;if ISUI and IS_SCROLL_ITEM, then we ARE using intermediate mode -
	; even in menus.
...
considerDepressedExcl:
	...
	;
	;ISUI:		in menu:	can show depressed emphasis
