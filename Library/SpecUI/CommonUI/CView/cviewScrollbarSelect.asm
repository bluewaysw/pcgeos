...
DESCRIPTION:
	...
	$Id: cviewScrollbarSelect.asm,v 1.2 98/03/11 06:03:10 joon Exp $
...
if _CUA_STYLE		;START of MOTIF/ISUI/CUA specific code -----
...
ISU<	mov	ax, MO_SCR_AREA_MARGIN		;assume a normal scrollbar>
...
ISU <	test	ds:[di].OLSBI_attrs, mask OLSA_TWISTED			>
ISU <	jz	10$				;not twisted, branch	>
ISU <	mov	ax, NUI_SPIN_ARROW_HEIGHT+2	;else use height	>
ISU <10$:								>
...
ISU <	cmp	ds:[di].OLSBI_elevOffset, NO_THUMB_ELEV_OFFSET		>
	je	skipCheck
...
endif		;END of MOTIF/ISUI/CUA specific code --------------------
...
5$:
	test	ds:[di].OLSBI_attrs, mask OLSA_TWISTED
	jz	10$				;not twisted, branch
	mov	bx, NUI_SPIN_ARROW_HEIGHT+2
...
ISU <	sub	bx, ds:[di].OLSBI_elevLen    				   >
...
ISU <	mov	di, C_LIGHT_GREY		;lt-grey to black, dk-grey to >
...
ISU <	mov	ds, ax							>
ISU <	test	ds:[moCS_flags], mask CSF_BW				>
ISU <	pop	ds							>
ISU <	jz	setColor			;color, branch		      >
	mov	di, C_WHITE			;use white in B/W
ISU <setColor:								      >
...
ISU<	add	bx, MO_SCR_AREA_MARGIN		;add arrow height for thumb top>
...
ISU <	add	dx, ds:[di].OLSBI_elevLen				     >
...
ISU<	add	ax, MO_SCR_AREA_MARGIN		;add arrow height for thumb top>
...
ISU <	add	cx, ds:[di].OLSBI_elevLen				    >
