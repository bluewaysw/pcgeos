...
DESCRIPTION:
	...
	$Id: copenMoniker.asm,v 1.2 98/03/11 06:00:56 joon Exp $
...
ISU <	test	cx, mask OLMA_DARK_COLOR_BACKGROUND			>
ISU <	jnz	2$							>
...
if _ISUI
CheckIfInPinnedMenu	proc	near
	uses	bx, di
	.enter
	call	SwapLockOLWin			; lock parent win
	jnc	done				; no parent, not in pinned menu
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	.warn -private
	mov	di, {word}ds:[di].OLWI_specState
	.warn @private
	call	ObjSwapUnlock			; unlock parent win
	test	di, mask OLWSS_PINNED
	jz	done				; not pinned, C clear
	stc					; else, is pinned menu, C set
done:
	.leave
	ret
CheckIfInPinnedMenu	endp
endif
...
drawInsideBounds:
	...
	;
	; For ISUI we need to shrink the selection rectangle a bit so it
	; doesn't extend over the down arrow button.
	;
ISU <	test	ss:[bp].OMA_monikerAttrs, mask OLMA_DISP_DOWN_ARROW	>
ISU <	jz	haveLineBounds	   	       ;not doing arrows, branch>
ISU <	sub	cx, OL_DOWN_MARK_WIDTH + OL_MARK_SPACING - 1		>
ISU <haveLineBounds:							>
...
60$:
	...
	;Motif/ISUI: if on black background, OFF means C_BLACK
...
if _ISUI
    ...
    push	es, di, si
	clr	ax
   	call	OpenGetBackgroundColorFar	; ax = bg colors
	push    ax
    ...
    pop     ax
	pop	es, di, si
	jc	haveColor			; if scrollable item, use
						;	default bg color
						; else, use wash color
endif

	;Motif/ISUI: if on light background, OFF means C_LIGHT_GREY
	...
	;Motif/ISUI: if on dark background, OFF means C_DARK_GREY
