...
DESCRIPTION:
	...
	$Id: copenUtils.asm,v 1.2 98/03/11 06:00:59 joon Exp $

-------------------------------------------------------------------------------@

Resident segment resource

GetDarkColor	proc	far
	uses	ds
	.enter
...
SpecGetDisplayScheme	proc	far		uses ds
	...
	mov	bx, {word} ds:[specDisplayScheme.DS_lightColor]
...
ROUTINE:	OpenCheckIfDisplayLarge

SYNOPSIS:	Checks to see if we're on at least SVGA or higher.

CALLED BY:	utility

PASS:		nothing

RETURN:		carry set if SVGA+
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	MeyerK	05/13/2024	Initial version

------------------------------------------------------------------------------@
OpenCheckIfDisplayLarge	proc	far
	uses ax, ds
	.enter

	mov	ax, segment idata		;get segment of core blk
	mov	ds, ax
	mov	ah, ds:[moCS_displayType]
	and	ah, mask DT_DISP_SIZE
;checkLarge:
	cmp	ah, DS_LARGE shl offset DT_DISP_SIZE
	je	exitWithCarrySet
checkHuge:
	cmp	ah, DS_HUGE shl offset DT_DISP_SIZE
	je	exitWithCarrySet
...
OpenCheckIfTV	proc	far
	uses	ax, ds
	.enter
	mov	ax, segment dgroup
	mov	ds, ax
	test	ds:[moCS_flags], mask CSF_TV
	jz	exit
	stc
exit:
	.leave
...
------------------------------------------------------------------------------@
OpenGetBackgroundColorFar	proc	far

    call OpenGetBackgroundColor
    ret

OpenGetBackgroundColorFar	endp
...
if _ISUI
	...
	mov	dx, cx				;else use same as unselected
endif
...
bubbleColor	local	ColorQuad
	uses	ax,bx,cx,si,di,bp
	.enter

	; get bubble color

	call	VisCheckIfFullyEnabled		; carry set if so
	mov	bubbleColor.CQ_info, CF_RGB
	mov	bubbleColor.CQ_redOrIndex, 250
	mov	bubbleColor.CQ_green, 240
	mov	bubbleColor.CQ_blue, 185
	jc	haveColor
	mov	bubbleColor.CQ_redOrIndex, 185
...
createRegions:
	...
	mov	cl, ss:[bubbleColor].CQ_redOrIndex
	mov	ch, ss:[bubbleColor].CQ_info
	mov	dl, ss:[bubbleColor].CQ_green
	mov	dh, ss:[bubbleColor].CQ_blue
	...
	mov	bp, si			; expose OD
	clrdw	cxdx			; mouse OD
	mov	si, WIN_PRIO_POPUP-1	; WinOpenFlags - above everything
	call	WinOpen
	pop	bp, si
...
afterXlat:
	...
	mov	di, bx
	call	TimerGetCount
	movdw	ds:[di].BHD_startTime, bxax
	pop	bx
	;
	; start timer
	;
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OpenCheckBubbleMinTime
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Check if bubble minimum time threshold is reached

CALLED BY:	GLOBAL
PASS:		*ds:si	= object with
		ax = BubbleHelpData vardata
RETURN:		carry set if within bubble minimum time
DESTROYED:	nothing
...
OpenCheckBubbleMinTime	proc	far
	uses	ax, bx, cx, dx, es
	.enter
	call	ObjVarFindData
	jnc	done				; return not within min time
	movdw	dxcx, ds:[bx].BHD_startTime
	call	TimerGetCount
	subdw	bxax, dxcx
	cmp	bx, 0
	jne	checkLow
	segmov	es, dgroup, bx
	cmp	ax, es:[olBubbleHelpMinTime]
checkLow:
	jle	withinMin
	clc					; not within min time
	jmp	short done
...
SYNOPSIS:	open help menu

CALLED BY:	INTERNAL
PASS:		*ds:si = object
		cx, dx = mouse position to open at
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	gene	2/5/02		Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if _ISUI

ShowHelpMenu	proc	far
	uses	ax, bx, cx, dx, si, di, bp
	.enter
	;
	; send bring up help for focus
	;
	clr	bx, si
...
locs	local	GradientLocals
	...
	;
		...
		mov	dh, al				;dh <- end R
		lea	si, ss:locs.GL_red
		call	CalcStep
		mov	dl, cl				;dl <- start G
		mov	dh, bl				;dh <- end G
		lea	si, ss:locs.GL_green
		call	CalcStep
		mov	dl, ch				;dl <- start B
		mov	dh, bh				;dh <- end B
		lea	si, ss:locs.GL_blue
		call	CalcStep
	;
	; set up the starting X pos
	;
		mov	ax, ss:locs.GL_bounds.R_left
		mov	ss:locs.GL_curX, ax

drawLoop:
		mov	al, ss:locs.GL_red.CAS_cur.WBF_int.low
		mov	bl, ss:locs.GL_green.CAS_cur.WBF_int.low
		mov	bh, ss:locs.GL_blue.CAS_cur.WBF_int.low
		mov	ah, CF_RGB
		call	GrSetAreaColor

		mov	ax, ss:locs.GL_curX
		mov	cx, ax
		add	cx, GRADIENT_STEP_SIZE+1

		mov	bx, ss:locs.GL_bounds.R_top
		mov	dx, ss:locs.GL_bounds.R_bottom
		call	GrFillRect
	;
	; advance to next color and position
	;
		add	ss:locs.GL_curX, GRADIENT_STEP_SIZE
		addwbf	ss:locs.GL_red.CAS_cur, ss:locs.GL_red.CAS_step, ax
		addwbf	ss:locs.GL_green.CAS_cur, ss:locs.GL_green.CAS_step, ax
		addwbf	ss:locs.GL_blue.CAS_cur, ss:locs.GL_blue.CAS_step, ax
	;
	; reached right edge?
	;
		mov	ax, ss:locs.GL_curX
		add	ax, GRADIENT_STEP_SIZE
		cmp	ax, ss:locs.GL_bounds.R_right
		jb	drawLoop

		sub	ax, GRADIENT_STEP_SIZE
		mov	cx, ss:locs.GL_bounds.R_right
		call	GrFillRect
...
CalcStep	proc	near
		...
		mov	bx, ss:locs.GL_bounds.R_right
		sub	bx, ss:locs.GL_bounds.R_left	;bx.ax <- rect. width
		call	GrSDivWWFixed
		jnc	gotValue
zeroStep:
		clr	dx, cx
