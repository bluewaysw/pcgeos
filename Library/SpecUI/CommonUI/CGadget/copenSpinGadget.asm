...
ClearSpinDisplay	proc	near
	...
	mov	al, bl				;al <- light color
	mov	bl, bh				;bl <- dark color
	clr	ah
clear:
	pop	di				;restore gstate
...
ClearSpinDisplay	proc	near
	...
	call	GetDarkColor
	mov	ss:ltColor, ax			;store as left/top color
	mov	ss:rbColor, C_WHITE		;store right/bottom color

	call	GetLightColor			;else use light color
clear:
	call	GrSetAreaColor			;set area color
	mov	ax, ss:ltColor			;and black for the frame     
	call	GrSetLineColor			;set line color		    
...
exitWithPrePassive:
	...
	push	ax, cx, dx, bp
if BUBBLE_HELP
	ornf	ds:[di].OLSGI_states, mask OLSGS_TOOLBOX_PRE_PASSIVE
	test	ds:[di].OLSGI_states, mask OLSGS_BUBBLE_PRE_PASSIVE
	jnz	alreadyHavePrePassive
endif
	call	VisAddButtonPrePassive
alreadyHavePrePassive::
	pop	ax, cx, dx, bp
	jmp	short exit
...
OLSpinGadgetPrePassiveButton	method dynamic	OLSpinGadgetClass, \
	...
	;
	; stop bubble help if press
	;
if BUBBLE_HELP
	test	bp, mask BI_PRESS
	jz	done
	call	OLSpinGadgetDestroyBubbleHelp
	;
	; prevent re-opening of bubble help
	;
	push	bx, cx, di
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP_JUST_CLOSED
	clr	cx
	call	ObjVarAddData
	mov	ax, MSG_META_DELETE_VAR_DATA
	mov	cx, TEMP_SPEC_SPIN_BUBBLE_HELP_JUST_CLOSED
	mov	bx, ds:[LMBH_handle]
...
OLSpinGadgetPrePassiveStartSelect	method dynamic	OLSpinGadgetClass, \
	...
	;
if BUBBLE_HELP
	andnf	ds:[di].OLSGI_states, not mask OLSGS_TOOLBOX_PRE_PASSIVE
	test	ds:[di].OLSGI_states, mask OLSGS_BUBBLE_PRE_PASSIVE
	jnz	leavePrePassive
endif
...
if BUBBLE_HELP
	mov	di, ds:[si]
	add	di, ds:[di].OLSpinGadget_offset
	andnf	ds:[di].OLSGI_states, not mask OLSGS_TOOLBOX_PRE_PASSIVE
	test	ds:[di].OLSGI_states, mask OLSGS_BUBBLE_PRE_PASSIVE
	jnz	leavePrePassive
endif
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetStartMoveCopy	method	dynamic	OLSpinGadgetClass,
				MSG_META_START_MOVE_COPY
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP_JUST_CLOSED
	call	ObjVarDeleteData
	jnc	callsuper		; found and deleted
	call	VisGrabMouse
	call	OLSpinGadgetCreateBubbleHelp
callsuper:
	mov	di, offset OLSpinGadgetClass
	GOTO	ObjCallSuperNoLock
		
OLSpinGadgetStartMoveCopy	endm


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetEndMoveCopy
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetEndMoveCopy	method	dynamic	OLSpinGadgetClass,
				MSG_META_END_MOVE_COPY
	push	ax
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP
...
OLSpinGadgetEndMoveCopy	endm


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetVisClose
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetVisClose	method dynamic OLSpinGadgetClass, MSG_VIS_CLOSE
	push	ax
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP
	call	ObjVarFindData
	pop	ax
	...
	mov	di, offset OLSpinGadgetClass
	GOTO	ObjCallSuperNoLock
OLSpinGadgetVisClose	endm


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetCreateBubbleHelp
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Create bubble help

CALLED BY:	OLSpinGadgetGainedSystemFocusExcl
PASS:		*ds:si	= OLSpinGadgetClass object
RETURN:		ds:di	= OLSpinGadgetClass instance data
DESTROYED:	nothing
SIDE EFFECTS:	
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetCreateBubbleHelp	proc	far
	...
	mov	dx, MSG_SPEC_SPIN_BUBBLE_DELAY_TIME_OUT
...
noDelay:
	...
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP
	mov	bx, MSG_SPEC_SPIN_BUBBLE_TIME_OUT
	call	OpenCreateBubbleHelp

	mov	di, ds:[si]
	add	di, ds:[di].OLSpinGadget_offset
	ornf	ds:[di].OLSGI_states, mask OLSGS_BUBBLE_PRE_PASSIVE
	test	ds:[di].OLSGI_states, mask OLSGS_TOOLBOX_PRE_PASSIVE
	jnz	alreadyHavePrePassive
	call	VisAddButtonPrePassive
alreadyHavePrePassive:
...
OLSpinGadgetCreateBubbleHelp	endp

SpinFindBubbleHelp	proc	far
	push	si, di
	mov	di, ds:[si]
	add	di, ds:[di].Vis_offset
	test	ds:[di].VI_typeFlags, mask VTF_IS_GEN
	jz	gotFocusHelp			; carry clear -- no help
	mov	ax, ATTR_GEN_FOCUS_HELP
	test	ds:[di].VI_attrs, mask VA_FULLY_ENABLED
	jnz	haveFocusHelpType
...
gotFocusHelp:
	pop	si, di
	ret
SpinFindBubbleHelp	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetCreateBubbleHelpLow
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Really Create bubble help

CALLED BY:	OLSpinGadgetCreateBubbleHelp, OLSpinGadgetBubbleDelayTimeOut
PASS:		*ds:si	= OLSpinGadgetClass object
RETURN:		ds:di	= OLSpinGadgetClass instance data
DESTROYED:	nothing
SIDE EFFECTS:	
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetCreateBubbleHelpLow	proc	near
	uses	ax,bx,cx,dx,es,bp
	.enter
...
alreadyHavePrePassive:
	...
	.leave
	ret
OLSpinGadgetCreateBubbleHelpLow	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetDestroyBubbleHelp
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Destroy bubble help

CALLED BY:	OLSpinGadgetLostSystemFocusExcl, OLMenuButtonLostFocusExcl
PASS:		*ds:si	= OLSpinGadgetClass object
RETURN:		ds:di	= OLSpinGadgetClass instance data
DESTROYED:	nothing
SIDE EFFECTS:	
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetDestroyBubbleHelp	proc	far
	uses	ax,bx
	.enter

	mov	di, ds:[si]
	add	di, ds:[di].OLSpinGadget_offset
	andnf	ds:[di].OLSGI_states, not mask OLSGS_BUBBLE_PRE_PASSIVE
	test	ds:[di].OLSGI_states, mask OLSGS_TOOLBOX_PRE_PASSIVE
	jnz	leavePrePassive
	call	VisRemoveButtonPrePassive
leavePrePassive:
	...
	.leave
	ret
OLSpinGadgetDestroyBubbleHelp	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetBubbleTimeOut
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	bubble time out, close bubble

CALLED BY:	MSG_SPEC_SPIN_BUBBLE_TIME_OUT
PASS:		*ds:si	= OLSpinGadgetClass object
		ds:di	= OLSpinGadgetClass instance data
		ds:bx	= OLSpinGadgetClass object (same as *ds:si)
		es 	= segment of OLSpinGadgetClass
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetBubbleTimeOut	method dynamic OLSpinGadgetClass, 
					MSG_SPEC_SPIN_BUBBLE_TIME_OUT
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP
	call	ObjVarFindData
	jnc	done
	...
	ret
OLSpinGadgetBubbleTimeOut	endm


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetBubbleDelayTimeOut
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	bubble delay time out, open bubble

CALLED BY:	MSG_SPEC_SPIN_BUBBLE_DELAY_TIME_OUT
PASS:		*ds:si	= OLSpinGadgetClass object
		ds:di	= OLSpinGadgetClass instance data
		ds:bx	= OLSpinGadgetClass object (same as *ds:si)
		es 	= segment of OLSpinGadgetClass
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetBubbleDelayTimeOut	method dynamic OLSpinGadgetClass, 
					MSG_SPEC_SPIN_BUBBLE_DELAY_TIME_OUT
	mov	ax, TEMP_SPEC_SPIN_BUBBLE_HELP
	call	ObjVarFindData
	jnc	done
	mov	ds:[bx].BHD_timer, 0
	call	OLSpinGadgetCreateBubbleHelpLow
done:
	ret
OLSpinGadgetBubbleDelayTimeOut	endm

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		OLSpinGadgetMetaExposed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
OLSpinGadgetMetaExposed	method dynamic OLSpinGadgetClass, 
					MSG_META_EXPOSED
	mov	di, cx
	call	GrCreateState
	call	GrBeginUpdate
