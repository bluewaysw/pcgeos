...
DESCRIPTION:
	...
	$Id: copenMenuButton.asm,v 1.4 98/03/18 01:10:43 joon Exp $
...
if _ISUI
	ORNF	ds:[di].OLBI_moreAttrs, mask OLBMA_CENTER_MONIKER
endif
	ret
...
EC <	ERROR_NZ OL_ERROR						   >
	...
	;
	; pretend we are still interacting with the menu button when
	; we are obscured by our menu, to fix problems where the menu will
	; get the mouse release and immediately close
	;
	test	bp, (mask OLMBFA_IN_BOUNDS) shl 8
	jnz	inBounds		; already in bounds
	call	VisTestPointInBounds
	jnc	noForceInBounds		; not in menu button bounds
	push	ax, cx, dx, bp
	mov	ax, MSG_OL_APP_GET_IMPLIED_WIN
	call	UserCallApplication	; no lmem-movement
	cmp	cx, ds:[LMBH_handle]
	jne	haveMenuCheck
	cmp	dx, ds:[di].OLBI_genChunk
haveMenuCheck:
	pop	ax, cx, dx, bp
	jne	noForceInBounds		; not our menu window
	ornf	bp, (mask OLMBFA_IN_BOUNDS) shl 8
noForceInBounds:
...
ISU   <	call	OLMenuButtonEnsureIsDrawnDepressedNOW			>
NOT_MO<	call	OLMenuButtonEnsureIsDrawnHollow				>
...
if not _CASCADING_MENUS  ; if cascading, we grab gadget excl and let
			 ; cascading feature keep higher-level menus open
...
if 0
	;
	; ND-000115.  Keep field-relative.  Making it parent-relative
	; is wrong according to OLMenuWinActivate(), which says it wants
	; things field-relative.  The result of making it parent-relative
	; is that the system menu for overlapping multiple document
	; windows (move, size, next, etc.) appears in strange places,
	; most commonly over the window button that opened it.
	;
	push	ax, si
...
OLMenuButtonSaveStateSetCursored	proc	near
	...
	;Why
ISU <	test	ds:[di].OLMBI_specState, mask OLMBSS_OPENS_POPUP_LIST	>
ISU <	jnz	done							>
...
OLMenuButtonUpdateVisMoniker	method dynamic	OLMenuButtonClass, \
	...
	jz	markInvalid
	test	ds:[di].OLBI_specState, mask OLBSS_MENU_DOWN_MARK
	jz	markInvalid		; no menu mark, inval whole thing
	;
	; Opening popup lists, we'll painfully only the moniker area to the
	; inval region, to avoid the border being redrawn.  We'll still
	; mark the geometry invalid, in case it needs to be resized.
