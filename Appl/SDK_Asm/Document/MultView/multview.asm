...
DESCRIPTION:
	This file source code for the MultView application. This code will
...
RCS STAMP:
	$Id: multview.asm,v 1.1 97/04/04 16:33:09 newdeal Exp $
...
MVProcessClass	class	GenProcessClass
...
idata	segment
	MVProcessClass	mask CLASSF_NEVER_SAVED
idata	ends
...
MVDocumentClass	class	GenDocumentClass

MVDocumentClass endc
...
include		multview.rdef		;include compiled UI definitions

;------------------------------------------------------------------------------
;		Code for MVProcessClass
;------------------------------------------------------------------------------
...
MESSAGE:	MVDocumentRecalcSize -- MSG_VIS_RECALC_SIZE for MVDocumentClass

DESCRIPTION:	Calculate and return our size

PASS:
	*ds:si - instance data
	es - segment of MVDocumentClass
...
------------------------------------------------------------------------------@
MVDocumentRecalcSize	method dynamic	MVDocumentClass, MSG_VIS_RECALC_SIZE
	mov	cx, DOCUMENT_WIDTH
	mov	dx, DOCUMENT_HEIGHT
	ret

MVDocumentRecalcSize	endm

COMMENT @----------------------------------------------------------------------

MESSAGE:	MVDocumentInitializeDocumentFile --
...
------------------------------------------------------------------------------@
MVDocumentInitializeDocumentFile	method dynamic	MVDocumentClass,
				MSG_GEN_DOCUMENT_INITIALIZE_DOCUMENT_FILE

	; Handle this message by creating our version of an empty
	; document, which is a map block with the MVMapBlock structure
	...
	mov	ds, ax				;ds = block
	mov	ds:MVMB_xPos, INITIAL_X_POS
	mov	ds:MVMB_yPos, INITIAL_Y_POS
...
MVDocumentInitializeDocumentFile	endm

COMMENT @----------------------------------------------------------------------

MESSAGE:	MVDocumentDraw -- MSG_VIS_DRAW for MVDocumentClass
...
------------------------------------------------------------------------------@
MVDocumentDraw	method dynamic	MVDocumentClass, MSG_VIS_DRAW
...
MESSAGE:	MVDocumentStartSelect -- MSG_META_START_SELECT for MVDocumentClass
...
------------------------------------------------------------------------------@
MVDocumentStartSelect	method dynamic	MVDocumentClass, MSG_META_START_SELECT
