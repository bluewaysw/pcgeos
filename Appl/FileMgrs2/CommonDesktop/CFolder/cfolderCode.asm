...
BuildDiskAndPathName	proc	near
	...
	.enter
if _NEWDESK
	; NewDesk folder headers use friendly names for standard paths.
	call	BuildPrettyDiskAndPathNameFromVarData
else
	mov	ax, ATTR_FOLDER_PATH_DATA
	mov	dx, TEMP_FOLDER_SAVED_DISK_HANDLE
...
if _NEWDESK
BuildPrettyFolderPathVars	struct
	BPPFV_destSegment	word
	BPPFV_destOffset	word
	BPPFV_tailOffset	word
	BPPFV_prefixOffset	word
	BPPFV_prefixLength	word
	BPPFV_standardPath	word
BuildPrettyFolderPathVars	ends

BuildPrettyDiskAndPathNameFromVarData	proc	near
	uses	ax, bx, cx, dx, si, di, ds, es, bp

vars	local	BuildPrettyFolderPathVars
pathBuffer	local	PathName

	.enter
	; Build the actual path separately so the normalized display can be
	; written to the caller's buffer without changing the source path.
	mov	ss:[vars].BPPFV_destSegment, es
	mov	ss:[vars].BPPFV_destOffset, di
	...
	clr	bx
	call	FileParseStandardPath
	mov	ss:[vars].BPPFV_standardPath, ax
	mov	ss:[vars].BPPFV_tailOffset, di

	GetResourceHandleNS	FileOpDialogStrings, bx
	call	MemLock
	mov	ds, ax
	mov	ax, ss:[vars].BPPFV_standardPath
	...
	mov	di, ss:[vars].BPPFV_tailOffset
...
setPrefix:
	mov	ss:[vars].BPPFV_prefixOffset, si
	mov	bp, si
	call	FolderGetStringLength
	mov	ss:[vars].BPPFV_prefixLength, cx
	mov	di, ss:[vars].BPPFV_tailOffset
	cmp	{TCHAR}es:[di], 0
	jne	copyPrefix
	dec	cx
...
copyPrefix:
	mov	es, ss:[vars].BPPFV_destSegment
	mov	di, ss:[vars].BPPFV_destOffset
	mov	cx, ss:[vars].BPPFV_prefixLength
	mov	si, ss:[vars].BPPFV_prefixOffset
	LocalCopyNString
	segmov	ds, ss, si
	mov	si, ss:[vars].BPPFV_tailOffset
	LocalCopyString
	jmp	unlockStrings

noPrefix:
	segmov	ds, ss, si
	lea	si, ss:[pathBuffer]
	mov	es, ss:[vars].BPPFV_destSegment
	mov	di, ss:[vars].BPPFV_destOffset
	LocalCopyString
...
unlockStrings:
	...
	.leave
	ret
BuildPrettyDiskAndPathNameFromVarData	endp

FolderGetStringLength	proc	near
	uses	ax, di, es
	.enter
	segmov	es, ds, di
	mov	di, bp
