...
ROUTINES:
	Name			Description
	----			-----------
    INT SetupForMerge		This sets up for merging by updating the
				file on disk and loading up the SSMetaStruc
				needed by later merge code.

    INT SetupMergeFeedbackString 
				Set up a merge-feedback string.

    INT CleanupAfterMerge	Clean up after merging by reverting the
				file from the disk and forcing it to
				recalculate.

    INT MergeNextEntry		If we are merging, set up for the next
				document.

    INT BuildFieldNameList	Build a block which contains the
				field-names to use.

    INT ComputeSearchStringSize Compute the total size of the search-string

    INT GetNullTermStringChunkSize 
				Get the size of the string in a
				null-terminated chunk.

    INT InsertMergePrefixString Insert the merge prefix.

    INT InsertMergeSuffixString Insert the merge suffix

    INT InsertMergeChunk	Insert a null-terminated chunk string from
				the StringsUI resource

    INT LockFieldName		Lock a field name and return a pointer and
				size.

    INT ReleaseFieldName	Release a field name.

    INT PointAtSomethingSimilarToData 
				Points at something, usually data.

    INT SubstituteForFieldNames Substitute for all field names in a given
				document.

    INT ReplaceFieldNameCallback 
				Replace a field name with the appropriate
				field data.

    INT ReplaceInAllObjects	Perform a replace-all in all text objects
				in the document.

    INT CreateSearchReplaceBlock 
				Create a block for the
				replace-all-occurrences call.

    INT RevertToOriginalDocument 
				Use the undo mechanism to allow us to undo
				the merge

    INT StartUndoChain		Begin a new undoable action.

    INT EndUndoChain		End a new undoable action.

    INT NukeUndoability		Nuke the undo chain so the user sees
				"nothing to undo"

    INT SetupContextForMergeUndo 
				Setup a context for the merge undo. This
				allows us to retain the old undo context so
				the user will still have their own undo
				stuff when we're done.

    INT RestoreContextAfterMergeUndo 
				Restore the old undo context after the
				merge undo. This allows us to recover the
				old undo context so the user will still
				have their own undo stuff.

    INT SuspendAllTextObjects	Suspend all articles in the document.

    INT UnsuspendAllTextObjects Unsuspend all articles in the document.

    INT SendToAllTextObjects	Send a message to all text objects

    INT SendToAllTOCallback	Send a message to all text objects
...
DESCRIPTION:
	...
	$Id: documentMerge.asm,v 1.1 97/04/04 14:38:54 newdeal Exp $
...
CALLED BY:	StudioDocumentStartPrinting
PASS:		*ds:si	= StudioDocument class
		ss:bp	= Inheritable stack frame
...
appendNameLoop:
	...
	LocalCopyNString			; Copy in the string
...
PASS:		*ds:si	= Field name ChunkArray
		...
		ss:bp	= Inheritable stack frame (StudioDocumentStartPrinting)
		*cx:dx	= StudioDocument instance to work with
...
SBCS <	add	ax, 1				; Account for NULL	>
DBCS <	add	ax, 2				; Account for NULL	>
...
RevertToOriginalDocument	proc	far
	...
	; Well, if we suspend the text objects, then a new undo chain
	; will be started, which will kill the old one, which will
	; break mailmerge altogether.   Suspending is also a bad idea
	; since GenProcessUndoPlaybackChain will crash in EC if the
	; undo start count is nonzero.
	;
	;
;;	call	SuspendAllTextObjects
...
SendToAllTextObjects	proc	near	uses bx, cx, dx, si, bp, ds, es
	...
	mov	ax, MSG_STUDIO_GROBJ_BODY_SEND_TO_ALL_TEXT_OBJECTS
	clr	di
	call	ObjMessage
