...
FixedCode	segment resource  ; this must get called a lot, so FIX it

;
; watch for .ini file changes to sign-up/register state
;
DesktopNotify	method	DesktopClass, MSG_META_NOTIFY
		cmp	cx, MANUFACTURER_ID_GEOWORKS
		jne	done
		cmp	dx, GWNT_INIT_FILE_CHANGE
		jne	done
		cmp	bp, IFE_OWNER_INFO
		jne	done
...
NDDesktopDelayedSaveOptions	method	DesktopClass, MSG_DELAYED_SAVE_OPTIONS
	;
	; save via UI thread queue (to avoid problems when sending out
	; apply message when restoring options)
	;
	...
	ret
NDDesktopDelayedSaveOptions	endm
endif
endif
