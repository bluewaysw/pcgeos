...
if _DOCMGR
	;
	; set menu items for wastebasket, or not-wastebasket
	;
	cmp	diskHandle, SP_WASTE_BASKET
	jne	notWasteFolder
	pushdw	dssi
	movdw	dssi, pathname
	LocalGetChar	ax, dssi
	LocalGetChar	cx, dssi, noAdvance
	popdw	dssi
	LocalIsNull	ax
	je	wasteFolder
	...
	je	wasteFolder
notWasteFolder:
	mov	si, offset Interface:FileMenuWastebasketGroup
	mov	di, offset Interface:FileMenuFileOperationsGroup
	jmp	short setMenuCommon

wasteFolder:
	mov	si, offset Interface:FileMenuFileOperationsGroup
	mov	di, offset Interface:FileMenuWastebasketGroup
setMenuCommon:
	push	di
...
if GPC_FOLDER_WINDOW_MENUS
	;
	; if wastebasket window, attach OptionsMenu
	;
	cmp	ss:[objectType], WOT_WASTEBASKET
	jne	notWaste
	mov	ax, MSG_GEN_ADD_CHILD
...
if GPC_DEBUG_MODE
	;
	; enable show hidden files option in debug mode (folder and
	; wastebasket)
	;
	cmp	ss:[objectType], WOT_DESKTOP
	je	noHiddenOpt
	mov	ax, MSG_GEN_SET_NOT_USABLE
...
noHidden:
	mov	bx, windowBlock
	mov	si, offset NDFolderMenuDisplayOptions
	mov	dl, VUM_NOW
	call	ObjMessageCallFixupAndSaveBP
noHiddenOpt:
endif
if GPC_PRESERVE_DIRECTORIES
	;
	; disable create folder if special directory
	;
	cmp	ss:[objectType], WOT_DESKTOP
	je	notSpecial
	cmp	ss:[objectType], WOT_WASTEBASKET
	...
	movdw	cxdx, pathname
	mov	bp, diskHandle
	call	CheckSpecialFolder		; carry set if so
	pop	bp
	jnc	notSpecial
	mov	bx, windowBlock
	mov	si, offset NDFolderMenuCreateFolder
...
if GPC_FOLDER_WINDOW_MENUS
	;
	; special case: remove Create Folder and Create Link for
	; Wastebasket subdirs
	;
	cmp	ss:[objectType], WOT_DESKTOP
	je	leaveCreate
	...
	pop	es, ds
	jc	haveWasteSub			; error -- leave create
	cmp	al, PCT_SUBDIR			; only looking for waste subs
	je	haveWasteSub			; carry clear -- remove create
	stc					; else, leave create
haveWasteSub:
	jc	leaveCreate
	mov	bx, windowBlock
	mov	si, offset NDFolderMenuCreateFolder
	...
	mov	dl, VUM_NOW
	call	ObjMessageCallFixupAndSaveBP
	mov	si, offset NDFolderMenuCreateLink
	mov	ax, MSG_GEN_SET_NOT_USABLE
	mov	dl, VUM_NOW
	call	ObjMessageCallFixupAndSaveBP
leaveCreate:
...
PSEUDO CODE/STRATEGY:
		sound is short, so no need to keep token
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	brianc	2/19/99		Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@

if GPC_DRAG_SOUND

soundToken	GeodeToken	<<'nDSK'>, MANUFACTURER_ID_GEOWORKS>

UtilDragSound	proc	far
		uses	ax, bx, cx, dx, ds, si, es, di, bp  ; pusha
		.enter
		mov	cx, cs
		mov	dx, offset soundToken
		mov	bx, ST_DRAG
...
SYNOPSIS:	get registration and sign up status

CALLED BY:	EXTERNAL

PASS:		nothing

RETURN:		ax - TRUE if registered
		bx - TRUE if signed up
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	brianc	2/26/99		Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
				   
if GPC_SIGN_UP_ICON

welcomeStatusCat	char	"welcome",0
signUpStatusKey		char	"finishedSignUp",0
registrationStatusKey	char	"finishedRegistration",0

UtilGetSignUpState	proc	far
		uses	cx, dx, ds, si, bp
		.enter
		segmov	ds, cs, cx
		mov	si, offset welcomeStatusCat
		mov	dx, offset signUpStatusKey
		call	InitFileReadBoolean
		jnc	haveSignUp
		mov	ax, FALSE
haveSignUp:
		mov	bx, ax
		mov	dx, offset registrationStatusKey
		call	InitFileReadBoolean
		jnc	haveReg
		mov	ax, FALSE
haveReg:
		.leave
		ret
UtilGetSignUpState	endp

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		UtilUpdateSignUp
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	update sign/up registration icon
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
SignUpIconFlags	record
	:12
	;order must match iconRouts table
	SUIF_NEED_SIGN_UP:1
	SUIF_NEED_REG:1
	SUIF_HAVE_SIGN_UP:1
	SUIF_HAVE_REG:1
...
UtilUpdateSignUp	proc	far
	uses	ax, bx, cx, dx, ds, si, es, di, bp
iconFlags	local	SignUpIconFlags
	.enter
	...
	mov	ds, ax
	;
	; first, check existance of possible links
	;
	mov	si, offset DeskStringsCommon:GPCSignUpLink
	mov	dx, ds:[si]
	call	FileGetAttributes
	jc	gotSignUpHave
	ornf	iconFlags, mask SUIF_HAVE_SIGN_UP
gotSignUpHave:
	mov	si, offset DeskStringsCommon:GPCRegLink
	mov	dx, ds:[si]
	call	FileGetAttributes
	jc	gotRegHave
	ornf	iconFlags, mask SUIF_HAVE_REG
gotRegHave:
	;
	; then, check which ones we need
	;
	call	UtilGetSignUpState		; ax = reg, bx = sign up
	cmp	bx, TRUE
	je	gotSignUpNeed
	ornf	iconFlags, mask SUIF_NEED_SIGN_UP
gotSignUpNeed:
	cmp	ax, TRUE
	je	gotRegNeed
	ornf	iconFlags, mask SUIF_NEED_REG
gotRegNeed:
	;
...
irDeleteR	label	near
	mov	si, offset GPCRegLink
	jmp	short deleteLink

irDeleteS	label	near
	mov	si, offset GPCSignUpLink
deleteLink:
	mov	dx, ds:[si]
	call	FileDelete
	retn

irDeleteRS	label	near
	call	irDeleteR
	call	irDeleteS
	retn

irRenameStoR	label	near
	mov	si, offset GPCSignUpLink
	mov	di, offset GPCRegLink
	jmp	short renameLink

irRenameRtoS	label	near
	mov	si, offset GPCRegLink
	mov	di, offset GPCSignUpLink
renameLink:
	segmov	es, ds, dx
	mov	dx, ds:[si]
	mov	di, es:[di]
...
UtilUpdateSignUp	endp

LocalDefNLString	signUpIconPath <ND_DESKTOP_RELATIVE_PATH, 0>

iconRouts	label	word
					;need-s	need-r	have-s	have-r
	word	0			;0	0	0	0	
	word	offset irDeleteR	;0	0	0	1
	word	offset irDeleteS	;0	0	1	0
	word	offset irDeleteRS	;0	0	1	1
	word	offset irAddR		;0	1	0	0
	word	0			;0	1	0	1
	word	offset irRenameStoR	;0	1	1	0
	word	offset irDeleteS	;0	1	1	1
	word	offset irAddS		;1	0	0	0
	word	offset irRenameRtoS	;1	0	0	1
	word	0			;1	0	1	0
	word	offset irDeleteR	;1	0	1	1
	word	offset irAddS		;1	1	0	0
	word	offset irRenameRtoS	;1	1	0	1
	word	0			;1	1	1	0
	word	0			;1	1	1	1

endif  ; GPC_SIGN_UP_ICON
