...
if GPC_PRESERVE_ARCHIVE
		...
		jne	notDocument
		tst	{TCHAR}ds:[bx].GFP_path[0]
		jnz	notDocument
		inc	dx			; is SP_DOCUMENT
notDocument:
		pop	bx
endif
		push	ds, di
...
if GPC_PRESERVE_PAK_UPD
		call	checkIfPAKUPD
		jne	notPAKUPD
	; fake executable status
ND <		mov	ds:[si].FR_desktopInfo.DI_objectType, WOT_EXECUTABLE >
if _DOCMGR
	; set flag, checked in UpdateFileMenu
		mov	ds:[si].FR_desktopInfo, -1
endif
notPAKUPD:
endif
if GPC_PRESERVE_ARCHIVE
		...
		call	checkIfArchive
		jne	notArchive2
		mov	ds:[si].FR_desktopInfo.DI_objectType, WOT_SYSTEM_FOLDER
notArchive2:
endif
...
if GPC_PRESERVE_ARCHIVE
checkIfArchive	label	near
	push	si, es, di, cx
	lea	si, ds:[si].FR_name
	segmov	es, cs, di
	mov	di, offset archiveObjectName
	clr	cx
...
if GPC_PRESERVE_PAK_UPD
;
; can't use [system]/batch ext since we don't where/how else that list is used
;
checkIfPAKUPD	label	near
	push	es, di, ax, cx
	segmov	es, ds
	mov	di, si
...
DBCS <	mov	cx, (size FileLongName)/2			>
	LocalLoadChar	ax, '.'
	LocalFindChar
	jne	donePAKUPD		; if no '.', not PAK, UPD
	; check *.PAK
SBCS <	cmp	{byte} es:[di], 'P'				>
DBCS <	cmp	{wchar} es:[di], 'P'				>
	je	checkPAK2
SBCS <	cmp	{byte} es:[di], 'p'				>
DBCS <	cmp	{wchar} es:[di], 'p'				>
	jne	checkUPD
checkPAK2:
SBCS <	cmp	{byte} es:[di+1], 'A'				>
DBCS <	cmp	{wchar} es:[di+2], 'A'				>
	je	checkPAK3
SBCS <	cmp	{byte} es:[di+1], 'a'				>
DBCS <	cmp	{wchar} es:[di+2], 'a'				>
	jne	checkUPD
checkPAK3:
SBCS <	cmp	{word} es:[di+2], 'K' or (0 shl 8)		>
DBCS <	cmp	{wchar} es:[di+4], 'K'				>
DBCS <	jne	checkUPD					>
DBCS <	cmp	{wchar} es:[di+6], C_NULL			>
	je	donePAKUPD
SBCS <	cmp	{word} es:[di+2], 'k' or (0 shl 8)		>
DBCS <	cmp	{wchar} es:[di+4], 'k'				>
DBCS <	jne	checkUPD					>
...
checkUPD:
	; check *.UPD
SBCS <	cmp	{byte} es:[di], 'U'				>
DBCS <	cmp	{wchar} es:[di], 'U'				>
	je	checkUPD2
SBCS <	cmp	{byte} es:[di], 'u'				>
DBCS <	cmp	{wchar} es:[di], 'u'				>
	jne	donePAKUPD
checkUPD2:
SBCS <	cmp	{byte} es:[di+1], 'P'				>
DBCS <	cmp	{wchar} es:[di+2], 'P'				>
	je	checkUPD3
SBCS <	cmp	{byte} es:[di+1], 'p'				>
DBCS <	cmp	{wchar} es:[di+2], 'p'				>
	jne	donePAKUPD
checkUPD3:
SBCS <	cmp	{word} es:[di+2], 'D' or (0 shl 8)		>
DBCS <	cmp	{wchar} es:[di+4], 'D'				>
DBCS <	jne	donePAKUPD					>
DBCS <	cmp	{wchar} es:[di+6], C_NULL			>
	je	donePAKUPD
SBCS <	cmp	{word} es:[di+2], 'd' or (0 shl 8)		>
...
if GPC_PRESERVE_ARCHIVE
archiveObjectName	TCHAR	"Archive",0
endif
...
if GPC_CREATE_DESKTOP_LINK
linkDest	char	ND_DESKTOP_RELATIVE_PATH, 0
endif
...
gotSlash:
	...
	mov	bx, dx					;  -dir place.
if not GPC_CREATE_DESKTOP_LINK
...
ifdef GPC_CREATE_DESKTOP_LINK
	mov	cx, (size FileLongName) - 1
else
...
if GPC_CREATE_DESKTOP_LINK
SetupRecoverBoxForLink	proc	near
		uses	ax, bx, cx, dx, bp, ds, si, di
		.enter
		call	MemLock			; lock file block
		mov	ds, ax
		cmp	ss:[debugLinks], TRUE	; check after locking
		je	allowFS			;  so we can always unlock
		mov	cx, ds:[FQTH_numFiles]
		jcxz	allowFS
		mov	si, offset FQTH_files
checkLink:
		test	ds:[si].FOIE_attrs, mask FA_LINK
		jz	allowFS			; non-link, must use FS
		add	si, size FileOperationInfoEntry
		loop	checkLink
	;
	; only links -- remove FS and set destination path to Desktop
	;
		...
		call	noFSStuff
		push	bx
		mov	bp, STANDARD_PATH_OF_DESKTOP_VOLUME
		mov	cx, cs
		mov	dx, offset recoverDesktopPath
		mov	ax, MSG_GEN_PATH_SET
		mov	bx, handle RecoverToEntry
...
noFSStuff	label	near
		mov	cx, handle RecoverLinkPrompt
		mov	si, offset RecoverLinkPrompt
		call	vumNow
		retn
		
fsStuff	label	near
		mov	cx, handle RecoverToEntry
		mov	si, offset RecoverToEntry
		call	vumNow
		mov	cx, handle RecoverToCurrentDir
		mov	si, offset RecoverToCurrentDir
		call	vumNow
		retn

resetSize	label	near
		mov	ax, MSG_GEN_RESET_TO_INITIAL_SIZE
		mov	cx, handle RecoverBox
		mov	si, offset RecoverBox
		call	vumNow
		retn
...
vumNow	label	near
		...
		pop	ax, bx
		retn
SetupRecoverBoxForLink	endp

recoverDesktopPath	char	ND_DESKTOP_RELATIVE_PATH,0
endif
