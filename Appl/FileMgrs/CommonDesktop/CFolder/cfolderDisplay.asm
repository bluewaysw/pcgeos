...
drawIcon:
		...
		test	dh, mask FIDM_LICON or mask FIDM_SICON
		jz	drawIcon			
endif
...
drawIcon:
	;
	; get icon position and draw icon
	;
		
		mov	ax, es:[bp].FR_iconBounds.R_top	; get icon position
		mov_tr	bx, ax
		mov	ax, es:[bp].FR_iconBounds.R_left
		push	ds, si				; *ds:si - folder
		call	GetFolderObjectIcon		; ds:si = icon
							; for this object
		clr	dx				; no callback
		call	GrFillBitmap			; draw icon
	;
	; If XIP GetFolderObjectIcon copied the icon to the stack.  We now
	; have to reset the stack.
	;
FXIP<		call	SysRemoveFromStack				>

		pop	ds, si				; retreive folder
endif
selectTest:
...
;drawNormalIcon:				; mode (i.e. use tool icon)...
	...
	;
if GPC_TEXT_MODE_ICONS and 0  ; have small doc icons now, use FR_token
	...
	jnz	useToken
	mov	ax, {word}es:[di].FR_creator.GT_chars[0]
	tst	al				; null-token, don't bother
	jz	noIcon				;	looking up
	mov	bx, {word}es:[di].FR_creator.GT_chars[2]

	ornf	es:[di].FR_state, cx		; flag lookup attempt

	xchg	dh, dl				; dh = DisplayType
	push	si
	mov	si, es:[di].FR_creator.GT_manufID
	jmp	short tokenCommon
...
gotIconResult:
	...
	call	FolderTokenLookupMoniker
EC<	ERROR_C	CANT_FIND_FILE_MONIKER				>
endif
...
if _DOCMGR
defaultDOSDataIconChars		dword	"dDOS"
defaultGEOSDataIconChars	dword	"dDAT"
defaultGEOSApplIconChars	dword	"dAPP"
else
...
if GPC_TEXT_MODE_ICONS
	...
	sub	ax, 3				; shift over three pixels
; FIX!!! 
;	^^^ make constant: TEMPLATE_ICON_SHIFT = 3
;
endif
...
if _DOCMGR
	mov	ax, ('d') or ('A' shl 8)	; store 'dAPP'
	mov	bx, ('P') or ('P' shl 8)
else
...
subdir:
if _DOCMGR
	mov	ax, ('d') or ('F' shl 8)	; store 'dFLD'
	mov	bx, ('L') or ('D' shl 8)
else
...
nonGEOSExec:
if _DOCMGR
	mov	ax, ('d') or ('D' shl 8)	; store 'dDAT'
	mov	bx, ('A') or ('T' shl 8)
else
...
nonGEOS:
	...
	;
if _DOCMGR
	mov	ax, ('d') or ('F' shl 8)	; default token	'dFIL'
	mov	bx, ('I') or ('L' shl 8)
else
...
noDiskSpaceRound:
	...
	mov	ax, ds:[si].FOI_diskInfo.DIS_freeSpace.high
	mov	dx, ds:[si].FOI_diskInfo.DIS_freeSpace.low
	call	ASCIIizeDWordAXDX		; ax:dx = bytes free on disk
endif
	pop	ax, dx
...
UpdateFileMenu	proc	near
		...
		mov	ax, MSG_GEN_SET_NOT_ENABLED	; empty select list?
		cmp	ds:[di].FOI_selectList, NIL
		je	updateSelect
		mov	ax, MSG_GEN_SET_ENABLED
updateSelect:
		cmp	ds:[di].NDFOI_ndObjType, WOT_WASTEBASKET
		LONG je	updateWastebasket
	;
	; update folder window file menu based on selection list
	;
		mov	cx, NUM_FOLDER_SELECTION_MENU_ITEMS
		mov	di, offset folderSelectionMenuTable
		call	enumObjList
		cmp	ax, MSG_GEN_SET_NOT_ENABLED
		LONG je	done
	;
	; disable based on executable selection (or debug mode)
	; disable based on inside or outside document tree or floppy
	; (if not debug mode)
	;
if GPC_DEBUG_MODE
		cmp	ss:[debugMode], TRUE
		LONG je	done			; leave enabled for debug mode
endif
	;
	; don't disable if on floppy or add-on drive
	;
		push	bx
		call	Folder_GetActualDiskAndPath	; handle links
		mov	bx, ds:[bx].GFP_disk
		call	DiskGetDrive		; al = drive
		call	NDGetDriveLinkIndexBlock	; es = seg, bx = han
		jc	error			; mem error, fall thru
		call	NDDoWeNeedADriveLink	; carry clear if needed
		call	MemUnlock		; (preserves flags)
error:
		pop	bx
		jnc	leaveEnabled
	;
	; disable if not under document directory
	;
		clr	ax
		push	ax
		segmov	es, ss, di
		mov	di, sp
		mov	cx, SP_DOCUMENT		; path1 = SP_DOCUMENT
		push	bx, si, ds
		call	Folder_GetDiskAndPath	; ds:bx = GenFilePath
		mov	dx, ds:[bx].GFP_disk
		lea	si, ds:[bx].GFP_path
		xchg	si, di
		segxchg	ds, es			; path2 = folder path
		call	FileComparePathsEvalLinks	; al = result
		pop	bx, si, ds
		pop	cx
		jc	disableForExec		; path error, disable
if GPC_PRESERVE_ARCHIVE
		cmp	al, PCT_EQUAL
		jne	notArchive
		call	checkArchiveSelected
		jc	disableForExec		; Archive folder, disable
notArchive:
endif
.assert (PCT_EQUAL lt PCT_SUBDIR)
		cmp	al, PCT_SUBDIR
		ja	disableForExec		; not under doc, disable
leaveEnabled:
		DerefFolderObject	ds, si, di
...
execLoop:
	...
	;
	; links shouldn't be in folder windows unless in debug mode, so
	; disable if link and not in debug mode
	;
		test	es:[di].FR_fileAttrs, mask FA_LINK
		jz	notLink
		cmp	ss:[debugLinks], TRUE
		stc				; assume link found, disable
		jne	execDone		; not debugLinks, disable
notLink:
		cmp	es:[di].FR_fileType, GFT_EXECUTABLE
		stc
		je	execDone
;check fake executables also
		cmp	es:[di].FR_desktopInfo.DI_objectType, WOT_EXECUTABLE
...
updateWastebasket:
	;
	; update wastebasket window file menu based on selection list
	;
		mov	cx, NUM_WASTEBASKET_SELECTION_MENU_ITEMS
		mov	di, offset wastebasketSelectionMenuTable
		call	enumObjList
	;
	; disable "Emtpy Wastebasket" if folder is empty
	;
		DerefFolderObject	ds, si, di
		mov	ax, MSG_GEN_SET_ENABLED
		...
		mov	ax, MSG_GEN_SET_NOT_ENABLED
updateEmpty:
		mov	si, offset NDWastebasketMenuEmptyWastebasket
...
if GPC_PRESERVE_ARCHIVE
checkArchiveSelected	label	near
		push	es, di
		DerefFolderObject	ds, si, di
		mov	di, ds:[di].FOI_selectList
		call	FolderLockBuffer
checkArchiveLoop:
		cmp	di, NIL
		je	checkArchiveDone	; C clr, Archive not selected
		test	es:[di].FR_fileAttrs, mask FA_SUBDIR
		jz	checkArchiveNext
		push	ds, si, di, cx
		lea	di, es:[di].FR_name
		segmov	ds, cs, si
		mov	si, offset archiveFolderName
		clr	cx
		call	LocalCmpStrings
		pop	ds, si, di, cx
		stc				; indicate Archive selected
		je	checkArchiveDone
checkArchiveNext:
		mov	di, es:[di].FR_selectNext
		jmp	short checkArchiveLoop
checkArchiveDone:
		call	FolderUnlockBuffer	; preserves flags
		pop	es, di
		retn
...
if GPC_PRESERVE_ARCHIVE
archiveFolderName	TCHAR	"Archive",0
endif

folderSelectionMenuTable	label	lptr
	lptr	offset NDFolderMenuOpen,
		offset NDFolderMenuGetInfo,
		offset NDFolderMenuCopy,
		offset NDFolderMenuMove,
		offset NDFolderMenuRename,
		offset NDFolderMenuThrowAway,
		offset NDFolderMenuCreateLink
NUM_FOLDER_SELECTION_MENU_ITEMS = ($-folderSelectionMenuTable)/(size lptr)

folderExecutableDisableMenuTable	label	lptr
	lptr	offset NDFolderMenuCopy,
		offset NDFolderMenuMove,
		offset NDFolderMenuRename,
		offset NDFolderMenuThrowAway
NUM_FOLDER_EXECUTABLE_DISABLE_MENU_ITEMS = ($-folderExecutableDisableMenuTable)/(size lptr)

wastebasketSelectionMenuTable	label	lptr
	lptr	offset NDWastebasketMenuOpen,
		offset NDWastebasketMenuGetInfo,
		offset NDWastebasketMenuRecover
NUM_WASTEBASKET_SELECTION_MENU_ITEMS = ($-wastebasketSelectionMenuTable)/(size lptr)
endif
