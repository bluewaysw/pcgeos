...
static FileHandle rfp = NullHandle;
static FileHandle pfp = NullHandle;
#else
...
#ifdef __GEOS__
    sfp = FileOpen( script_name, ( FILE_ACCESS_RW | FILE_DENY_RW ) );
    if ( NullHandle == sfp ) {
#else
...
#ifdef __GEOS__
    rfp = FileOpen( new_name, ( FILE_ACCESS_W | FILE_DENY_RW ) );
    if ( NullHandle == rfp ) {
#else
...
#ifdef __GEOS__
    pfp = FileOpen( new_name, ( FILE_ACCESS_R | FILE_DENY_W ) );
    if ( NullHandle == pfp ) {
#else
