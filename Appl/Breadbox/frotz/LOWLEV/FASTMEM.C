...
long pcp = 0;
FileHandle story_fp = NullHandle;
 ...
 *  jfh - added return value to indicate success or failure
 *                                        TRUE       FALSE
 */
...
Boolean init_memory (void)
{
    ...
    /* Open story file */
#ifdef __GEOS__
    story_fp = FileOpen( story_name, ( FILE_ACCESS_R | FILE_DENY_W ) );
    if ( NullHandle == story_fp ) {
#else
    if ((story_fp = fopen (story_name, "r")) == NULL) {
#endif
		  os_fatal ("Cannot open game file");
		  return (FALSE);
	 }
    ...
    LOW_BYTE (H_VERSION, h_version)
	 ...
	 if (h_version < V1 || h_version > V8)  {
		  os_fatal ("This is not a valid game file.");
		  return (FALSE);
	 }
    ...
    LOW_BYTE (H_CONFIG, h_config)
	 ...
	 if (h_version == V3 && (h_config & CONFIG_BYTE_SWAPPED))  {
		  os_fatal ("Byte swapped game file");
		  return (FALSE);
	 }
...
#ifdef __GEOS__
    story_size = FilePos( story_fp, 0, FILE_POS_END );
#else
...
#ifdef __GEOS__
    gfp = FileOpen( new_name, ( FILE_ACCESS_R | FILE_DENY_W ) );
    if ( NullHandle == gfp ) {
#else
...
#ifdef __GEOS__
        gfp = FileCreate( new_name,( FCF_NATIVE | FILE_CREATE_TRUNCATE
                | FILE_ACCESS_W | FILE_DENY_RW ), 0 );
        if ( NullHandle == gfp ) {
#else
        if ((gfp = fopen (new_name, "w")) == NULL) {
#endif
            goto finished;
        }
