...
#define DEFAULT_SAVE_NAME "UNNAMED.SAV"
#endif
...
typedef void _cdecl (*OpcodePrototype)( void );
typedef void (*pcfm_OpcodePrototype)( OpcodePrototype );
...
#define SET_BYTE(addr,v)  { geos_set_byte( addr, v ); }
#define SET_WORD(addr,v)  { geos_set_word( addr, v ); }
#define LOW_BYTE(addr,v)  { v = geos_low_byte( addr ); }
#define LOW_WORD(addr,v)  { v = geos_low_word( addr ); }
#define HIGH_WORD(addr,v) { v = geos_high_word( addr ); }
#define CODE_BYTE(v)      { v = geos_code_byte( pcp ); pcp++; }
#define CODE_WORD(v)      { v = geos_high_word( pcp ); pcp += 2; }
#define GET_PC(v)         { v = pcp; }
#define SET_PC(v)         { pcp = v; }
...
#if defined (__MSDOS__) && !defined (__GEOS__)
...
#if !defined (AMIGA) && !defined (__MSDOS__) && !defined(__GEOS__)
...
#ifdef __GEOS__
extern char story_name[ MAX_FILE_NAME + 1 ];
#else
...
void _pascal geos_set_byte( zword addr, zword v );
void _pascal geos_set_word( zword addr, zword v );
zbyte _pascal geos_low_byte( zword addr );
zword _pascal geos_low_word( zword addr );
zword _pascal geos_high_word( dword addr );
zbyte _pascal geos_code_byte( long addr );

#define fseek( file, pos, mode ) FilePos( file, pos, mode )
#define fgetc( file ) (int)GetByte( file )
#define fread( buf, elemSize, num, file ) FileRead( file, buf, elemSize*num, 0 )
#define fwrite( buf, elemSize, num, file ) FileWrite( file, buf, elemSize*num, 0 )
#define fclose( file ) FileClose( file, 0 )

byte _pascal GetByte( FileHandle fh );
int _pascal fputc( int c, FileHandle fh );
