...
SetVESAWin	proc	far
		...
		mov	fs:[pixelOffset], dx		; save offset
		mov	cx, dx				; save offset into scan
		mov	bx, fs:[modeInfo].VMI_scanSize	; get bytes/scan
...
notneg:
		...
		adc	dx, 0
wasneg:
		mov	bx, fs:[modeInfo].VMI_winGran	; bx = granularity
...
havePage:
		push	ax				; save offset
		mov	fs:[curWinPage], dx		; save page number
		tst	dx
		js	pageSet
		clr	bh				; set window A
		mov	bl, fs:[writeWindow]
		tst	fs:[modeInfo].VMI_winFunc.segment ; check for routine
		jz	useInterrupt
		call	fs:[modeInfo].VMI_winFunc	; set page
...
pageSet:
		mov	dx, fs:[curWinPage]		; restore page number
		...
		sub	ax, fs:[curWinEnd]		; check for partialscan
		neg	ax
		cmp	ax, fs:[modeInfo].VMI_scanSize
		jae	normalScan

		inc	ax
		shr	ax, 1
		mov	fs:[pixelsLeft], ax

normalScan:
		pop	ax				; restore offset
		mov	dx, fs:[writeSegment]		; load up segment
...
computePage:
		mov	bx, fs:[modeInfo].VMI_winSize	; bx = granularity
		cmp	bx, 64
		jz	k64
...
CalcLastScanPtr	proc	near
		...
		cmp	fs:[modeInfo].VMI_scanSize, 1024
		jne	oddScanSize
...
oddScanSize:
		cmp	fs:[modeInfo].VMI_winGran, 64
		je	k64
		mov	ax, dx				; ax = win number
		mov	dx, fs:[modeInfo].VMI_winGran
		xchg	dh, dl				; *256
		shl	dx, 1				; *512
		shl	dx, 1				; *1024
...
both:
		...
		add	ax, fs:[curWinEnd]
		adc	dx, 0
		mov	bx, fs:[modeInfo].VMI_scanSize
		div	bx
		mul	bx
		pop	bx
		sub	ax, bx
...
SetVESAWinSrc	proc	far
		...
		mov	fs:[pixelOffsetSrc], dx	 ; save offset
		mov	cx, dx				; save offset into scan
		mov	bx, fs:[modeInfo].VMI_scanSize	; get bytes/scan
		mul	bx				; dxax = 32-bit offset
		add	ax, cx				; add offset into line
...
havePage:
		push	ax				; save offset
		mov	fs:[curWinPageSrc], dx
		tst	dx
		js	pageSet

		clr	bh				; set mode
		mov	bl, fs:[readWindow]		; set window A
		tst	fs:[modeInfo].VMI_winFunc.segment ; check for routine
		jz	useInterrupt
		call	fs:[modeInfo].VMI_winFunc	; set page
		...
		; scan lines around this one.
pageSet:
		mov	dx, fs:[curWinPageSrc]		; restore page number
		call	CalcLastScanPtr
		mov	fs:[lastWinPtrSrc], ax
		...
		inc	ax
		shr	ax, 1	
		mov	fs:[pixelsLeftSrc], ax

normalScan:
		pop	ax				; restore offset
		mov	dx, fs:[readSegment]		; load up segment
...
nextPage:
		...
		mov	dx, fs:[curWinPage]		; get current page #
		add	dx, fs:[nextWinInc]		; bump to next page
		mov	fs:[curWinPage], dx		; get new page
		tst	dx
		js	pageSet
...
doPartial:
		mov	ax, fs:[curWinEnd]		; compute #pixs left
		sub	ax, di
partialScan:
		inc	ax
		shr	ax, 1
...
nextPage:
		sub	di, fs:[curWinEnd]
		dec	di				; di = offset into next
		mov	dx, fs:[curWinPageSrc]		; get current page #
		add	dx, fs:[nextWinInc]		; bump to next page
		mov	fs:[curWinPageSrc], dx		; get new page
		tst	dx
		js	pageSet
...
SetPrevWin	proc	far
		...
		mov	dx, fs:[modeInfo].VMI_winSize
		xchg	dl, dh				; *256
		shl	dx, 1				; *512
		shl	dx, 1				; *1024
		add	di, dx				; add winGran size
		mov	dx, fs:[curWinPage]		; get new page
		sub	dx, fs:[nextWinInc]
		mov	fs:[curWinPage], dx
		tst	dx
		js	pageSet

		clr	bh
		mov	bl, fs:[writeWindow]		; set window A
		tst	fs:[modeInfo].VMI_winFunc.segment ; check for routine
		jz	useInterrupt
		call	fs:[modeInfo].VMI_winFunc	; set page
...
SetPrevWinSrc	proc	far
		...
		shl	dx, 1				; *1024
		add	di, dx				; add winGran size
		mov	dx, fs:[curWinPageSrc]		; get new page
		sub	dx, fs:[nextWinInc]
		mov	fs:[curWinPageSrc], dx
		tst	dx
		js	pageSet
