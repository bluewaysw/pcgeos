...
DESCRIPTION:
	...
	$Id: simp4bitRaster.asm,v 1.1 97/04/18 11:43:49 newdeal Exp $
...
BS_srLeft:	
		...
		xchg	bl, ah		; BL <- right most pixel from source
...
;	I don't know why we don't use "loop" here, but I'm unwilling to
; 	change the VidMem code, so I'll "NMEM" it...

MEM <		dec	cx		; keep going till middle done	>
MEM <		jnz	BS_smallLoop					>
NMEM <		loop	BS_smallLoop					>
...
BSL_smallLoop:
		...
		stosb			; save in temp buffer
MEM <		dec	cx		; keep going till middle done	>
MEM <		jnz	BSL_smallLoop					>
NMEM <		loop	BSL_smallLoop					>
...
BSHL_smallLoop:
		...
		pop	cx
MEM <		tst	bl			; see if high bit set	>
NMEM <		tst	cl						>
		mov	cx, 0
...
FillBWScan	proc	near
		uses	bp
		.enter
	mov	bx, ss:[bmLeft]		;BX - screen offset of left edge of bm
	mov	ax, ss:[bmRight]	;AX - screen offset of rightmost pixel
...
ifdef	USE_186
	sar	ax, 3			;AX = pixel offset/8
	sar	bx, 1			;BX = pixel offset/2 = byte offset
	sar	bp, 3			;BP = byte offset into data
else
	...
	and 	bx, 0xfffc		;clear low 2 (we write out 8 pixels
					;  = 4 bytes) at a time, so start on
					; a dword boundary
	add	di, bx			; add to screen offset too
	mov	bp, ax			; get screen-byte-offset of
					; right side in bp
	sar	bx, 1
	sar	bx, 1
	sub	bp, bx			; bp = # dest bytes to write - 1
	mov	dx, ss:ditherScan	;  dx = line's dither pattern, or color
...
FBS_left:
;
;	Copied this line from PBS_left - nuke it if it causes problems
;
...
;
;	NOTE - this routine *only* modifies the output buffer at positions that
;	       correspond to set pixels (1 bits) in AL. It does nothing for
;	       pixels that are 0. If you want to set the output buffer for
;	       both set *and* cleared bits in AL, use CopyBitmapByte below
;
...
;
	...
	and	al, bh			;Nuke any bits that are not in the mask
	call	BuildDataByteFar	; expand byte into buffer
	test	bh, 11000000b		; if first two pixels masked out, skip
	jz	byte2
...
byte2:
	inc	di
	xchg	dl, dh
	test	bh, 00110000b		; check out next two pixels
	jz	byte3
...
byte3:
	inc	di
	xchg	dl, dh
	test	bh, 00001100b		; check out next two pixels
	jz	byte4
...
byte4:
	inc	di
	xchg	dl, dh
	test	bh, 00000011b		; check out last two pixels
	jz	done	
...
;	We want to map 1 bits to C_BLACK (0000b) and 0 bits to white (1111b)
;	we invert the incoming data, as BuildDataMaskFar maps 0 to 0000b and
;	1 to 1111b.
...
byte3:
	inc	di
	mov	dx, ss:[dataBuff4+2]
	mov	ah, {byte} ss:[dataMask4+2]
	tst	ah
	jz	byte4
	mov	al, es:[di]		; al = screen data
	call	ss:[modeRoutine]					   
...
moveShift:
		...
		mov	dh, ah			; DH <- second nibble from bm
...
;	If frame buffer is nibble-swapped, nibble swap the bitmap data before
...
GetMaskCommon	proc	near
	...
	and	ah, MASK_FOR_RIGHTMOST_PIXEL_IN_BYTE
...
checkRight:
	...
	and	ah, MASK_FOR_LEFTMOST_PIXEL_IN_BYTE
