...
DESCRIPTION:
	...
	$Id: dumbcomGenChar.asm,v 1.1 97/04/18 11:42:27 newdeal Exp $
...
CLF_lastChar	equ  (this word) + 2
	cmp	dx, 0x1234		    ; check against last character in
	ja	CLF_afterLast		    ; font, use default if beyond.
CLF_firstChar	equ  (this word) + 2
	sub	dx, 0x1234		    ; Subtract from first to get offset
	jb	CLF_beforeFirst		    ; If char is below the first char
else
...
FastCharCommon	label  near
	...
	CalcScanLine	di,bp		;di = screen offset with bp added in 
...
EGA <	mov	dl,ah			;;dl = extra bits		>
	...
	cmp	bx,SCREEN_BYTE_WIDTH	;check for off screen left.
	LONG jge CGS_skipEOLN		;skip to next line if it is.

	; the lineMaskBuffer is stored in a fixed location

	and	al,ds:[bx][lineMaskBuffer]  ; mask with region.
...
BIT <	mov	al, {byte} fs:[bp][ditherMatrix] ;al = pattern 		>
...
CGS_afterWrite:
	NextScanMod	di, CGS_nextScanOffset	
...
MEM <	tst	fs:[bm_scansNext]	; see if off the end		>
MEM <	js	CGS_end			;				>
	;
...
CGRS_notEOLN:				;
	...
	;
BIT <	and	al, {byte} fs:[bp][drawMask]		;		>
BIT <	mov	dl, al			; 				>
BIT <	not	dl			;				>
BIT <	RW <	xornf di, 1					>	>
BIT <	and	es:[di],dl		; 				>
BIT <	RW <	xornf di, 1					>	>
BIT <	and	al, {byte} fs:[bp][ditherMatrix]	 ;		>
BIT <	RW <	xornf di, 1					>	>
BIT <	or	es:[di], al		;				>
BIT <	RW <	xornf di, 1					>	>
...
CGRS_afterWrite:
	NextScanMod	di, CGRS_nextScanOffset	
...
CGF_extra:
	...
	;
BIT <	mov	dh,al							>
BIT <	and	al, {byte} fs:[bp][ditherMatrix] ;al = pattern 		>
BIT <	not	dh			;;dh = NOT mask			>
BIT <	RW <	xornf di, 1					>	>
BIT <	mov	dl,es:[di]		;;dl = screen			>
BIT <	RW <	xornf di, 1					>	>
...
BIT <	mov	dl,ah			;;dl = extra bits		>
	...
	NextScanMod	di, CGF_nextScanOffset
MEM <	tst	fs:[bm_scansNext]	; see if off the end		>
MEM <	js	CGF_end			;				>
	;
	; For bitmap drivers.
...
BIT <	and	bp,7							>
	...
	NextScanMod	di, CL_nextScanOffset
MEM <	tst	fs:[bm_scansNext]	; see if off the end		>
MEM <	js	CC_end			;				>
	;
	; One less line to do.
