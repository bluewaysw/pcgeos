...
DESCRIPTION:
	...
	$Id: clr8GenChar.asm,v 1.1 97/04/18 11:42:58 newdeal Exp $
...
REVISION HISTORY:
	...
	Don	10/26/00	Modified to follow code in clr4GenChar.asm
				  regarding how to deal with shifting data
				  onto the screen.
...
continue:
		...
		shl	bp, 1			; 4 bytes/scan
		and	bp, 0xc			; 4 scan lines
...
MEM <		jge	lastByte		 ; jump if so		>
		...
		sub	di, 8			; so we don't add twice
nextByte:
		add	di, 8			; onto next byte
...
nextScan:
		...
		add	bp, 4			; bump to next scan of 
		and	bp, 0xc			;  dither matrix
nextLine:
		pop	ds			;
		assume	ds:dgroup
		pop	si			;
...
CharGeneralRealSlow	label  near
		...
		call	CalcCharVars		; calculate drawing variables
		shl	bp, 1			; build index into ditherMatrix
		shl	bp, 1
		and	bp, 0xc
...
RSnextScan:
		...
		add	bp, 4			; bump to next scan
		and	bp, 0xc			; bind to ditherMatrix
...
CharGeneralFast	label  near
		...
		call	CalcCharVars		; calculate drawing variables
		.warn	@unreach
		shl	bp, 1			; 4bytes/scan in dither
		shl	bp, 1
		and	bp, 0xc			; restrict to ditherMatrix
...
CharLarge 	label near
		...
		push	si, ax
		mov	si, bp			; si = x position 
		call	SetTempDither		; setup temp dither matrix
		pop	si, ax
		...
		shl	bp, 1
		and	bp, 0xc			; and there are six of them
...
CalcCharVars	proc	near
		...
		; first setup the dither matrix for the character drawing

		push	ax
		mov	bp, si				; save char data ptr
		mov	si, ax
		and	si, 3
		call	SetTempDither
		mov	si, bp				; restore char data ptr
		pop	ax

		; now save byte offset in mask data
...
EC <		pop	ax						>
		...
		; load up the dither bytes from the matrix

		mov	bx, {word} cs:[tempDither][bp]
		mov	cx, {word} cs:[tempDither][bp+2]

		shl	al, 1			; test next bit
		jnc	pix2
...
EC <		sub	ax, cx						>
EC <		cmp	di, ax						>
EC <		ERROR_A	VIDMEM_HUGE_ARRAY_PROBLEM			>
EC <		pop	ax						>
