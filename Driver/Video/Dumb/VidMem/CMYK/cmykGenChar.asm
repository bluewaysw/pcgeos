...
DESCRIPTION:
	...
	$Id: cmykGenChar.asm,v 1.1 97/04/18 11:43:06 newdeal Exp $
...
CGS_writeLoop:
	...
	cmp	bx, fs:[bm_bpMask]	; check fro off screen left
...
CGS_ditherBase equ (this word) + 1				
	...
	pop	bx	
	pop	si							
	and	al, dh			; al = mask AND pattern		
	not	dh			;;dh = NOT mask			
	mov	dl,es:[di]		;;dl = screen			
	and	dl,dh			;;dl = NOT mask AND screen	
	not	dh			;;dh = restore mask
	or	al,dl			;;al = data to store		
	mov	es:[di], al						

	push	bx
	mov	al, {byte} fs:[cyanWord]	; cyan is next	(setup by
	and	al, dh				;  NextDitherByte)
	not	dh
	mov	bx, fs:[bm_bpMask]		; index to cyan plane
	mov	dl, es:[bx][di]
	and	dl, dh
	not	dh
	or	al, dl
	...
	add	bx, fs:[bm_bpMask]		; on to black
	mov	al, {byte} fs:[blackWord]
	and	al, dh
	not	dh
	mov	dl, es:[bx][di]
	and	dl, dh
...
CGRS_writeLoop:
	...
	LONG jg	CGRS_afterWrite		;skip to next line if it is.
...
CGRS_mask equ (this byte) + 1
	...
	and	es:[di],dl		; 				
	mov	bp, fs:[bm_bpMask]	
	and	es:[bp][di], dl		; cyan plane			
	shl	bp, 1			; magenta plane	
	and	es:[bp][di], dl				
	add	bp, fs:[bm_bpMask]
	and	es:[bp][di], dl			
	push	si
CGRS_ditherBase equ (this word) + 1				
	...
	pop	si							
	or	es:[di], al		;				
	mov	bp, fs:[bm_bpMask]		; do other planes 	
	mov	al, {byte} fs:[cyanWord]
	and	al, dl
...
CGF_ditherBase equ (this word) + 1				
	...
	or	al, dl
	mov	es:[bp][di], al
	add	bp, fs:[bm_bpMask]	; onto black
	mov	al, {byte} fs:[blackWord]
	and	al, dh
	not	dh
	mov	dl, es:[bp][di]
...
CL_ditherBase equ (this word) + 1				
	...
	pop	si							
	not	dh			;;dh = NOT mask			
	mov	ch,es:[di]		;;dl = screen			
	and	ch,dh			;;dl = NOT mask AND screen	
	not	dh			;;dh = restore mask
	or	al,ch			;;al = data to store		
	stosb								
	dec	di
	mov	bp, fs:[bm_bpMask]	; onto cyan
	mov	al, {byte} fs:[cyanWord]
