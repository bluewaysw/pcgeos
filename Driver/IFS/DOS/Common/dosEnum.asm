...
DESCRIPTION:
	...
	$Id: dosEnum.asm,v 1.1 97/04/10 11:55:03 newdeal Exp $
...
if _MSLF
	...
	;
		...
		mov	ds:[FGEAD_spec].FGEASD_enum.FED_fd.W32FD_fileName[0], 0
...
afterInt21:
	;
	; We're not supposed to pass . or .. to the callback.  We can't
	; simply just check the first char for being '.', because DOS long
	; names are allowed to begin with one or more '.'s.
	; 

		cmp	{word}ds:[FGEAD_spec].FGEASD_enum.FED_fd.W32FD_fileName[0], '.' or (0 shl 8)
		je	endFindLoop
		cmp	{word}ds:[FGEAD_spec].FGEASD_enum.FED_fd.W32FD_fileName[0], '.' or ('.' shl 8)
		jne	10$
		cmp	ds:[FGEAD_spec].FGEASD_enum.FED_fd.W32FD_fileName[2], 0
		je	endFindLoop
10$:
else
	;
	...
	; 
if _MSLF
		cmp	ds:[FGEAD_spec].FGEASD_enum.FED_fd.W32FD_fileName[0], 0
else
	...
	; 
		andnf	ds:[FGEAD_flags], 
			not (mask FGEAF_NOT_GEOS or mask FGEAF_HAVE_HEADER)
	;
...
if _MSLF
		cmp	ds:[FGEAD_spec].FGEASD_enum.FED_fd.W32FD_fileName[0], '@'
else
		cmp	ds:[FGEAD_spec].FGEASD_enum.FED_dta.FFD_name[0], '@'
endif
		jne	fillFECD
...
if GSDOS
		push	ax
		mov	al, FGEAD_spec.FGEASD_enum.FED_dta.FFD_drive
...
NEC <					; ignore error and strive on	>
freeFGEAD:
else
		push	bx
endif		
