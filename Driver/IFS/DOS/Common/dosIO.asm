...
DESCRIPTION:
	...
	$Id: dosIO.asm,v 1.1 97/04/10 11:54:59 newdeal Exp $
...
noError:
		...
		pop	bx
	;
	; Set HF_sfn to -1 to indicate that this HandleFile is no longer
	; associated with an SFN entry.  This closes a synchronization hole
	; where another thread could open a file and DOS will re-use the
	; SFN entry before the GEOS HandleFile for _this_ file is freed,
	; causing GEOS to think that other file is already opened.
	; (Although the below comment says HF_sfn should be NILed, it doesn't
	; seem to do it anywhere.) -- brianc 12/26/97
	;
		push	ax, dx		; no need to preserve DS
		mov	ax, SGIT_HANDLE_TABLE_SEGMENT
		call	SysGetInfo	; ax = handle table segment
		mov	ds, ax
		mov	ds:[bx].HF_sfn, -1
		pop	ax, dx
	...
	CheckHack <offset DFF_REF_COUNT + width DFF_REF_COUNT eq 8>
		...
		call	LoadVarSegDS
		sub	{byte}ds:[bp].DFE_flags, 1 shl offset DFF_REF_COUNT
...
if SEND_DOCUMENT_FCN_ONLY
		clr	ax
		xchg	ax, ds:[bp].DFE_flags	; ax = old DOSFileFlags
...
if SEND_DOCUMENT_FCN_ONLY
		test	ax, mask DFF_LIKELY_DOC	; clears carry
		jz	notifyDone
endif	; SEND_DOCUMENT_FCN_ONLY
		mov	ax, FCNT_CLOSE
...
if SEND_DOCUMENT_FCN_ONLY
		test	ds:[bp].DFE_flags, mask DFF_LIKELY_DOC
		jz	popDSNotifyContentChangeDone
endif	; SEND_DOCUMENT_FCN_ONLY
