...
DESCRIPTION:
	...
	$Id: textPrintStyleRunPCL.asm,v 1.1 97/04/18 11:50:01 newdeal Exp $
...
tiled:
	...
	cmp	ax,es:PS_previousAttribute ;see if the attribute number is the
					;same for this run.
	je	sendTheChars		;if so, skip setting the font, etc....
...
beforePop:
	...
	jc	exit
sendTheChars:
		;bp has to be the PState address here.....
	mov	cx,ds:[si].SRI_numChars	;cx = length
	mov	dx,ds			;dx:si = string
	add	si,SRI_text
