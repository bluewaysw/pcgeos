...
FILE:		streamHexToASCIILeading0.asm
...
DESCRIPTION:
	...
	$Id: streamHexToASCIILeading0.asm,v 1.1 97/04/18 11:49:34 newdeal Exp $
...
PASS:		ax	- hex number
		cx	- number of digits to write (leading zeros incl.)
		es	- PState segment.
...
count	local	word
	.enter
	mov	count,cx		;save away the digit count.
	segmov	ds,ss,dx
...
nextDigit:
	...
	mov	ax,'0'
addZeros:
	cmp	cx,count			;see if we have filled the
	jae	copyChars			;buffer yet.
	push	ax				;push a leading 0
	inc	cx
	jmp	addZeros
