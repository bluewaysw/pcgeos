...
DESCRIPTION:
	...
	$Id: ccomUI.asm,v 1.1 97/04/18 11:52:34 newdeal Exp $
...
jobParamsFptr	local	fptr.JobParameters push	es, si
	...
	;
	; The JobParameters structure may start at a nonzero offset, or 
	; it may not. 
	;
		mov	ax, es:[si].JP_size
		add	ax, si
		
	;
	; ReAlloc more room for the phone number.
	;
		push	bx				; UI handle
		mov	bx, ss:[jobParamsHandle]
		add	ax, ENOUGH_BYTES_TO_FIT_PHONE_NUMBER
		clr	ch
		call	MemReAlloc
		mov	es, ax				; es = updated segment
		
	;
	; Update JP_size field.
	;
		add	es:[si].JP_size, ENOUGH_BYTES_TO_FIT_PHONE_NUMBER
	...
	;
		...
		push	bp
		lea	bp, es:[si].JP_printerData
		add	bp, 2			; add 2 to compensate for
						; first word stored.
						; buffer => DX:BP
		...
		mov	di, mask MF_CALL
		call	ObjMessage			; fill the buffer
		pop	bp
...
selectLoop:
	...
	; Finally, set the number of pages 
	;
	mov	si, offset FaxDialogBox
