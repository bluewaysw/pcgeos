...
REVISION HISTORY:
	...
	----	----		-----------
	ayuen	11/05/98	Initial revision
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	ayuen	11/19/98	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpInit	proc	near
	...
	call	ds:[lslProtoEntry]	; es:si = MLID entry point, ax =
					;  LSLErrorCode
	Assert	e, ax, LSLEC_SUCCESSFUL

	mov	bx, MLIDF_GET_MLID_CONFIG
	mov	ax, ds:[lslBoardNum]
	pushdw	essi			; push MLID entry point
	mov	si, sp			; ss:[si] = MLID entry point
	call	{fptr} ss:[si]		; es:si = MLIDConfigStruct, ax =
					;  LSLErrorCode
	Assert	e, ax, LSLEC_SUCCESSFUL
	popdw	axax			; restore stack

	segxchg	ds, es			; ds:si = MLIDconfigStruct, es = dgroup
	add	si, offset MLIDCS_nodeAddr	; ds:si = MLIDCS_nodeAddr
	mov	di, offset localEtherAddr	; es:di = localEtherAddr
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	ayuen	11/22/98	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpAddCompleteEntry	proc	near
	...
	mov	bx, SEGMENT_CS
	mov	di, offset EthODIArpCompareIp	; bx:di = callback
...
sendPacket:
	...
	mov	di, ds:[si]
	pushdw	<ds:[di + PACKET_HEADER_MAX_SIZE].ECB_protocolWS.LSLPR_next>
	call	HugeLMemUnlock
...
nextPacket:
	...
	.leave
	ret
EthODIArpAddCompleteEntry	endp
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	ayuen	11/05/98	Initial version
	ed	06/01/00	ARP lookup support (cx register)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpIpToEther	proc	near
...
onSameSubnet:
;;;    Moved elsewhere, leaving here for now just in case
if 0
	;
	; Search our table.
	;
...
notFound:
	...
	;
	call	EthODIArpBroadcastRequest
...
incomplete:
	...
	mov	si, es:[si]
	movdw	<es:[si + PACKET_HEADER_MAX_SIZE].ECB_protocolWS.LSLPR_next>, \
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpIpTableLookup	proc	far
	;
	; Search our table.
	;
	mov_tr	cx, ax			; dxcx = IPAddr
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	ayuen	11/25/98	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpBroadcastRequest	proc	near
	uses	ax, bx, cx, dx, si, di, ds, es
	.enter
	...
	GetDGroup	ds, ax
	INT_OFF
	mov	si, ds:[recvEcbFreeList]
	tst	si
	jz	exit			; can't alloc.  Do nothing.
	mov	ax, ds:[si].ECB_nextLink.offset
	mov	ds:[recvEcbFreeList], ax
	INT_ON
	...
	lea	di, ds:[si].ECB_protID	; es:di = ECB_protID
	mov_tr	ax, si			; ds:ax = ECB
	mov	si, offset arpProtoId	; ds:si = arpProtoId
		CheckHack <(PROTOCOL_ID_SIZE and 1) eq 0>
	mov	cx, PROTOCOL_ID_SIZE / 2
	rep	movsw
	mov_tr	si, ax			; ds:si = ECB

	;
	; Fill in ethernet address for broadcasting (ff:ff:ff:ff:ff:ff).
	;
	add	di, offset ECB_immAddr - (offset ECB_protID + size ECB_protID)
					; es:di = ECB_immAddr
	mov	al, 0xff
	...
	;
	mov	ds:[si].ECB_ESR.segment, segment EthODIArpSendCompleteHandler
	mov	ds:[si].ECB_ESR.offset, offset EthODIArpSendCompleteHandler
	mov	ax, ds:[arpStackId]
	mov	ds:[si].ECB_stackID, ax
	mov	ax, ds:[lslBoardNum]
	mov	ds:[si].ECB_boardNum, ax
	mov	ds:[si].ECB_dataLen, size IpEtherArp

	;
	; Fill in ARP header.
	;
	mov	di, ds:[si].ECB_fragments.PFP_addr.offset
					; ds:di = IpEtherArp buffer
	mov	ds:[di].IEA_arpHeader.AH_hwAddrFormat, AHAF_ETHER_NET_ORDER
	mov	ax, {word} ds:[ipProtoId][PROTOCOL_ID_SIZE - 2]
					; ARP only supports word-size ID's
	mov	ds:[di].IEA_arpHeader.AH_protAddrFormat, ax
	mov	ds:[di].IEA_arpHeader.AH_hwAddrLen, NODE_ADDR_SIZE
	mov	ds:[di].IEA_arpHeader.AH_protAddrLen, IP_ADDR_SIZE
	mov	ds:[di].IEA_arpHeader.AH_op, AO_REQUEST_NET_ORDER

	;
	; Fill in ARP addresses.
	;
	movdw	ds:[di].IEA_targetProtoAddr, dxbx
	movdw	dxax, ds:[localIpAddr]
	;	xchg	al, dh
	;	xchg	ah, dl
	movdw	ds:[di].IEA_senderProtoAddr, dxax
	add	di, offset IEA_senderHwAddr	; es:di = IEA_senderHwAddr
	mov_tr	ax, si			; es:ax = ECB
	mov	si, offset localEtherAddr	; ds:si = localEtherAddr
		CheckHack <(NODE_ADDR_SIZE and 1) eq 0>
	mov	cx, NODE_ADDR_SIZE / 2
	rep	movsw
	mov_tr	si, ax			; es:si = ECB
...
exit:
	...
	.leave
	ret
EthODIArpBroadcastRequest	endp
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	ayuen	11/18/98	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpRecvHandler	proc	far
	.enter
...
ifdef	LOG
		...
		inc	ds:[currentIndex]
		mov	ds:[logBuf][si], LOG_ARPRECV_START
		pop	ds
		popa
endif

	cmp	ds:[di].LAS_dataSize, size IpEtherArp
EC <	WARNING_B ARP_PACKET_TOO_SMALL					>
	jb	reject
	
	; ed 6/5/00 - pray that this isn't important since it's causing trouble
	;	cmp	ds:[di].LAS_dataSize, RECV_BUFFER_SIZE
	;EC <	WARNING_A ARP_PACKET_TOO_LARGE_FOR_US				>
	;	ja	reject

	;
	; If driver thread does not exist (ie. TCPIP is not registered with
	; us), we can't, and have no need to, process ARP packets.
	;
	GetDGroup	es, ax
	tst	es:[etherThread]
	jz	reject
...
EC <	WARNING_Z OUT_OF_RECV_BUFFERS					>
	...
	mov	es:[si].ECB_ESR.segment, segment EthODIArpRecvCompleteHandler
	mov	es:[si].ECB_ESR.offset, offset EthODIArpRecvCompleteHandler
...
ifdef	LOG
		...
		mov	si, ds:[currentIndex]
		inc	ds:[currentIndex]
		mov	ds:[logBuf][si], LOG_ARPRECV_END
		pop	ds
		popa
endif
...
EthODIArpRecvHandler	endp


COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		EthODIArpRecvCompleteHandler
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
DESTROYED:	(ax, bx, cx, dx, si, di, es allowed)
SIDE EFFECTS:	
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpRecvCompleteHandler	proc	far
	.enter
...
ifdef	LOG
		...
		mov	si, ds:[currentIndex]
		inc	ds:[currentIndex]
		mov	ds:[logBuf][si], LOG_ARPRECV_COMP_START
		pop	ds
		popa
endif
...
ifdef	LOG
	...
	.leave
	ret
EthODIArpRecvCompleteHandler	endp
...
EPProcessArpPacket	method dynamic EtherProcessClass, 
	...
	mov	si, ds:[si].ECB_fragments.PFP_addr.offset
					; ds:si = IpEtherArp
	mov	ax, ds:[si].IEA_arpHeader.AH_protAddrFormat
	cmp	ax, {word} ds:[ipProtoId][PROTOCOL_ID_SIZE - 2]
					; ARP only supports 2-byte ID's
	jne	freeBuf

	;
	; Ignore packet if target IP addrs doesn't match ours.
	;
	movdw	dxax, ds:[localIpAddr]	; dxax = local IP addr, network order
	;	xchg	al, dh
	;	xchg	ah, dl		; dxax = local IP addr, network order
...
addAddr:
	...
	call	EthODIArpAddCompleteEntry
	...
	;
	; Opcode is AO_REQUEST.  Construct a reply by reusing the ECB and
	; data buffer.
	;
	...
	;
	; Fill in the ECB protocol ID, board #, and completion handler.
	;
	mov	si, offset arpProtoId	; ds:si = arpProtoId
	lea	di, es:[bp].ECB_protID	; es:di = ECB_protID
		CheckHack <(PROTOCOL_ID_SIZE and 1) eq 0>
	mov	cx, PROTOCOL_ID_SIZE / 2
	rep	movsw
	mov	si, bp			; ds:si = es:si = ECB
	mov	ax, ds:[lslBoardNum]
	mov	ds:[si].ECB_boardNum, ax
	mov	ds:[si].ECB_ESR.segment, segment EthODIArpSendCompleteHandler
	mov	ds:[si].ECB_ESR.offset, offset EthODIArpSendCompleteHandler
...
freeBuf:
	...
	;
	INT_OFF
	mov	ax, bp			; ds:ax = ECB to free
	xchg	ax, ds:[recvEcbFreeList]	; ds:ax = old head of buf list
	mov	ds:[bp].ECB_nextLink.offset, ax
	INT_ON
	jmp	exit
...
DESTROYED:	ax (ax, bx, cx, dx, si, di, es allowed)
SIDE EFFECTS:	
...
REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	ayuen	11/20/98	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
EthODIArpSendCompleteHandler	proc	far
	.enter
...
ifdef	LOG
		...
		mov	si, ds:[currentIndex]
		inc	ds:[currentIndex]
		mov	ds:[logBuf][si], LOG_ARP_SEND_COMP_START
		pop	ds
		popa
endif
	...
	; Free the ECB.
EC <	mov	ax, es							>
EC <	Assert	e, ax, dgroup						>
	mov	ax, si			; es:ax = ECB to free
	xchg	ax, es:[recvEcbFreeList]	; es:ax = old head of buf list
	mov	es:[si].ECB_nextLink.offset, ax
...
ifdef	LOG
	...
	.leave
	ret
EthODIArpSendCompleteHandler	endp
