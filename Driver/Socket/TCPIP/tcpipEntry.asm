...
udata	segment
	...
	dhcpCookie	dword		; Cookie to keep track of our session
...
EC <		ERROR_C TCPIP_INVALID_CONNECTION_HANDLE		.
	...
	;
	; Fail if another connection with the same local port, 
	; remote port and remote address exists.
	;
		push	bx, si
		mov	bx, si				; ds:bx = addr
		lodsw					; ax = link addr size
		add	si, ax				; ds:si = tcp part
		movdw	disi, ds:[si]			; disi = remote addr
		call	TSocketCheckDuplicate		; ax = error if exists
		pop	bx, si
		jc	errorDestroy
	;
	; Make sure link is open (so we have a local address).  Then 
	; store the connection info.
	;
		call	LinkOpenConnection		; ax = error or link
		jc	errorDestroy
...
EC <		call	ECCheckCallerThread			>
	...
	;
		...
		jcxz	freeBuffer
	;
	; Add the data to the output queue for the connection and
	; have driver's thread send the data.  If output queue was full, 
	; wait for space if blocking is requested.
	;
		mov	di, bx			; di = connection handle
		xchg	ax, bx			; ax = connection handle
						; bx = timeout value
		call	TSocketNewOutputData	; carry set if not queued
		jnc	sendIt
		...
		mov	ax, di			; ax = connection handle
		xchg	si, bp			; ^ldx:si = buffer
		call	TcpipQueueSendDataRequest	; ax = error, if any
		xchg	si, bp			; ^ldx:bp = buffer, si = SendMode
		jc	freeBuffer		
sendIt:
		Assert	etype si, SocketSendMode			
		mov	bx, handle dgroup
		...
		mov	dx, si			; dx = SocketSendMode
		mov	cx, di			; cx = connection handle
		mov	ax, MSG_TCPIP_SEND_DATA_ASM
		mov	di, mask MF_CALL   	; must use MF_CALL!
		call	ObjMessage		; returns ax = error
		cmp	ax, SDE_NO_ERROR
		clc				; assume no error
		je	exit			; yes, no error
