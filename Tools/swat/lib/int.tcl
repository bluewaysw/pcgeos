...
#	
#
# 	$Id: int.tcl,v 3.2 90/05/26 17:05:19 tony Exp $
#
###############################################################################
    ...
    HardDisk	2:6 	Hard-disk drive}
{
    ...
    if {[length $args] == 0} {
    	...
    	for {var i [expr [length $args]-2]} {$i >= 0} {var i [expr $i-2]} {
    	    ...
    	    var foo [assoc $ic1bits $irq] foo2 [assoc $ic2bits $irq]
    	    [if {![null $foo]} {
...
[defcommand intr {num {what send}} misc
...
argument is given, the interrupt is delivered}
{
	...
	#
	    ...
	    # table.
	    #
	    assign {dword 0:[expr $num*4]} $irqhandlers+[expr $i*8]
	    #
