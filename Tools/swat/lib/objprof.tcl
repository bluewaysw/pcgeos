...
# DESCRIPTION:
#
#	$Id: objprof.tcl,v 1.4 90/10/05 17:17:50 john Exp $
#
###############################################################################
...
[load showcalls]    	# we need 'remove-brk'
[load object]	    	# we need 'obj-class'
...
[defsubr obj-prof {args}
{
    ...
    var oi_brk_list [list
			...
			[brk ObjMessage::doRet oi-end-call]
			[brk SendMessage::fullyDiscarded+5 oi-om-discarded]
			[brk SendMessage::fixupNone oi-end-call]
			[brk SendMessage::fixupDS oi-end-call]
			[brk SendMessage::fixupDSES oi-end-call]
			[brk ObjMessage::exit oi-end-call]
			[brk ObjMessage::afterLink+31 oi-end-call]
			[brk ObjMessage::customRemoteWait+9 oi-end-call]
			]
...
[defsubr oi-print-method {routine mod reg1 reg2 obj class arrow} {
    ...
    # convert METHOD_FOO to just FOO to shorten the output.
    if {[string match $method METHOD_*]} {
	...
	if {[null $s]} {
		...
		if {[null $s]} {
		    ...
		    var q [type emap $query $s]
		    if {[null $q]} {
	    	    	var method [format {%s(0x%04x)} $method $query]
    	    	    } else {
...
# Print associated graphics string
    ...
    if {[field $type VMTB_MONIKER_LIST] != 0} {
#	echo is moniker list
    	return {}
    }
    if {[field $type VMTB_GSTRING] != 0} {
#	echo is gstring
        return {}
    }
    if {[null [field $type VMTB_GS_COLOR]]} {
#    	echo no color
        return {}
    }
