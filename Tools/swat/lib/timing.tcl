...
#
#	$Id: timing.tcl,v 3.9 90/10/01 11:40:57 tony Exp $
#
###############################################################################
defvar timingTable 0
[defvar timingProcessor 8088 variable
...
#
global timingTable file-root file-system-library
if {!$timingTable} {
    echo -n Reading timing database...
    var timingTable [table create 32]

    if {![string match ${file-system-library} /*]} {
    	var s [stream open ${file-root}/${file-system-library}/timing.80x8x r]
    } else {
    	var s [stream open ${file-system-library}/timing.80x8x r]
    }
...
[defsubr decode-cycles {inst times {noCheck 0} {repCount 0}}
{
	...
	if {[string c $timingProcessor 80286] == 0} {
	 ...
	 *DWORD* {
	    ...
	    #32-bit integer.
	    var dest [range $dest [expr [string first = $dest]+1]
	    	    	    [expr [length $dest chars]-1] chars]
	    var dest [format {%04xh:%04xh} [expr (0x$dest>>16)&0xffff]
	    	    	    [expr 0x$dest&0xffff]]
...
}
{
	...
	foreach i [explode [range $opts 1 end chars]] {
		...
		x {
		    ...
		    var eqpos [expr [string first {=} $xel]+1]
		    if {$eqpos == 0} {
			var xtime 0
		    } else {
			var xtime [range $xel [expr $eqpos] end chars]
			var xel [range $xel 0 [expr $eqpos-2] chars]
		    }
		    var excludeList [concat $excludeList [list [list $xel $xtime]]]
		    var args [cdr $args]
		}
		f {
	...
	for {} {[string c [addr-parse cs:ip] $dest]} {if [break-taken] break} {
	    var inst [unassemble cs:ip 1]
    	    #
    	    # If the address that unassemble gave us cannot be parsed in the
    	    # current context, we better do something about it
    	    #
    	    if {[null [sym find any [sym name [sym faddr proc cs:ip]]]]} {
    	    	var pname [patient name [handle patient [handle find cs:ip]]]
    	    	if {[string compare [sym name [sym faddr proc cs:ip]]
    	    	    	    	     [sym name [sym faddr label cs:ip]]]} {
    	    	    var pname [format {%s::%s} $pname
    	    	    	    	    [sym name [sym faddr proc cs:ip]]]
    	    	}
    	    	var pname [format {%s::%s} $pname [index $inst 0]]
    	    	var inst [concat $pname [range $inst 1 end]]
    	    }
	    ...
	    #
	    if {$rout == 0} {
		echo [format {+%3d: %s} $cyc [format-instruction $inst]]
	    }
	    #
		...
		[case [index $inst 1] in
		 INT* {
			...
			var a [format-instruction $inst]
			var rpos [string first CALL $a]
			var a [range $a $rpos end chars]
			echo [format {%s (INT)} $a]
			var rpos [expr [string last { } $a]+1]
			var rname [range $a $rpos end chars]
			doroutine $excludeList $rname $indent $inst 0
		    }
		 }
		 JMP* {
		    #
			...
			var rname [sym name
				    [sym faddr func $seg:[index $a 1]]]
			echo [format {JMP     %s (INDIRECT)} $rname]
			doroutine $excludeList $rname $indent $inst 1
		    } else {
    	    	        #
...
[defsubr doroutine {excludeList rname indent inst jumpFlag}
{
    global cycles_totalList cycles_rnameList cycles_ilevel cycles_total
	...
	#
	# if ignoring this routine then continue
	#
	foreach xel $excludeList {
	    if {![string c $rname [index $xel 0]]} {
		var found $xel
		break
	    }
	}
    ...
    if {![null $found]} {
	var routineTotal [index $found 1]
	var cycles_total [expr $cycles_total+$routineTotal]
	var rname [index $found 0]
	if {$indent == 1} {
	    echo -n [format {%*s} $cycles_ilevel {}]
        } elif {$indent == 2} {
            echo -n [format {(%d) } $cycles_ilevel]
        }
	echo $routineTotal cycles forced for $rname ($cycles_total total)
	stepcall $inst n
    } else {
