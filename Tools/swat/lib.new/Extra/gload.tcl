...
#
#	$Id: gload.tcl,v 1.8 97/04/29 16:59:33 dbaumann Exp $
#
###############################################################################
...
#
##############################################################################
[defcommand loadgeode {file {arg1 0} {arg2 0}} obscure
...
NOTE: THIS MAY NOT BE USED TO LAUNCH AN APPLICATION. USE loadapp FOR THAT}
{
	...
	# backslash.
	#
	value store ss:sp [value fetch topLevelPath $pt] $pt
	...
	foreach c [explode $file] {
	    if {[string c $c /] == 0} {
	    	...
	    	value store ss:sp+$j $c [type char]
    	    }
	    var j [expr $j+1]
    	}
	#
	...
	#
    	value store ss:sp+$j 0 [type byte]
...
#
##############################################################################
[defcommand loadapp {file} obscure
...
the loading is accomplished by sending a method to the UI}
{
	...
	foreach c [explode $file] {
	    if {[string c $c /] == 0} {
	    	value store ss:sp-$len+$j \\ [type char]
	    } else {
	    	value store ss:sp-$len+$j $c [type char]
    	    }
	    var j [expr $j+1]
    	}
    	value store ss:sp-$len+$j 0 [type byte]
	assign sp sp-$len
	...
	#
		...
		al 0
		cx ui::MSG_GEN_PROCESS_OPEN_APPLICATION
		dx 0
	    	ds ss
		si [read-reg sp]
		bx SP_APPLICATION]} {
