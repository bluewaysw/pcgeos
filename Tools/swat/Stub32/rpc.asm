...
stepVector	dword 0			        ; Initial STEP vector
stepVectorSel   word 0
ForceRef	stepVectorSel
...
Serve		macro	num, rout
	...
	org	servers+size servers
		...
		Serve	RPC_GPMI_GET_INFO, RpcGPMIGetInfo
		Serve	RPC_GPMI_GET_DESCRIPTOR, RpcGPMIGetDescriptor
...
RpcExit		proc	near
	...
	DA	<DEBUG_EXIT>, 	<push ax>
	DPC	<DEBUG_EXIT>, 'E', inv
	DPW	<DEBUG_EXIT>, ds
	DPW	<DEBUG_EXIT>, ds:[sysFlags]
	DA	<DEBUG_EXIT>, 	<pop ax>
...
RpcStep		proc	near
	...
	;
		...
		DPC	DEBUG_RPC_WAIT, 's'

		mov	al, es:[di]
		cmp	al, 0xe4	; byte IN?
		jne	checkPushf
		cmp	{byte}es:[di+1], 21h	; from 21h?
		jne	checkPushf
...
KNOWN BUGS/SIDE EFFECTS/IDEAS:
	...
	Since the debug exception is being caught as an exception instead of
	an interrupt, we would need to go thru the same gyrations as
	IRQCommon to handle it.  It is easiest to CatchInterrupt the debug
	exception and tell IRQCommon to branch to us.
...
if 0
		push	es, di
		mov	es, cs:[kdata]
		mov	di, cs:[kernelVectors].SVT_contextSwitching
		tst	{byte}es:[di]
		pop	es, di
		jnz	RSRPassTheBuck
		;
...
RpcBeep		proc	near
	...
	; 
		...
		mov	ax, ds:[kernelHeader].exe_ss
		mov	({BeepReply}ds:[rpc_ToHost]).br_kstack, ax
                mov     ax, ds:[loaderStackSize]
		mov	({BeepReply}ds:[rpc_ToHost]).br_kstacksize, ax
	...
	; 
		...
		mov	ax, ds:[kernelBIOSSelector]
		mov	({BeepReply}ds:[rpc_ToHost]).br_biosseg, ax
...
RpcSavePathAndSwitchToTopLevelPath	proc	near
	...
	;
		...
		PointDSAtStub
		mov	ds:[initialDrive], al	; save default drive
...
CALLED BY:	RPC_GPMI_GET_INFO
PASS:		nothing
RETURN:		nothing
DESTROYED:	various important yet undocumented things

REVISION HISTORY:
	Name	Date		Description
	----	----		-----------
	dhunter	11/26/00    	Initial version

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
RpcGPMIGetInfo	proc	near
		mov	bx, word ptr CALLDATA	; bx = GPMIInfoType
		call	GPMIGetInfo		; bxcx = info

		mov	si, offset rpc_ToHost
		mov	word ptr ds:[si], cx	;store low word
		mov	word ptr ds:[si+2], bx	;store high word
		mov	cx, 4
		call	Rpc_Reply
		ret
RpcGPMIGetInfo	endp
...

COMMENT @%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		RpcGPMIGetDescriptor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

SYNOPSIS:	Read a descriptor from the LDT.

CALLED BY:	RPC_GPMI_GET_DESCRIPTOR
PASS:		nothing
RETURN:		nothing
DESTROYED:	various important yet undocumented things
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%@
RpcGPMIGetDescriptor	proc	near
		mov	bx, word ptr CALLDATA	; bx = selector
		mov	di, offset rpc_ToHost	; es:di = dest buffer
		...
		ret
error:
		mov	al, RPC_SYSTEMERR
		call	Rpc_Error
		ret
RpcGPMIGetDescriptor	endp
...
SYNOPSIS:	Fill the reply packet with 0xCC values so that we can
                partially fill the packet and have some feedback we didn't
                get the whole thing.

CALLED BY:	Rpc servers
PASS:		CX	= number of bytes of data to CC
                DS should be pointing to the cgroup (scode data alias)
RETURN:		Nothing
DESTROYED:	AX, SI
...
resendLoop:
	...
	; don't allow control-Cingout of the swat stub if we are attached
		...
		jnz	waitLoop
if 0
; Doesn't seem to work under GEOS32 ... going to turn it off for now
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%}
Rpc_Run		proc	near
		; Ensure the correct segment registers
		mov	ax, cs:[stubDSSelector]
		mov	ds, ax
		mov	es, ax
