...
zoomerTable	SwatVectorTable \
<
    0d54h, 0ee2h, 0d5ah, 0d41h, 0a34h, 0000h, 0005h, 000ah, 0692eh,
    019bh, 0a42fh, 09dfh, 0a12h, 0000h, 0000h, 0000h, 
    0d9ch, 0041h, 104dh, 10e6h, 0155h, 019ah, 0852ah, 085dah, 0873dh, 08755h,
    8663h, 88f9h, 8609h, 861dh, 861eh, 8629h, 860bh, 0861dh, 1785h,
    1787h, 1666h, 1782h, 9541h, 9547h, 9561h, 9565h, 9552h, 955ch,
    0a13eh, 0a177h
>

zoomerNoXIPTable	SwatVectorTable \
<
    0d54h, 0ee2h, 0d5ah, 0d41h, 0a34h, 0000h, 0005h, 000ah, 06909h,
    019bh, 0a40ah, 09dfh, 0a12h, 0000h, 0000h, 0000h, 
    0d9ch, 0041h, 104dh, 10e6h, 0155h, 019ah, 08505h, 085b6h, 08744h, 0875fh,
    866ah, 8902h, 8610h, 861eh, 8625h, 862dh, 8612h, 08624h, 178ch,
    178eh, 166dh, 1789h, 9548h, 954eh, 9568h, 956ch, 9559h, 9562h,
    0a119h, 0a152h
>
endif
...
UnderAtron	byte	'a', 0		; Overwritten by result of search.
					; Just tells us if we're under the
					; atron board or should catch NMI's
					; too.
...
SYNOPSIS:	special-handling to cope with non-maskable nature of the
...
10$:
		...
		jnz	10$
		mov	ax, 1 shl 13
		out	26h, ax	; finish the interrupt
		pop	ax
		call	IRQCommon
		.unreached
		byte	0x7d
...
if TRACK_INT_21
		...
		clr	bh
		shl	bx
		inc	cs:[int21Counts][bx]
		cmp	ah, MSDOS_READ_FILE
		jne	trackingDone
		add	cs:[bytesRead].low, cx
		adc	cs:[bytesRead].high, 0
trackingDone:
endif
		;
...
checkResize:
	...
	; 
		...
		mov	{word}cs:[rpc_ToHost], bx	; pass new size
							;  as word arg
beginDosExec:
	;
...
checkFree:
	...
	; 
		...
		mov	{word}cs:[rpc_ToHost], 0	;assume so
		push	ax
		mov	ax, es
		cmp	ax, cs:[kdata]
		pop	ax
...
isExit:
	...
	DPW DEBUG_DOS, ax
		...
		mov	cs:[reloadState], RS_WATCH_EXEC
...
KernelMemoryNear	proc	near
		CreateNearFrame
		.fall_thru
KernelMemoryNear	endp
...
KernelProcessNear 	proc	near
		CreateNearFrame
		.fall_thru
KernelProcessNear	endp
...
KernelLoadResNear	proc	near
		CreateNearFrame
		.fall_thru
KernelLoadResNear	endp
...
notify:
		call	SaveState
		;
		; Preserve the dataAddress for the handle and store in the
		; correct one now. Store these two parameters in the rpc
		; packet while we've got them handy...
		; 
		mov	es, ds:[kdata]
		mov	bx, [bp].state_bx
		mov	ax, [bp].state_ax
...
preSDKRoutines	nptr.near	KernelLoadResNear, KernelMemoryNear, KernelProcessNear
...
KernelIntercept	proc	near	uses es, di, ax, si, cx
	...
	DA	DEBUG_TSR, 	<pop ax>
		...
		mov	bx, offset preSDKRoutines - offset interceptPoints
...
ifdef ZOOMER
		mov	si, offset zoomerNoXIPTable
		cmp	ax, 690Ch
		je	useInternalTable
		mov	si, offset zoomerTable
useInternalTable:
else
...
ifdef ZOOMER
	;
	; Intercept write-protect violation.
	; 
		mov	ax, 7dh
		mov	bx, offset prevWP
		mov	dx, offset KernelWPError
		call	SetInterrupt
	;
	; enable write-protect faults
	; 
;		in	al, 0xcf
;		ornf	al, 0x40
		mov	al, mask ZPC_INT_ENABLE
		out	Z_PROTECT_CTRL, al
endif
...
ifdef ZOOMER
	;
	; Disable write-protect faults
	; 
		in	al, 0xcf
		andnf	al, not 0x40
		out	0xcf, al
	;
	; Reset write-protect violation.
	; 
		mov	ax, 7dh
		mov	bx, offset prevWP
		call	ResetInterrupt
...
Kernel_GetNextDataBlock	proc	near
	...
	; get the size to read and store it in saved ReadGeodeArgs
		...
		mov	cs:[savedReadGeodeArgs].RGA_size, ax
...
KernelGetGeodeResourcesInfo	proc	near
	...
	; we left off
		...
		add	si, cs:[savedReadGeodeArgs].RGA_dataValue1
		add	cs:[savedReadGeodeArgs].RGA_dataValue1, \
				FILE_XFER_BLOCK_SIZE - size ReadGeodeReply
		jmp	getInfo
resetState:
	; use one of the generic values to remember where to pick from
		mov	cs:[savedReadGeodeArgs].RGA_dataValue1, \
				FILE_XFER_BLOCK_SIZE - size ReadGeodeReply
getInfo:	
...
afterFileRead:
	...
	; next time
		clr	dx
		adddw	cs:[savedReadGeodeArgs].RGA_offset, dxcx
...
KernelSafeUnlock proc	near
		...
		segmov	ds, cs, ax	; make sure ds = scode
...
scanLoop:
	...
	;
		...
		jz	nextBlock
		cmp	ax, cx
		ja	nextBlock		; Handle starts after desired
						; segment -- can't cover
		add	ax, es:[bx].HM_size	; Add extent of handle
		cmp	cx, ax
		jae	nextBlock		; Start below or equal,
						; end below -- can't cover
	;
	; It does cover. make sure it's neither free nor fake.
	;
		mov	ax, es:[bx].HM_owner
...
KRRKernelThread:
		...
		; 
		push	cs		; Shift cgroup into ES for STOSW
		pop	es
		mov	di, offset rpc_ToHost
...
ifdef ZOOMER
		...
		mov	dx, es
		cmp	dx, Z_EMS_START
		jb	done		; nothing protected below a000
		cmp	dx, Z_EMS_END	; in ROM bank?
		jb	emsWin

		in	al, Z_ROM_BANK_REG
		mov	ah, al		; save old value
		andnf	al, not 1	; clear write-protect
		out	Z_ROM_BANK_REG, al
		mov	al, Z_ROM_BANK_REG; al <- port number
		jmp	done		
...
emsWin:
	.186
		sub	dx, Z_EMS_START
		shr	dx, 9		; /1K * 2
		add	dx, Z_EMS_BASE_REG
		in	ax, dx
		push	ax
		andnf	ax, not 0x8000
		out	dx, ax
		pop	ax
		inc	dx		; want to write the high byte...
		mov	al, dl		; al <- port number
done:
		pop	dx
endif ;ZOOMER
...
ifdef GULLIVER
		...
		clr	bx			; this will contain the
						; value to return in ax
					
		mov	dx, es
		
		; This jump list is probably overkill, but oh well.
		
		; Check if in the first EMS range
		cmp	dx, G_EMS_1_START
		jb	done
		cmp	dx, G_EMS_1_END
		jbe	makeWritable
		
		; Check if in second EMS range.
		cmp	dx, G_EMS_2_START
		jb	done
	
		; There's no need to check the end of the second EMS range.
	CheckHack <G_EMS_2_END eq 0ffffh>
		
makeWritable:
		
		; There are three write protects that we have to deal with.
		; The first is the write protect for ROMDOS which contains
		; the 4Mb of SRAM.  The second is the write protect for
		; ROMBIOS which may also point to an area in the SRAM.  The
		; third is the Shadow RAM, which may be used on some
		; prototypes.  Instead of trying to figure out which one to
		; use, just set them all writable.
		
		; Also, we cannot tell if ROMDOS or ROMBIOS is writable
		; because the register to set the W/P for those areas is
		; write-only.  We can tell about the Shadow RAM, but it
		; doesn't matter.  We'll just try to write a word out there
		; and see what comes back.
		
	; Test if area is already write protected:
		push	cx
		mov	ax, es:[0]		; save value in es:0
		mov	dx, ax
		xor	dx, 0a53ch		; 1010 0101 0011 1100
		mov	es:[0], dx		; write out modified value
		mov	cx, es:[0]		; read back in
		mov	es:[0], ax		; always restore value
		cmp	cx, dx			; check to see if we got
						; what we wrote
		pop	cx
		je	done			; Yes, already writable
						; bx still 0
	
	; Okay, now we need to un-write protect these areas:
		ElanSaveIndex	TRASH_AX

		; un-write protect shadow ram:
		mov	al, ELAN_ROM_CONFIGURATION
		out	ELAN_INDEX, al
		in	al, ELAN_DATA
		or	al, ELAN_DISW		; enable writing to shadow RAM
		xor	al, ELAN_ENROMF		; have to toggle this bit.
						; this bit is opposite on
						; write than read..
		out	ELAN_DATA, al
		
		; un-write protect ROMDOS and ROMBIOS area
		mov	al, ELAN_MEMORY_WAIT_STATE	; (Write-only reg)
		out	ELAN_INDEX, al
		mov	al, ELAN_NFROMEN or ELAN_NFRDOSEN
		out	ELAN_DATA, al

		ElanRestoreIndex	TRASH_AX
...
;	Don't do anything if ES is in the writable memory range.
;	We return AH non-zero if ES was in the ROM area

		DPC	DEBUG_WRITE_PROTECT, 'W'
		push	dx
		clr	ah
		mov	dx, es
		cmp	dx, RESP_ROM_SPACE
		jb	exit
		DPC	DEBUG_WRITE_PROTECT, 'Y'

		mov	ah, 1

;	Write-enable the ROM banks

		mov	dx, MEM_ENABLE_REG
		in	al, dx
		DPB	DEBUG_WRITE_PROTECT, al
		push	ax			;Save current value

		ornf	al, mask RME_ENABLE_WRITES_TO_ROM
		DPB	DEBUG_WRITE_PROTECT, al
...
ifdef GULLIVER
	...
	; Okay, just set all of the areas write protected.
		ElanSaveIndex	TRASH_AX

		; write protect shadow ram:
		mov	al, ELAN_ROM_CONFIGURATION
		out	ELAN_INDEX, al
		in	al, ELAN_DATA
		and	al, not ELAN_DISW	; disable writing to shadow RAM
		xor	al, ELAN_ENROMF		; have to toggle this bit.
						; this bit is opposite on
						; write than read..
		out	ELAN_DATA, al
		
		; write protect ROMDOS and ROMBIOS area
		mov	al, ELAN_MEMORY_WAIT_STATE	; (Write-only reg)
		out	ELAN_INDEX, al
		clr	al
		out	ELAN_DATA, al
...
;	If AH == 0, just exit, as there is nothing to change...

		tst	ah		;If nothing to restore, branch
		jz	exit
		push	dx
		mov	dx, MEM_ENABLE_REG
		out	dx, al
...
;
kcodeSeg	word	stubInit	; Where to load it
ifdef ZOOMER
kernelName	char	"loader.exe",0		; Storage for loader's name
else
...
Kernel_Load	proc	far
		...
		;
		; Check for /a flag to know if we should catch NMI or not.
		; 
		mov	di, offset UnderAtron
		clr	bx
		mov	dx, bx
		call	FetchArg
		or	ds:[intsToIgnore][RPC_HALT_NMI], bl
		;
...
useDefault:
		...
		push	ds
		segmov	ds,cs		; Point DS at init variables
		lodsb			; Fetch length
