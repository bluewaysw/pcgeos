...
if INTEL_BREAKPOINT_SUPPORT
	...
	;
	; ESP doesn't know that we are really loading eax, so pretend
	; that we are only interested in the low word of the source
	; or it will complain about a size mismatch
	;
use32 <		mov	ax, {word}ss:[bp].state_dr7			>
		stosd
use32 <		mov	ax, {word}ss:[bp].state_dr6			>
		stosd
		movsp	eax, dr3
		stosd
		movsp	eax, dr2
...
if 0
		...
		segmov	ds, cs
		lodsd
use32 <		mov	{word}ss:[bp].state_dr7, ax			>
		lodsd
use32 <		mov	{word}ss:[bp].state_dr6, ax			>
		lodsd
		movsp	dr3, eax
		lodsd
		movsp	dr2, eax
...
setupForSkip:
	...
	; 
		clr	ax
		mov	es, ax
		mov	es:[1*4].offset, offset IBptSkipRecover
		segmov	es, ds
	;
...
IBptSkipRecover	proc	far
	...
	; 
		clr	ax
		mov	es, ax
		mov	es:[4].offset, offset RpcStepReply
	;
