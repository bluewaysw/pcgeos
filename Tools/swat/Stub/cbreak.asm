...
wordLoop:
	...
	;
	; New for 32-bit: If we're checking the 32-bit registers
	; (reg_eax - reg_edi), then bit 0 of CX determines if we do a
	; word (zero) or dword (non-zero) comparison. The order is
	; thread, ax (eax-low), eax-high, cx (ecx-low), ecx-high, ...
	;
		mov	al, [bx]	; Fetch comparison byte
if _Regs_32
		clr	ah		; Assume word comparison (ah = zero)
		cmp	cl, REG_NUM_REGS-(reg_di/2)-1	; Checking 32-bit regs?
		jb	checkShift	;  nope, so ah is correct
		cmp	cl, REG_NUM_REGS+1	; thread is 16-bit register
		je	checkCompare	; don't shift as CX is odd
		test	cl, 1		; CX(0) = zero means word
		jz	doShift		; skip redundant test
		inc	ah		; dword comp (ah = non-zero)
		jmp	checkCompare
checkShift:
endif
		test	cl, 1
...
if _Regs_32
		tst	ah		; ah is zero for word compare
		jz	HCBCmpSW	; (Clear out prefetch queue)
		sub	si, 2		; move si and di back to the low word
		sub	di, 2		;  which was skipped on the last pass
		cmpsd			; Compare the dwords
		jmp	short HCBJump	; Skip ahead (Clear out prefetch queue)
HCBCmpSW:
endif
...
match:
	...
	;
		...
		segmov	es,cs,di	; Restore ES after thrashing by LES,
					;  above.
		mov	ax, BCR_OK
		ret
skipWord:
