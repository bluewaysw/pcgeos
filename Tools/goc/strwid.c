...
#ifndef lint
static char *rcsid =
"$Id: strwid.c,v 1.6 97/09/22 23:40:33 allen Exp $";
#endif lint
...
int GetStringWidth(char *string, int widthTable[], int ptsize)
{
    ...
    while (c = Scan_ScanSJISChar((unsigned char **) &string)) {
	/*
	...
	if (dbcsRelease && ptsize == 10) {
	    ...
	    /* We don't have the width here, so we stuff in a huge width here
	       so that our call will drop the total width and force it to
	       be calculated when needed. */
	    width = 10000;               /* big-enough width which will
					    eventually be dropped by our
					    caller */
	}
    }
    return(width);
