 ...
 *
 * PROJECT:	  PMake
 * MODULE:	  Prefix -- Formatted Output
 * FILE:	  printf.c
 *
 ...
 *
 * DESCRIPTION:
 *	This thing is necessary so prefix can work with gcc on a Sparc.
 *	I've not found a way to use vprintf and GCC's varargs, so I'm
 *	giving up and snarfing this from another project of mine.
 *
...
#ifndef lint
static char *rcsid =
"$Id: printf.c,v 1.2 89/11/14 17:18:22 adam Exp Locker: adam $";
#endif lint
...
static int
i_vfprintf(stream, format, args, isstrm)
    ...
    va_list args;		/* Variable number of values to be formatted
				 * and printed. */
		Boolean isstrm; /* TRUE if the stream paramater is actually a
...
vfprintf(stream, format, args)
		...
		va_list args;		/* Variable number of values to be formatted
				* and printed. */
{
 	return i_vfprintf(stream, format, args, FALSE);
}
    ...
    va_list args;
{
    ...
    res = i_vfprintf((FILE*)strPtr, fmt, args, TRUE);
 ...
 *
 ***********************************************************************/
#if defined(is68k) || defined(vax) || defined(sparc)
char *
#else
int
#endif
