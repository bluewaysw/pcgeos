...
/* 
  since the lib files for BORLANDC (same as MicroSoft) do not contain any
  modification time, this whole module is pretty much useless...
*/
#if defined(unix)

#ifndef lint
static char *rcsid = "$Id: arch.c,v 1.2 93/09/02 19:44:23 jimmy Exp $ SPRITE (Berkeley)";
#endif lint
...
Arch_ParseArchive (char **linePtr, Lst nodeLst, GNode *ctxt)
{
	...
	*cp = '\0';
	...
	if (doSubst) {
	     ...
	     */
	    MallocCheck(buf, strlen(memName)+strlen(libName)+3);
	    sacrifice = buf;
 ...
 *      Boolean hash    : TRUE if archive should be hashed if not alread so.
 *
 * Side Effects:
 *
 *-----------------------------------------------------------------------
...
#define AR_MAX_NAME_LEN	    (sizeof(arh.ar_name)-1)
	...
	if (strncmp ( arh.ar_fmag, ARFMAG, sizeof (arh.ar_fmag)) != 0) {
	    ...
	    he = Hash_CreateEntry (&ar->members, Str_New (memName),
				   (Boolean *)NULL);
	    MallocCheck(mymem, sizeof(struct ar_hdr));
	    Hash_SetValue (he, (ClientData)mymem);
 ...
 * Arguments:
 *      char          *archive : Path to the archive
 *      char          *member  : Name of member.  If is is a path, only the
 *                               last component is used.
...
Arch_Touch (GNode *gn)
{
    ...
    arch = ArchFindMember(Var_Value (ARCHIVE, gn),
			  Var_Value (TARGET, gn),
			  &arh, "r+");
    sprintf(arh.ar_date, "%-12d", (int) now);
...
Arch_TouchLib (GNode *gn)
{
	...
	fd = open(gn->path, O_RDONLY, 0);
	unixtodos(now, &datep, &timep);
	_dos_setftime(fd, datep, timep);
    }
#else
...
Arch_MemMTime (GNode *gn)
{
	...
	if (pgn->type & OP_ARCHV) {
	     ...
	     */
	    nameStart = (char*) index (pgn->name, '(') + 1;
	    nameEnd   = (char*) index (nameStart, ')');
...
Arch_FindLib (GNode *gn, Lst path)
{
    ...
    MallocCheck (libName, strlen (gn->name) + 6 - 2);
