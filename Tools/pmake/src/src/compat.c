...
CompatRunCommand (char *cmd, GNode *gn)
{
	...
	if (stat > -1) {
		...
		mystatus = reason.w_stopval;		/* stopped */
	    } else if (WIFEXITED(reason)) {
		mystatus = reason.w_retcode;		/* exited */
		if (mystatus != 0) {
		    printf ("*** Error code %d", mystatus);
		}
	    } else {
		mystatus = reason.w_termsig;		/* signaled */
...
Compat_Run(Lst targs)
{
    ...
    if (signal(SIGINT, SIG_IGN) != SIG_IGN) {
	signal(SIGINT, (void (*)(void))CompatInterrupt);
    }
    if (signal(SIGTERM, SIG_IGN) != SIG_IGN) {
	signal(SIGTERM, (void (*)(void))CompatInterrupt);
    }
#ifdef unix
