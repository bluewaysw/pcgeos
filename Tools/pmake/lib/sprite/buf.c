...
#ifndef lint
static char rcsid[] =
"$Id: buf.c,v 1.2 96/06/24 15:03:34 tbradley Exp $ SPRITE (Berkeley)";
#endif lint
 ...
 */
void
Buf_UngetByte (Buffer buf, Byte byte)
{
    register BufPtr	bp = (BufPtr) buf;
	...
	*bp->inPtr = 0;
	...
	memcpy ((Address)(newBuf+BUF_UNGET_INC), 
		    (Address)bp->outPtr, numBytes+1);
		...
		Byte   *bytesPtr)
{
	...
	memcpy((Address)(newBuf+newBytes), (Address)bp->outPtr, curNumBytes+1);
	...
	bp->size += newBytes;
	bp->outPtr -= numBytes;
	memcpy((Address)bp->outPtr, (Address)bytesPtr, numBytes);
    }
}

/*-
