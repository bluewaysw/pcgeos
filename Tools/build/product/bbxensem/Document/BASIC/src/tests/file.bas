...
REM **********************************************************************
    ...
    button29 = MakeComponent("button",popup3)
    CompInit button29
	proto="button29"
	caption="exists-a01"
	visible=1
...
sub button29_pressed(self as button)
    outputText.AppendString("Executing: exists-a01 test sequence\r")
    testfile.chdir("c:\\")
    testfile.name = "config.sys"
    if testfile.exists then
    	outputText.AppendString("  c:\\config.sys exists!\r")
    end if
    testfile.name = "autoexec.bat"
    if testfile.exists then
    	outputText.AppendString("  c:\\autoexec.bat exists!\r")
    end if
    testfile.name = "autoexec.bad"
    if testfile.exists then
    	outputText.AppendString("  c:\\autoexec.bad exists!\r")
    end if
end sub
