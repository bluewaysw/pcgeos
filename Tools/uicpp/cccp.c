...
char *xmalloc (int), *xrealloc (), *xcalloc (), *savestring ();
...
U_CHAR *skip_quoted_string (U_CHAR*, U_CHAR*, int, int*, int*, int*);
int warning3(char*, int, int);
int warning2(char*, int);

...
U_CHAR *skip_to_end_of_comment (FILE_BUF*, int*);
...
struct file_name_list include_defaults[] =
  {
#ifndef VMS
    { &include_defaults[1], "this is never used" },
...
struct file_name_list cplusplus_include_defaults[] =
  {
    ...
    { &cplusplus_include_defaults[1], "unused" },
    /* Use GNU CC specific header files.  */
    { &cplusplus_include_defaults[2], "unused" },
    { 0, "/usr/include" }
#else
...
struct hashnode *install (U_CHAR*, int, enum node_type, int, int);
...
DEFINITION *collect_expansion (U_CHAR*, U_CHAR*, int, struct arglist*);
      ...
      char **argv;
{
  ...
  for (i = 1; i < argc; i++) {
...
#if 0
	/* For uicpp, we never want standard includes */
      case 'n':
	...
	if (in_fname == NULL) {
      ...
      default:
	fatal2 ("Invalid option `%s'", argv[i]);
      }
    }
  }
     ...
     FILE_BUF *buf;
{
  ...
  if (warn_trigraphs && fptr != bptr)
    warning2 ("%d trigraph(s) encountered", (fptr - bptr) / 2);
}

...
hashcollision:
 ...
 ending:
  if (if_stack != ip->if_stack) {
    ...
    error_with_line2 (line_for_error (if_stack->lineno),
		     "unterminated #%s conditional", (int) str);
  }
	...
	while (xp < bp) {
	  ...
	  *cp++ = c;
	  ...
	  case '/':
		...
		cp[-1] = ' ';
	      ip->bufp = xp + 1;
	      skip_to_end_of_comment (ip, (int*) 0);
	      xp = ip->bufp;
	    }
	  }
	}
...
get_filename:
  ...
  if (f < 0) {
    ...
    if (print_deps > (system_header_p || (system_include_depth > 0))) {
      if (system_header_p)
	warning3 ("nonexistent file <%.*s> omitted from dependency output",
		 fend - fbeg, fbeg);
      else
	{
     ...
     struct directive *keyword;
{
  ...
  while ((hp = lookup (buf, -1, -1)) != NULL) {
    if (hp->type != T_MACRO)
      error2 ("undefining `%s'", (int) hp->name);
    delete_macro (hp);
  }
}

     ...
     struct directive *keyword;
{
  ...
  if (end == buf) {
    ...
    if (! traditional)
      warning2 (end == limit ? "#%s with no argument"
	       : "#%s argument starts with punctuation",
	       keyword->name);
  } else {
    if (pedantic && buf[0] >= '0' && buf[0] <= '9')
      warning2 ("#%s argument starts with a digit", keyword->name);
    else if (end != limit && !traditional)
      warning2 ("garbage at end of #%s argument", keyword->name);
     ...
     int any;
{
  ...
  while (bp < endb) {
	...
	if (strncmp (cp, kt->name, kt->length) == 0
	  ...
	  case T_ELIF:
	    if (if_stack == instack[indepth].if_stack) {
	      error2 ("#%s not within a conditional", (int) kt->name);
	      break;
	    }
     ...
     FILE_BUF *op;
{
  ...
  if (nargs >= 0) {
    ...
    if (nargs == 0 && i > 0)
      error2 ("arguments given to macro `%s'", (int) hp->name);
    else if (i < nargs) {
      /* traditional C allows foo() if foo wants one argument.  */
      if (nargs == 1 && i == 0 && traditional)
	;
      else if (i == 0)
	error2 ("no args to macro `%s'", (int) hp->name);
      else if (i == 1)
	error2 ("only 1 arg to macro `%s'", (int) hp->name);
      else
	error3 ("only %d args to macro `%s'", i, (int) hp->name);
    } else if (i > nargs)
      error3 ("too many (%d) args to macro `%s'", i, (int) hp->name);
...
error (msg)
     char *msg;
{
  error3(msg, (int) NULL, (int) NULL);
}

error2 (msg, arg1)
     char *msg;
{
  error3(msg, arg1, (int) NULL);
}

error3 (msg, arg1, arg2)
     char *msg;
{
  int i;
  FILE_BUF *ip = NULL;
...
warning2 (msg, arg1)
     char *msg;
{
  warning3(msg, arg1, NULL);
}

warning (msg)
     char *msg;
{
  warning3(msg, NULL, NULL);
}

error_with_line (line, msg)
     int line;
     char *msg;
{
  error_with_line2 ( line, msg, (int) NULL);
}

error_with_line2 (line, msg, arg1)
     int line;
     char *msg;
{
  int i;
