...
The striped objects represent nodes in the target hierarchy. Each composite 
...
the top node changed its target child.
Figure 11-4 Hierarchy Terminology

Each object in the tree is a node. Each arrow points to the node having the 
exclusive at its level. The striped path is the active path; the shaded paths are 
all inactive paths.
Advanced Topic

You will probably 
never need to use pre- 
or post-passive grabs.
...
CF_RELEASE	Set if the user lets up on a key. Applications typically are not 
...
IH_bounds	The bounds of the Ink input on the screen (in document 
coordinates).
...
IH_data	Actually a label indicating the beginning of the list of points. 
...
dest	The optr of the object that will receive the Ink data block after 
...
gs	The handle of a Graphic State (GState) containing clipping, 
...
callback	Virtual fptr to a callback routine to determine whether a stroke 
...
Node	Any composite in the object tree that can be part of a path for 
one of the hierarchies.

Exclusive	The active child of a composite node in the hierarchy is said to 
have the "exclusive." Only one child of any composite may have 
the exclusive at that level.

Level	A group of siblings, all of which can potentially be part of a 
...
Grab	When an object grabs a hierarchy (e.g. "grabs the focus"), it 
