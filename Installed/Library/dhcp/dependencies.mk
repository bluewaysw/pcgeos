...
dhcpManager.eobj \
dhcpManager.obj : Internal/gUtils.def Internal/heapInt.def\
                  Internal/netutils.def Internal/semInt.def\
                  ...
                  Objects/visC.def accpnt.def alb.def assert.def char.def\
                  chunkarr.def color.def dhcp.def dhcpConstant.def\
                  dhcpMacro.def dhcpMain.asm dhcpManager.asm dhcpUtils.asm\
                  disk.def drive.def driver.def ec.def file.def font.def\
                  fontID.def gcnlist.def geode.def geos.def geoworks.def\
                  graphics.def heap.def hwr.def iacp.def initfile.def\
                  input.def library.def lmem.def localize.def medium.def\
                  object.def resource.def sac.def sem.def sllang.def\
                  socket.def sockmisc.def stylesh.def sysstats.def\
                  system.def text.def thread.def timedate.def timer.def\
                  token.def uDialog.def ui.def vm.def win.def
dhcpec.geo dhcp.geo: geos.ldf ui.ldf socket.ldf accpnt.ldf
