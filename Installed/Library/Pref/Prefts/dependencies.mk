prefts.rdef: generic.uih product.uih config.uih Objects/colorC.uih \
                Art/mkrPrefTS.ui
prefts.obj \
prefts.eobj: geos.def heap.def geode.def resource.def ec.def \
                library.def object.def lmem.def graphics.def fontID.def \
                font.def color.def gstring.def text.def char.def win.def \
                ui.def file.def vm.def input.def hwr.def localize.def \
                ...
                config.def Objects/colorC.def prefts.def prefts.rdef

preftsEC.geo prefts.geo : geos.ldf ui.ldf config.ldf 